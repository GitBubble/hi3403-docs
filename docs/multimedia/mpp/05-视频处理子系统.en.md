...
>![](public_sys-resources/icon-note.gif) **Note:** 
>-   Offline mode includes OT_VI_OFFLINE_VPSS_OFFLINE, OT_VI_ONLINE_VPSS_OFFLINE, and OT_VI_PARALLEL_VPSS_OFFLINE — in other words, any mode where both VI and VPSS are offline counts as offline mode.
>    For SS528V100/SS625V100/SS524V100/SS522V101/SS626V100, it is not necessary to distinguish which specific offline mode is being used.
...
获取指定 VPSS Group Input图像区域的亮度总和以及获取的图像的宽高信息。该接口主要用于对Input图像中即将进行 Overlay 或 OverlayEx 叠加的区域进行亮度和统计，根据统计的亮度，对 Overlay 或 OverlayEx 进行反色处理，使叠加的 Overlay 或OverlayEx 不至于和背景混淆。
...
-   The Group must be created first.
-   此接口仅在VPSS通过接口[ss\_mpi\_vpss\_send\_frame](#ZH-CN_TOPIC_0000002408100282)接收图像时，使能用户自己做帧率控制，如果VPSS通道工作在AUTOMode绑定到VO，此时VO的通道帧率控制不生效。
...
-   禁止用户自己做帧率控制时，VO通道帧率控制仅在VPSS为AUTOMode下生效，如果VPSS为USERMode，要求VPSS通道要绑定到某个模块，或者及时取走通道队列中的图像，否则VPSS接收图像会Failure。
...
-   对于Hi3403V100，先开启Low Latency单buffer再开启attach vb pool，Low Latency单buffer优先级更高，通道使用的VB为Low Latencybuffer。如果先attach vb pool，然后开启Low Latency单buffer，再detach vb pool，通道依然使用用户VB池中的VB，如果要释放USER VB，需要先关闭Low Latency单buffer，然后再detach vb pool。
...
-   仅支持OT\_FRAME\_INTERRUPT\_START、OT\_FRAME\_INTERRUPT\_EARLY和OT\_FRAME\_INTERRUPT\_EARLY\_END三种类型。OT\_FRAME\_INTERRUPT\_EARLYMode，VPSS在延时中断上报时发送图像，如果后端接收模块处理速度比VPSS处理要快，可能会出现花屏、解压错误等问题。三种中断类型的优缺点如[表1](#_Ref67299596)所示。
...
-   如果Physical Channel非紧凑段压缩和鱼眼功能同时开启，只有当校正区域数为1，且该区域可完全覆盖Output范围时，可不打背景色，其他情况必须打上背景色，即bg\_color\_en参数必须设置为TD\_TRUE。
...
-   Hi3403V100鱼眼仅支持Low LatencyInput，不支持Low LatencyOutput，VPSSPhysical Channel开启Low Latency和鱼眼，后端模块收到的是非Low Latency帧。
...
根据鱼眼矫正Output图像Coordinates点查找源图像Coordinates点。
...
-   OT_LDC_V3 parameters must use values from the calibration tool; arbitrarily configured values are not supported.
...
-   Hi3403V100Low Latency通道绑编码场景下，当有多个码流时，若设置编码通道优先级，此时Low Latency通道行数设置不能过小，建议设置128行以上，否则可能会引起非Low Latency通道阻塞不编码。
-   Hi3403V100在Low Latency通道绑编码场景下，编码不支持软件抽场。在Low Latency单buffer绑定编码场景下，编码不支持去呼吸效应，不能开启重复编码的功能，包含增帧Mode、超大帧重编码、码率波动导致的重编码，用户应把最大重编码次数设置为0。
...
-   [ot\_vpss\_ldci\_he\_weight\_attr](#ZH-CN_TOPIC_0000002408100146): Defines the LDCI histogram equalization and original histogram fusion weight attribute parameters.
...
-   Ifcrop\_mode as OT\_COORD\_RATIO(toCoordinatemode), Usescrop\_rectMemberwhenShoulddoConverts, Calculation Formula as :
    -   x = start_coordinate_x × raw_image_width / 1000, valid range: \[0, 999\]. After calculation, the result is rounded and aligned. The same formula applies to vertical coordinate calculation.
...
-   When Hi3403V100 3DNR is set to video mode and temporal denoising is enabled, end-to-end latency increases by one frame time (e.g., for a 30fps sensor, end-to-end latency increases by ~33ms). This can be mitigated by enabling `nr_low_del`ay\_enModule parameterDivideThislatency.nr\_low\_delay\_enClosewhen, Close3DNREnableOrCloseReferenceFrameEnable, willDroponeFrameImage.
...
It is recommended to first select the adaptive mode OT_VPSS_DEI_MODE_AUTO.
...
-   `max_width` and `max_height` are used to allocate the 3DNR reference frame and reconstruction frame buffers. The allocated memory size is calculated using width and height rounded up to 16-byte alignment.
...
-   The reconstruction frame compression type in `nr_attr` is also a static attribute — it must be set when creating the Group and cannot be changed afterward.
...
-   InputOutputMultiplexbufferFunctionHasThe followingConstraintOrNotes:
    ...
    -   must enable3DNR; 
    -   For SS528V100/SS625V100/SS524V100/SS522V101, images with width exceeding 4096 are not supported. When input width exceeds 2688, inconsistent scaling (e.g., width enlarged while height shrunk, or vice versa) is not supported, nor is pixel format conversion. When width exceeds 2688, the VB size needed by VI is larger than when multiplexed buffering is not enabled; an adjustment value width_adjust must be added to the width when calculating VB size, and a further adjustment vb_adjust must also be added to the final VB size. Specifics are as follows:
    ...
    -   For SS528V100/SS625V100/SS524V100/SS522V101, when the channel format or compression changes during scaling and the required VB changes, a few frames during the transition cannot be shared, causing frame drops.
...
-   After enabling the input/output multiplexed buffer function, when the related constraint parameters mentioned above change (other than 3DNR), VPSS will adaptively enable or disable the multiplexed buffer function.
...
-   When `chn_mode` is AUTO mode, settings for width, height, aspect_ratio, compress_mode, pixel_format, frame_rate, and video_format do not take effect immediately — they take effect after the backend module confirms; depth does not take effect; and images will not enter the channel queue.
...
-   AspectCompareParameterChannel attributeSet.OnlyVPSSChannelSet as USERmodeOnly Whentakes effect, AUTOmodeunderAspectCompareParameterbyVOSet.
    -   When set to OT_ASPECT_RATIO_MANUAL mode, users manually set the image start coordinates and dimensions. VPSS outputs the image with black borders added. The image display region must not exceed the output image dimensions.geWidthHeight.
...
-   Due to inherent limitations of the scaling algorithm, certain special images (such as black-and-white stripes) may exhibit pixel duplication at edges after scaling. In this case, you can use the ss\_mpi\_vpss\_set\_chn\_scale\_coef\_type interface to set the dual-linear scaling coefficient type to mitigate this effect.
...
-   The larger the four denoising-related parameter values, the quieter the picture. `tf_strength` is especially effective at denoising flat regions. If strong motion smearing or image ghosting appears, it is recommended toNeedwillwhendomainRelatedParameterreduce\(tf\_strength  And  ctf\_strength\).
-   sf\_strength (afterAbbreviation SFS)DirectlyAffectPictureMotionRegionnoisemeter tf\_strength(afterAbbreviation TFS)DirectlyAffectPictureStaticRegionnoisemeter, For examplelargechipFlatRegionnoisemeter.
    -   Excessive SFS strength causes overall image edge blur and fine detail loss. Excessive TFS strength causes visible smearing during motion. The recommended debug strategy is to increase SFS first. If fine detail loss is severe while static-region noise still cannot be effectively eliminated, then appropriately enhance the TFS effect while maintaining clarity, achieving an optimal balance between smearing and picture cleanliness.
...
-   `cvbs_strength` is a relatively independent function used to process luminance-chrominance crosstalk that occurs with standard-definition input. The higher the configured strength, the stronger the crosstalk suppression. However, for HD input, strong edges in the picture may exhibit more severe color residue. Therefore, if adjusting CTFS to its minimum still results in visible color residue with HD input, please adjust this parameter.
...
-   During service operation, if the ISO value from VPSS falls within a certain interval, the actual NR parameters are derived by interpolating between the standard parameters at the left and right boundaries of that interval.
...
-   `hard_tf_strength` (HTFR) and `weak_tf_strength` (WTFR) respectively represent the fusion ratio of temporal and spatial denoising results for the strong denoising channel and the weak denoising channel. When using, adjust based on the current application scenario. Setting these parameters larger means more visible motion smearing but better denoising effect. SS528V100/SS625V100/SS524V100/SS522V101 do not support `weak_tf_strength`; setting it will have no effect.
...
-   `edz` is used to determine whether a region is a strong edge. Regions above the threshold are judged as edge regions, where spatial denoising capability is enhanced and the picture becomes slightly blurred. If this value is set too low, visible jagged artifacts will appear at strong edges and temporal denoising smearing will increase. Therefore, it is recommended to set the value to 63 and not make excessively large adjustments.

...
>-   Unless otherwise specified, Hi3519AV200 content is consistent with Hi3403V100.
...
Hi3403V100 functional specifications refer to the data flow diagram in Figure 5.
...
**Figure 5**  Hi3403V100 VPSS Data Flow Diagram<a name="fig076104110215"></a>  
...
>![](public_sys-resources/icon-note.gif) **Note:** 
>-   Only Hi3403V100 groups in\[[OT\_VPSS\_VGS\_GRP\_NO](#ZH-CN_TOPIC_0000002441659549),  [OT\_VPSS\_MAX\_GRP\_NUM](#ZH-CN_TOPIC_0000002441699353)  - 1\]rangeSupportedInputTile64x16Format.
...
>![](public_sys-resources/icon-note.gif) **Note:** 
>Only Hi3403V100 groups in\[[OT\_VPSS\_VGS\_GRP\_NO](#ZH-CN_TOPIC_0000002441659549),  [OT\_VPSS\_MAX\_GRP\_NUM](#ZH-CN_TOPIC_0000002441699353)  - 1\]rangeSupportedInputTILE compression.
...
>    SS528V100/SS625V100/SS524V100/SS522V101/SS626V100NoneNeedclose注Offline ModeSpecific point to/indicate哪kindmodeOffline.
>    Hi3403V100Offline Mode only supportsOT\_VI\_OFFLINE\_VPSS\_OFFLINE, OT\_VI\_ONLINE\_VPSS\_OFFLINE.
>-   Hi3403V100 OT\_VI\_ONLINE\_VPSS\_ONLINEandOT\_VI\_OFFLINE\_VPSS\_ONLINEmode, ActualInputResolutionLimitGet determineAtFrontendVIModule.
...
>![](public_sys-resources/icon-note.gif) **Note:** 
>Only Hi3403V100 groups in\[0,99\]rangeSupportedOutputTile16x8Format.
...
>-   When Hi3403V100 VPSS physical channel downscaling exceeds 15x (14x when input image width is greater than 4096 and channel 0 has compression enabled), an additional temporary buffer is needed, with buffer width and height both 2x the output image size. VB Blk allocation must account for this configuration. This also requires one extra DDR read/write.
...
>-   When Hi3403V100 VPSS channel 1 outputs Tile16x8 format, the output image width must not exceed 4096.
...
**Table 11**  Hi3403V100 VPSSPhysical channelcompression outputwhenFunctionLimit
...
>-   Hi3403V100 VPSS: Compact segment compression input/output is not supported when input image width is greater than 4096; compact segment compression output is not supported when output width is greater than 4096.
...
>-   Hi3403V100 extension channels can output OT\_PIXEL\_FORMAT\_VY1UY0\_PACKAGE\_422, only supporting UVC connection for YUY2 output.
...
-   Hi3403V100 VI\_ONLINE\_VPSS\_ONLINEmodeonly supports creating group 0 and groups in\[[OT\_VPSS\_VGS\_GRP\_NO](#ZH-CN_TOPIC_0000002441659549),  [OT\_VPSS\_MAX\_GRP\_NUM](#ZH-CN_TOPIC_0000002441699353)  - 1\]range.
...
-   The Group must be created first.
...
-   The Group must be created first.
-   The Group attributes must be valid. Some static attributes cannot be set dynamically. For specific functional limitations, refer to [ot\_vpss\_grp\_attr](#ZH-CN_TOPIC_0000002441659581).
...
-   The Group must be created first.
-   For each member of `grp_param`, refer to the [ot\_vpss\_grp\_param](#ZH-CN_TOPIC_0000002408100322) structure description for value ranges, functional limitations, and 3DNR tuning guidance.
-   Be sure to first call [ss\_mpi\_vpss\_get\_grp\_param](#ZH-CN_TOPIC_0000002408260178) to get the current parameters, then perform the set operation.
...
-   The Group must be created first.
-   `grp_adv_param` contains 3DNR advanced parameters; for member values and tuning guidance, refer to the [ot\_vpss\_nrx\_adv\_param](#ZH-CN_TOPIC_0000002441699325) structure description.
-   Be sure to first call [ss\_mpi\_vpss\_get\_grp\_nrx\_adv\_param](#ZH-CN_TOPIC_0000002441659505) to get the current parameters, then perform the set operation.
-   Hi3403V100/SS626V100 do not support this interface.
...
-   The Group must be created first.
-   For specific member values and functional limitations, refer to the [ot\_vpss\_crop\_info](#ZH-CN_TOPIC_0000002441699337) structure description.
-   The crop region size cannot be smaller than the minimum input resolution supported by VPSS, nor larger than the maximum input resolution supported by VPSS. If the crop width exceeds the input image width, the crop output width is adjusted to the input image width; if the crop height exceeds the input image height, the crop output height is adjusted to the input image height. The crop region origin does not support negative coordinates; its right boundary cannot exceed the maximum input width supported by VPSS, and its bottom boundary cannot exceed the maximum input height supported by VPSS.
-   On SS528V100/SS625V100/SS524V100/SS522V101/SS626V100, when the crop region exceeds the image bounds, the out-of-range portion is discarded; the crop coordinates configured by the caller are preserved.
-   On Hi3403V100, when the crop region exceeds the image bounds, the crop coordinates are shifted toward the origin; the crop width/height configured by the caller are preserved.
...
-   The Group must be created first.
-   The caller may perform frame-rate control manually when using this interface.
-   Before calling this interface when the VPSS channel is in AUTO mode, VPSS must be started and bound to a downstream receiver; alternatively, set the VPSS channel to USER mode and let the caller retrieve frames via the get-frame interface.
-   For compressed input, the width/height alignment requirements must match those in the "[Input/Output Characteristics](#ZH-CN_TOPIC_0000002408100254)" section. For uncompressed input, alignment is automatically rounded down (crop).
-   Only offline mode supports this interface.
-   If the frame sent to VPSS is constructed by the caller (not obtained from another module), the `frame_flag` field of `ot_video_frame` must be initialized to 0.
...
-   The Group must be created first.
-   This interface applies to all VPSS channels, including physical channels and extension channels.
-   Frames can be retrieved only in USER mode and only when the queue depth is non-zero.
-   Calling this interface to retrieve frames does not affect the bound downstream module. For example, if VO is bound for display, the retrieved frames can still be displayed by VO without interruption.
-   When `milli_sec` is -1, blocking mode is used: the call always waits until a frame is available before returning. When `milli_sec` is 0, non-blocking mode is used. When `milli_sec` is greater than 0, timeout mode is used: the call waits at most `milli_sec` milliseconds, and if no frame is available within that time, the call returns due to timeout.
-   In the decode-and-rewind scenario, since frame drops are not allowed, as long as any one VPSS channel has not yet processed the new image (with the channel already enabled), the entire VPSS will stop processing new images. For example, if channels 0 and 1 are enabled, neither is bound to a downstream module, and both channel image queue lengths are 2: at most 2 already-buffered frames can be retrieved from channel 0 — because once channel 1 has 2 buffered frames it stops processing new images, the entire VPSS will also stop processing new images.
-   In online mode, when the interrupt mode is set to `OT_FRAME_INTERRUPT_EARLY`, frames retrieved from the physical channel may be images that VPSS hardware has not yet finished processing.
-   When channel low-latency is enabled, the retrieved frame is not a complete frame; the low-latency line number must be configured appropriately to ensure that reading the image does not produce errors.
...
This interface must be used together with [ss\_mpi\_vpss\_get\_chn\_frame](#ZH-CN_TOPIC_0000002441659661).
...
Gets one raw image frame from the Group. Main use case: in HD device decode-and-rewind scenarios requiring pause or step-in, the PIP layer and normal video layer display the same frame on two channels. This function can be implemented by using this interface together with [ss\_mpi\_vpss\_send\_frame](#ZH-CN_TOPIC_0000002408100282) and other related interfaces.
...
-   The Group must be created and started, and at least one channel must be enabled.
-   Retrieved frames must be released; otherwise VB will run out, or playback may stall during decode-and-rewind. It is recommended to use this interface together with [ss\_mpi\_vpss\_release\_grp\_frame](#ZH-CN_TOPIC_0000002441699393).
-   When backup frame is enabled, the backup frame is retrieved; otherwise the current frame is retrieved.
...
-   In practice, the `grp` parameter of this interface is not actually used; it may be set to any value within its valid range.
-   This interface must be used together with [ss\_mpi\_vpss\_get\_grp\_frame](#ZH-CN_TOPIC_0000002408100214).
-   Only Offline mode supports this function.
...
Gets the luma sum of a specified VPSS Group input image region and the width/height information of the retrieved image. This interface is mainly used to perform luma statistics on regions about to undergo Overlay or OverlayEx compositing in the input image, and to perform inverse colorprocessing ，make/enable overlay Overlay or OverlayEx not  to/untilin/at and background scene 混淆。
...
-   The Group must be created first.
-   This interface targets the VPSS Group input image. The luminance statistics are gathered before these images are processed by the VPSS hardware.
-   The number of statistics regions ranges from \[1, 64\].
-   Statistics region coordinates and width/height must be 2-pixel aligned.
-   The statistics region width/height minimum is 2x2, maximum is 16384x8192.
...
-   When the input image uses compact-segment compression, SS528V100/SS625V100 do not support retrieving input image luminance and width/height information.
...
Gets the luminance sum of a specified image region. This interface is mainly used to perform luminance sum statistics on OSD regions. Based on the measured luminance, inverted color processing is applied to make the OSD region more visually distinct.
...
<a name="ul5479mcpsimp"></a><a name="ul5479mcpsimp"></a><ul id="ul5479mcpsimp"><li>Set as -1when as Blockingmode; </li><li>SS528V100/SS625V100/<span xml:lang="es-ES" id="ph5482mcpsimp"><a name="ph5482mcpsimp"></a><a name="ph5482mcpsimp"></a>SS524V100/SS522V101Set</span>milli_secEqual to0when as Non-Blockingmode, Hi3403V100/SS626V100Setmilli_secEqual to0when as Blockingmode; </li><li>Greater Than0when as TimeoutWaitsmode.TimeoutUnit as ms(ms).</li></ul>
</td>
...
-   The Group must be created first.
-   This interface targets VPSS channel output images. The luminance statistics are gathered before these images are retrieved by the user or sent to a downstream module.
-   The number of statistics regions ranges from \[1, 64\].
-   Statistics region coordinates and width/height must be 2-pixel aligned.
-   The statistics region width/height minimum is 2x2. For SS528V100/SS625V100/SS524V100/SS522V101/SS626V100, the maximum is 16384x8192; for Hi3403V100, the maximum is 8192x8192.
-   The luminance region must not exceed the image boundary.
...
Sets VPSS physical channel rotation attributes.
...
-   The Group must be created first.
-   The channel attributes must already be set.
-   Fixed-angle rotation at 90° and 270° only supports uncompressed output. On SS528V100/SS625V100/SS524V100/SS522V101/SS626V100, 180° rotation supports compact-segment and non-compact-segment compressed output but not line-compressed output. On Hi3403V100, fixed-angle 180° rotation only supports uncompressed output; users can enable mirror and flip on the channel to achieve 180° rotation, which then also supports compressed output.
-   SS528V100/SS625V100/SS524V100/SS522V101 only support fixed-angle rotation.
-   On SS528V100/SS625V100/SS524V100/SS522V101/SS626V100, when enabling 90°/270° fixed-angle rotation, the physical channel pixel format cannot be set to semi-planar 4:2:2.
-   On Hi3403V100, when rotation is enabled, the physical channel pixel format cannot be set to semi-planar 4:2:2. When implementing 180° rotation by enabling mirror and flip, the pixel format can be set to semi-planar 4:2:2.
-   Fixed-angle rotation only supports 0°, 90°, 180°, and 270°.
-   Fixed-angle rotation only supports OT_VIDEO_FORMAT_LINEAR format output.
-   Rotation is not supported in channel AUTO mode.
-   For related usage notes, refer to the `ot_rotation_attr` structure in the System Control chapter.
-   On Hi3403V100/SS626V100, when VPSS simultaneously enables arbitrary-angle/high-precision arbitrary-angle rotation and non-compact-segment compression, VPSS hardware compression is adaptively disabled and compression is written out after rotation.
    -   On Hi3403V100, when enabling arbitrary-angle/high-precision arbitrary-angle rotation, the physical channel compression mode cannot be set to compact-segment compression.
    -   On SS626V100, when enabling arbitrary-angle/high-precision arbitrary-angle rotation, the physical channel compression mode cannot be set to compact-segment compression or line compression.
...
Gets VPSS physical channel rotation attributes.
...
-   The Group must be created first.
-   Only offline mode supports this function.
-   When backup frame is enabled, one additional VB block is consumed.
-   Backup frame is disabled by default, which affects VO picture switching, playback control, and other functions. Enable backup frame if these functions are needed.
...
Sets channel sharpening parameters. Processed using the LTI/CTI method.
...
-   The Group must be created first.
-   On SS528V100/SS625V100/SS524V100/SS522V101, channel 3 does not support sharpening.
-   On SS626V100, channels 2 and 3 do not support sharpening.
-   Hi3403V100 does not support this interface.
...
Gets channel sharpening parameters.
...
-   The Group must be created first.
-   Hi3403V100does not support this interface.
...
Sets the delay queue length for VPSS delayed-start processing.

Delay processing is mainly needed because images transmitted from the front-end to the main chip via PCI may have delays. To maintain synchronization, VPSS uses a delay queue to delay the main-chip images correspondingly. Another use case: the user retrieves an image from VI, draws bounding boxes via the TDE module, and simultaneously sends the same frame to VPSS for processing. Since VPSS processes faster than TDE, VPSS needs to perform delay processing.
...
Gets the delay queue length for VPSS delayed-start processing.
...
When VPSS receives images in non-binding mode, enables user frame-rate control. If the VPSS channel operates in AUTO mode and is bound to VO, VO channel frame-rate control does not take effect.
...
-   Calling this interface multiple times returns success.
-   The Group must be created first.
-   This interface is only valid when VPSS receives images via the interface [ss\_mpi\_vpss\_send\_frame](#ZH-CN_TOPIC_0000002408100282)receiveimage when, enableuser self/own  doframe ratecontrol ，ifVPSSchannel工作in/at AUTOModebindto VO，at this time VO's channelframe ratecontrol  does not take effect.
-   When VPSS receives images via [ss\_mpi\_vpss\_send\_frame](#ZH-CN_TOPIC_0000002408100282), user frame-rate control is enabled by default.
...
When VPSS receives images in non-binding mode, disables user frame-rate control. VO channel frame-rate control then takes effect.
...
-   Repeated disabling returns Success.
-   The Group must be created first.
-   This interface is only valid when VPSS receives images via the interface [ss\_mpi\_vpss\_send\_frame](#ZH-CN_TOPIC_0000002408100282)receiveimage when, Disabling user-controlled frame rate; the VO channel frame rate control takes effect.
-   disableuser self/own  doframe ratecontrol  when, VOchannelframe ratecontrol onlyin/at VPSSis/for AUTOModebelow/under takes effect ，ifVPSSis/for USERMode，requirements VPSSchannel requirebindto a certain  moduleblock ，or  and  when retrieve/get走channel queuecolumn  in image，otherwiseVPSSreceiveimagewill Failure。
...
Sets VPSS size filter information. Images that meet the filter criteria proceed with subsequent processing; those that do not are dropped.
...
-   Size filtering is mainly used in front-end mixed capture scenarios.
-   Size filtering is at the very front of VPSS processing, affecting all subsequent processing.
-   When using the [ss\_mpi\_vpss\_send\_frame](#ZH-CN_TOPIC_0000002408100282) interface to send user images, this is not affected.
-   This interface is not supported on Hi3403V100/SS626V100.
...
Gets the VPSS size filter information.
...
-   This interface can only be called when no groups have been created.
-   For parameter descriptions, refer to the [ot\_vpss\_mod\_param](#ZH-CN_TOPIC_0000002441699477) structure.
-   Be sure to first call [ss\_mpi\_vpss\_get\_mod\_param](#ZH-CN_TOPIC_0000002408100246) to get the current parameters before performing the set operation.
...
Gets the device file handle corresponding to the VPSS channel.
...
Users can use the returned file handle to implement priority-based selection for retrieving video frame data from multiple channels.
...
Closes the file descriptors for the device and channel.
...
Binds a VPSS channel to a video buffer VB pool.
...
-   The group must have been created.
-   Users must call ss\_mpi\_vb\_create\_pool to create a video buffer VB pool, then call this interface to bind the current group's channel to the VB pool with the fixed pool ID.
-   Multiple channels from multiple groups can be bound to the same VB pool.
-   To switch the VB pool bound to the current group, simply call this interface again with the correct VB pool configuration.
-   vb\_pool must be a valid Pool ID of an already-created VB pool.
-   Only valid for channels in USER mode; AUTO mode channels are not affected.
-   in/at call[ss\_mpi\_vpss\_detach\_vb\_pool](#ZH-CN_TOPIC_0000002441699433) after, destroycreate's VBbefore ，mustensure VBno/without is VPSSafter  endbind's  moduleblock use ，can via sleepor clear backend moduleblock channelbuffer's method/mode first set VBall release，again/then destroybufferVB pool。
-   UserVBsize based on VPSSchannelOutputimagecalculate ，specific calculate  public stylereference ot\_buffer.h。
-   After binding, all VBs requested by the channel are obtained from this VB pool; after unbinding, they are obtained from the common VB pool.
-   When buffer share and attach vb pool functions are both enabled, buffer share has higher priority and the channel uses the input VB.
-   for Hi3403V100，first enableLow Latencysingle-bufferagain/then enableattach vb pool，Low Latencysingle-bufferpriority more height ，channeluse 's VBis/for Low Latencybuffer。iffirst attach vb pool， then/naturallyafter enableLow Latencysingle-buffer，again/then detach vb pool，channel依 then/naturallyuse user VB pool in VB，if requirereleaseUSER VB，mustfirst closeLow Latencysingle-buffer， then/naturallyafter again/then detach vb pool。
-   for Hi3403V100，enablemcf\_en's Grouponlychannel0supportsbind/unbindVB pool。
-   Binding a VB pool and creating a VPSS group must be performed in the same process.
...
Unbinds a VPSS channel from a video buffer VB pool.
...
-   The Group must be created first.
-   For functional restrictions on the values of ldci\_attr members, refer to [ot\_vpss\_ldci\_attr](#ZH-CN_TOPIC_0000002441659477)structure Description。
-   Be sure to first call [ss\_mpi\_vpss\_get\_ldci\_attr](#ZH-CN_TOPIC_0000002441699285) to get the current parameters before performing the set operation.
-   The maximum image width supported by the LDCI function is 7680, and 2688 under compact segment compression.
-   After enabling the LDCI function, if the actual input width exceeds 7680, VPSS will adaptively enable/disable the LDCI function.
-   When the LDCI function is enabled, memory required by the attribute function is allocated; when the attribute switch is disabled, the corresponding memory is released.
-   This interface is only supported on SS524V100/SS522V101.
...
Sets the VPSS Group fisheye lens LMF parameters.
...
-   The Group must be created first.
-   Only Hi3403V100/SS626V100 supports this interface.
...
Gets the VPSS Group fisheye lens LMF parameters.
...
Sets the frame interrupt attributes used in VPSS Online mode.
...
-   The VPSS group number must be 0.
-   Only valid in VI\_ONLINE\_VPSS\_ONLINE mode.
-   This interface must be called after ss\_mpi\_sys\_init is configured.
-   All physical channels must be closed before modifying the interrupt mode.
-   onlysupportsOT\_FRAME\_INTERRUPT\_START、OT\_FRAME\_INTERRUPT\_EARLYand OT\_FRAME\_INTERRUPT\_EARLY\_ENDthree  typeType。OT\_FRAME\_INTERRUPT\_EARLYMode，VPSSin/at latency interrupt report  whensendimage，ifafter  endreceive moduleblock processing 速 degree compareVPSSprocessing  require快，may will appears screen corruption、decompression error , etc.issue/problem 。three  typeinterrupt Type's  better/optimalartifact/defect pointsuch as/if [ table1](#_Ref67299596)as shown。
-   Only Hi3403V100 supports this interface.
-   early\_line is used to set the number of lines VPSS hardware collects before reporting the delayed interrupt.

**Table 1** Advantages and disadvantages of VPSS Online interrupt types

<a name="_Ref67299596"></a>
<table><thead align="left"><tr id="row3312mcpsimp"><th class="cellrowborder" rowspan="2" valign="top" id="mcps1.2.7.1.1"><p id="p3314mcpsimp"><a name="p3314mcpsimp"></a><a name="p3314mcpsimp"></a>Online中断类型</p>
...
Gets the frame interrupt attributes used in VPSS Online mode.
...
-   This interface must be called after ss\_mpi\_sys\_init is configured.
-   Only Hi3403V100 supports this interface.
-   The default mode is OT\_FRAME\_INTERRUPT\_START with delay line number 0; destroying the group resets the frame interrupt attributes to default.
...
Sets the VPSS physical channel fisheye attributes.
...
-   The Group must be created first.
-   Only Hi3403V100/SS626V100 support this interface.
-   The channel number must be a VPSS physical channel.
-   The corresponding physical channel attributes must be configured before configuring the fisheye attributes.
-   After enabling fisheye correction on the physical channel, dynamic modification of channel Crop attributes, channel rotation attributes, and channel width/height attributes is not supported.
-   ifPhysical Channelnon-compact segmentcompressionand fisheyefunctionsimultaneously enable，will adaptivelycloseVPSShardware compression，fisheye correctionafter compressionOutput。
-   ifPhysical Channelnon-compact segmentcompressionand fisheyefunctionsimultaneously enable，onlyhas/have when  correction/calibrate positive/correctregion  numberis/for 1， andthe/this region  can complete全覆盖Outputrange  when,  cannot  enable/openbackground scene  color，other condition/casemust enable/openon/above background scene  color, i.e., bg\_color\_enparametermustSets is/for TD\_TRUE。
-   enablefisheyefunction when, Physical Channelpixel formatcannot Sets is/for semi-planar422，compressionModeonlysupports non- compressand non-compact segmentcompression。
-   enablefisheyefunction when, onlysupportsOT\_VIDEO\_FORMAT\_LINEARformatOutput。
-   Hi3403V100channelOutputVIDEO\_FORMAT\_TILE\_16x8format when, not supportedfisheye。
-   Hi3403V100fisheyeonlysupportsLow LatencyInput，not supportedLow LatencyOutput，VPSSPhysical ChannelenableLow Latencyand fisheye，backend moduleblock 收to 's is  non-Low Latencyframe。
-   SS626V100fisheyenot supportedLow Latency，VPSSPhysical ChannelenableLow Latencyand fisheye，Low Latency does not take effect.
-   channelAUTOModebelow/under not supportedfisheye。
-   fisheye correctionhas/have minimum 's InputOutputwidth/heightlimit : OT\_FISHEYE\_MIN\_IN\_IMAGE\_HEIGHT, etc.refer to “geometric distortion correction  positive/correctsub-system ”chapter 。
...
Gets VPSSPhysical Channelfisheyeattribute。
...
based on fisheye correctionOutputimageCoordinates point查找 sourceimageCoordinates point。
...
-   Only Hi3403V100/SS626V100 support this interface.
-   The channel number must be a VPSS physical channel.
-   callthis interfacebefore mustfirst enablefisheye correctionfunction。
-   startfisheye correction after, ifmustchange parameter， require遵循parameterchange 、Gets  correction/calibrate positive/correctimageand startCoordinates pointquery 's sequentially  originalthen 。ifin/at fisheyeparameterchange before then Gets correction  positive/correctimage，may causes 查找to error 's Coordinates point。
...
Sets the VPSS Lens Distortion Correction (LDC) attribute.
...
-   Only Hi3403V100 supports this interface.
-   The channel number must be a VPSS physical channel.
-   configureLDCattributebefore mustfirst configurecorresponding Physical Channel's attribute。
-   ifPhysical Channelnon-compact segmentcompressionand LDCfunctionsimultaneously enable，will adaptivelycloseVPSShardware compression，LDCprocessing after compressionOutput。
-   enableLDCfunction when, Physical Channelpixel formatcannot Sets is/for semi-planar422，compressionModeonlysupports non- compressand non-compact segmentcompression。
-   channelOutputVIDEO\_FORMAT\_TILE\_16x8format when, not supportedLDC。
-   启 useLDC when, VPSS's COVERmay will 变 shape，at this time recommendeduse COVEREX to generation/represent替COVER。
-   when enablespread when, OT\_LDC\_V1's distortion\_ratiovalue rangerange only capabilityis/for  \[0, 500\], i.e., onlysupportsLDC's  bucket shapeMode。
-   due to LDCwill for/to imageperform 几何拉伸or 几何compression，when Inputimagemay exist  clear/bright显噪声，in/at  bucket typecorrection  positive/correctpast/too  after, may may exist image四周噪声颗粒变large （四周is 拉伸）'s phenomenon ， canwill LDCconfigurein/at after improve this phenomenon 。噪声relatively large 's scenario recommendedin/at VPSS moduleblock configureLDC，not recommendedin/at VI moduleblock configureLDC。
-   OT\_LDC\_V3's parameter needuse 标定工具's parameter，cannot  usefollows 意configure's parameter。
-   LDCcorrection  positive/correcthas/have minimum 's width/heightlimit : OT\_LDC\_MIN\_IMAGE\_WIDTH，OT\_LDC\_MIN\_IMAGE\_HEIGHTrefer to “geometric distortion correction  positive/correctsub-system ”chapter 。
...
Gets VPSS镜头畸变correction  positive/correct（LDC）attribute。
...
Queries the input image coordinates corresponding to a given LDC output image coordinate point.
...
-   Only Hi3403V100 supports this interface.
-   The channel number must be a VPSS physical channel.
-   This interface only supports OT\_LDC\_V1 version LDC correction.
...
Queries the output image coordinates corresponding to a given LDC input image coordinate point.
...
Sets VPSSPhysical Channelspread attribute。
...
-   Only Hi3403V100 supports this interface.
-   The channel number must be a VPSS physical channel.
-   configurespread attributebefore mustfirst configurecorresponding Physical Channel's attribute。
-   ifPhysical Channelnon-compact segmentcompressionand spreadfunctionsimultaneously enable，will adaptivelycloseVPSShardware compression，spreadprocessing after compressionOutput。
-   enablespreadfunction when, Physical Channelpixel formatcannot Sets is/for semi-planar422，compressionModeonlysupports non- compressand non-compact segmentcompression。
-   channelOutputVIDEO\_FORMAT\_TILE\_16x8format when, not supportedspread。
-   when enablespread when, OT\_LDC\_V1's distortion\_ratiovalue rangerange only capabilityis/for   \[0, 500\], i.e., onlysupportsLDC's  bucket shapeMode。
...
Gets VPSSPhysical Channelspread attribute。
...
-   The Group must be created first.
-   specific Membersvalue rangeand functionlimit reference [ot\_vpss\_crop\_info](#ZH-CN_TOPIC_0000002441699337)structure  body/structDescription。
-   Only Hi3403V100 supports this interface.
-   Physical Channelfirst  do scale enlargeafter  docrop；Extension Channelfirst  docropafter  do scale enlarge。
-   ifcropregion exceeds imagerange ，discard exceeds some/partial image， better/optimalfirst ensure cropCoordinateswith/and  all thatSets 's parameter mutual same。
-    sub-block processing  when, channel0not supportedcrop。
-   Sets channelcropbefore mustfirst Sets channelattribute。
-   channelAUTOModewith/and channelcropfunctionare mutually exclusive.
-   channelenableany corner  degreerotation 、LDC、spread、fisheye whennot supportedchannelcrop。
...
Sets VPSSchannelLow Latencyparameter。
...
-   The Group must be created first.
-   specific Membersvalue rangereference [ot\_low\_delay\_info](#ss_low_delay_info)structure  body/structDescription。
-   Only Hi3403V100/SS626V100 support this interface.
-   Sets Low Latencyparameterbefore mustfirst Sets channelattribute。
-   Extension ChannelLow Latencynot supportedsingle-buffer。
-   Low Latencysingle-bufferonlysupports sendVENC， sendother  moduleblock  whenmay due to processing not and  when, causes VPSSprocessing Failure。
-   SS626V100not supportedLow Latencysingle-buffer。
-   Low Latencynot supportedFlip。
-   SS626V100Low Latencynot supportedrotation 180 degree。
-   Hi3403V100Low Latencychannel绑encodingscenario  under, when has/have multiple stream when, if Sets encoding channelpriority ，at this time Low Latencychannelline  numberSets cannot past/too small ，recommendedSets 128line above ，otherwisemay will 引起 non-Low Latencychannelblocking not encoding。
-   Hi3403V100in/at Low Latencychannel绑encodingscenario  under, encodingnot supportedsoftware 抽 field/scene。in/at Low Latencysingle-bufferbindencodingscenario  under, encodingnot supportedde-breathing effect，cannot enablerepeated encoding's function，contains  increaseframeMode、 exceed/ultralarge frame re-encoding、bitratefluctuationcauses 's  re-encoding，user  applyset maximum  re-encoding time numberSets is/for 0。
-   SS626V100 VPSSchannelUSERMode when, after  endbind moduleblock ifnot is VPSS/VO，then adaptiveclosechannelLow Latency；VPSSchannelAUTOMode when, after  endbind moduleblock ifnot is VO，or  bindVO，but VOnot is SingleMode，then adaptiveclosechannelLow Latency。
-   Low Latencyline  number/IDcannot less than 16，cannot greater than imageheight  degree；ifis Physical Channel andenablechannelcrop，then cannot greater than cropheight  degree。
-   Hi3403V100 OT\_FRAME\_INTERRUPT\_EARLYModePhysical Channelnot supportedLow Latency，recommended use OT\_FRAME\_INTERRUPT\_EARLY\_ENDModeenableLow Latency，so as to save memory ，reduce  through/channel-channellatency 。
-   Hi3403V100 VPSSPhysical ChannelenableLow Latency，again/then enablefixed corner  degreerotation /MosaicEx/CoverEx/OverlayEx/Corner\_rectEx/Line，will adaptivelycloseVPSSPhysical ChannelLow Latency。
-   SS626V100 VPSSPhysical ChannelenableLow Latency，again/then enablefisheye/rotation /MosaicEx/CoverEx/OverlayEx/Line，will adaptivelycloseVPSSPhysical ChannelLow Latency。
-   Hi3403V100 VPSSchannel1OutputOT\_VIDEO\_FORMAT\_TILE\_16x8 whennot supportedenableLow Latency。
...
Gets VPSSchannelLow Latencyparameter。
...
Sets VPSSchannelOutputYUVdata 's line stridealignment。
...
-   when alignless than VPSShardware requirements 's stridealignment when, system allocate bufferand calculate stridealso/still is so as to hardware requirements stridealignmentis/for  accurate/standard。
-   when alignnot equal to VPSShardware requirements 's stridealignmentmultiple  when, system allocate bufferand calculate strideto on/above alignmentto hardware requirements stridealignment's multiple 。 example: alignSets is/for 48，logical requirements 32alignment when, system allocate bufferand calculate stride whenuse 64alignment。
...
Gets VPSSchannelOutputYUVdata 's line stridealignment。
...
SetsVPSSExtension Channelattributes。
...
-   The Group must be created first.
-   Only Hi3403V100 supports this interface.
-   specific Membersvalue rangeand functionlimit reference [ot\_vpss\_ext\_chn\_attr](#ZH-CN_TOPIC_0000002441659501)structure  body/structDescription。
-   Extension Channel's imagesource attributeSets is/for OT\_EXT\_CHN\_SRC\_TYPE\_HEAD when, is bind's Physical Channelnot supported doCoverex/Mosaicex/Line/Overlayex/Corner\_rectexprocessing , i.e., make/enable Sets Coverex/Mosaicex/Line/Overlayex/Corner\_rectexalso will not  generateeffect.
...
GetsVPSSExtension Channelattributes。
...
启 useVPSSquicksendMode， provide/improvebefore sendVO。
...
-   onlySS528V100/SS625V100/SS524V100/SS522V101/SS626V100supportsthis interface。
-   The Group must be created first.
-   must be configured in channelhas not been enable whenSets 。
-   mustfirst call SYSinterfacess\_mpi\_sys\_set\_schedule\_mode（specific refer to “System Control”chapter ）and/also Sets parameteris/for OT\_SCHEDULE\_QUICK（specific refer to “System Control”chapter ），VPSSquicksendModebefore will  generateeffect.
-   VPSSchannelbindVO when, VPSSquicksendModebefore will  generateeffect.
-   SS528V100/SS625V100/SS524V100/SS522V101not recommended doAspect Ratio、Flip、exceeds 15 times's downscale and channelafter processing （coverex/mosaicex/line/overlayex）function，otherwisemay may exist VOscreen flickering风险。
-   SS626V100 not recommended doAspect Ratio、Flip、exceeds 15 times's downscale and channelafter processing （fisheye/coverex/mosaicex/line/overlayex）function，otherwisemay may exist VOscreen flickering风险。
-   not recommended doGets channelbrightnessand function，otherwiseGets 's brightnessand may not  accurate/standard confirm/ensure。
-   SS528V100/SS625V100/SS524V100/SS522V101not supportedVPSS's channelrotation function。
-   SS626V100 not recommendedenablechannelrotation function，rotation  does not take effect.
-   SS626V100 enablequicksendMode，channelLow Latencyfunction does not take effect.
-   SS528V100/SS625V100/SS524V100/SS522V101 VPSSafter 16Group（[OT\_VPSS\_VGS\_GRP\_NO](#ZH-CN_TOPIC_0000002441659549)\~[OT\_VPSS\_MAX\_GRP\_NUM](#ZH-CN_TOPIC_0000002441699353)  - 1）not supportedquicksendMode，Sets  does not take effect.
-   SS528V100/SS625V100/SS524V100/SS522V101/SS626V100 recommendedInputimagewidth  degreedoes not exceed 3840，otherwisemay Sets  does not take effect.
-   specific use method and scenario constraint refer to 《MPP media processing software  V5.0 FAQ》。
...
not enableVPSSchannelquicksendfunction。
...
-   Only SS528V100/SS625V100/SS524V100/SS522V101/SS626V100 support this interface.
-   The Group must be created first.
-   Must be set while the channel is not enabled.
...
Sets VPSSchannel on the  scale enlargecoefficient Type。
...
-   The Group must be created first.
-   the/this interface fitused for VPSSallchannel，including Physical Channelwith/and Extension Channel。
-   supportsdynamic Sets 。
...
Gets VPSSchannel on the  scale enlargecoefficient Type。
...
Sets VPSSchannel on the static configure。
...
-   The Group must be created first.
-   mustin/at ss\_mpi\_vpss\_enable\_chninterface before calling.
-   specific Membersvalue rangeand functionlimit reference ot\_vpss\_chn\_cfgstructure  body/structDescription。
-   Only SS626V100 supports this interface.
...
Gets VPSSchannel on the static configure。
...
-   [OT\_VPSS\_MAX\_ZOOMOUT](#ZH-CN_TOPIC_0000002408100326):DefinitionVPSSPhysical channelMaximum scalesmalltimesnumber.
...
OnlyHi3403V100SupportedExtension channel.
...
DefinitionVPSSPhysical channelMaximum scalesmalltimesnumber.
...
-   SS528V100/SS625V100/SS524V100/SS522V101InputWidthGreater Than2688Andcompression outputwhen, Physical channelMaximum scalesmalltimesnumberOT\_VPSS\_MAX\_ZOOMOUT as 10.
-   SS528V100/SS625V100/SS524V100/SS522V101/SS626V100 scalesmalltimesnumberExceedsOT\_VPSS\_MAX\_ZOOMOUTwhen, ThenwillCallsVGSPerformsagain scalesmall, VGSMaximumagain scalesmall2times, , i.e., Maximum scalesmalltimesnumber as OT\_VPSS\_MAX\_ZOOMOUT\*2.
-   SS528V100/SS625V100/SS524V100/SS522V101/SS626V100CallsVGSPerformsagain scalesmallwhen, Not supportedAspectCompare.
...
-   Hi3403V100Finally28GroupSubmitgiveVGSHardwareProcessing.
...
-   Ifcrop\_mode as OT\_COORD\_RATIO(toCoordinatemode), Usescrop\_rectMemberwhenShoulddoConverts, Calculation Formula as :
    -   x = StartPointCoordinatexRawImageWidth/1000, combinevalid range:\[0, 999\], CalculatesCompletesafterwillgetOperationandAlignedOperation.Formulasame samplesuitableUsed for纵CoordinateCalculates.
    -   width=RegionWidthxActualImageWidth/1000, RegionWidthvalid range:\[1, 1000\].CalculatesCompletesafterwillgetOperationandAlignedOperation.Formulasame samplesuitableUsed forRegionHeightCalculates.

-   tomodecropwhen, cropRegionCoordinatevalid range is \[0, 999\], cropRegionWidthHeightvalid range is \[1, 1000\].
-   Ifcrop\_mode as OT\_COORD\_ABS(absolutetoCoordinatemode):
...
-   Hi3403V100 VPSSEnable3DNRwhen, cropWidthHeightAutomatictowardunder4PixelAligned.
...
-   Hi3403V100only supportsOT\_VPSS\_NR\_TYPE\_VIDEO\_NORM, OT\_VPSS\_NR\_TYPE\_SNAP\_NORMandOT\_VPSS\_NR\_TYPE\_VIDEO\_SPATIAL.
...
-   Hi3403V100 3DNRSet as Videomodewhen, IfEnablewhendomaindenoising, May Cause endto endlatencyIncreasesoneFrameTime(For examplesensorTiming as Each秒30Frame,  endto endlatencywillIncreases33ms), CanViaEnablenr\_low\_delay\_enModule parameterDivideThislatency.nr\_low\_delay\_enClosewhen, Close3DNREnableOrCloseReferenceFrameEnable, willDroponeFrameImage.
...
SS528V100/SS625V100/SS524V100/SS522V101/Hi3403V100only supportsOT\_VPSS\_NR\_MOTION\_MODE\_NORMType.
...
-   max\_width, max\_heightUsed forAllocate3DNRReferenceFrame, heavy构Framebuffer, AllocateUsesMemorySizeIsPress16AlignedAfterWidthHeighttoCalculates.
    ...
    -   RegardingHi3403V100, WhenVPSSBlockProcessingAndEnable3DNRwhen, InputImageWidthcannot be greater thanGroupAttributeMaximumWidth, Enable3DNRwhen, bufferSizeWhetherEnoughByLinewhenverify as standard.VI\_ONLINE\_VPSS\_ONLINEmode, GroupAttributeMaximumWidthHeightMustAndVPSSActualInputImageResolutionConsistent.
...
-   RegardingSS528V100/SS625V100/SS524V100/SS522V101/Hi3403V100As followsFactorMay CauseCurrentProcessingImage3DNRFunctionTemporarilyClose:
...
-   WhenConfigure nr\_enEnablewhen, SS528V100/SS625V100/SS524V100/SS522V101 InputcompressionImageRequirementWidth4PixelAligned, Hi3403V100InputcompressionImageRequirementWidthHeight4PixelAligned.
...
-   InputOutputMultiplexbufferFunctionHasThe followingConstraintOrNotes:
    ...
    -   RegardingSS528V100/SS625V100/SS524V100/SS522V101, ChannelFormat, compressionoccurchange convert, Zoomwhen, IfNeedRequestVBoccurchange convert, ThenSwitcheswillHas几FrameNonetogetheruseCausesloseFrame.
    ...
    -   Hi3403V100OnlyOffline ModePhysical channelSupportedMultiplexbuffer.
    -   Hi3403V100 OnlyInputoutput resolution, compression mode, Pixel format, Video FormatConsistentOnly WhenSupportedMultiplexbuffer.
    -   Hi3403V100Enablemcf\_en, Groupcrop, BlockProcessing, SimultaneouslyEnableInputOutputLow latency, Low latencysinglebuffer, flipNot supportedMultiplexbuffer.
...
-   Hi3403V100Not supportedIE, DEI, DCIFunction, RelatedParameterinvalid.
-   Hi3403V100OnlyOffline ModeSupportedmcf\_enSet as TD\_TRUE.
-   Hi3403V100 mcf\_enEnableafter, only supportsThe followingInterface:
...
-   The target frame rate cannot be greater than the source frame rate.
-   SS528V100/SS625V100/SS524V100/SS522V101/SS626V100ChannelCompact SegmentcompressionwriteexitAndReceive endnotIsEncodingwhen, Not supportedmirror/flip.
...
-   Supports U/V swap by setting the channel output pixel format.
...
-   Hi3403V100Channel as Compact SegmentcompressionwriteexitwhenNot supportedCoverEx, OverlayEx, MosaicEx, LineandCorner\_rectEx.
-   AUTOmodeSuch asquerytoNot supportedcompressionwhenwillstrongmakechange as No CompressionOutput.
-   chn\_mode as AUTOmodewhen, width, height, aspect\_ratio, compress\_mode, pixel\_format, frame\_rate, video\_formatSetnottakes effect, should setbyBackendModule confirm/ensure, depthnottakes effect, ImageWill notenter入ChannelqueueColumn.
-   Hi3403V100Not supportedborderandAspectCompare.
...
-   Hi3403V100 VPSSOnline modeDivideexternalOthermode:
    -   OutputlinearFormatwhenMaximumSupported30Times Reduction, MaximumSupported16timesZoom, BlockScenarioChannel0Enablecompressionwhen, MaximumSupported scalesmall28times; 
    -   OutputOT\_VIDEO\_FORMAT\_TILE\_16x8FormatwhenMaximumSupported3Times Reduction2timesZoom.

-   Hi3403V100 VPSSWorkOT\_VI\_OFFLINE\_VPSS\_ONLINEmodeBlockProcessingwhen, Channel0Not supportedHorizontalScaling, Not supportedmirrorandcompressionSimultaneouslyEnable.
-   SS528V100/SS625V100/SS524V100/SS522V101/SS626V100 VPSSChannelVideo Format as invalidParameter.
-   Hi3403V100 VPSSChannelVideo FormatSupportedOT\_VIDEO\_FORMAT\_LINEARandOT\_VIDEO\_FORMAT\_TILE\_16x8, GroupNo.\[[OT\_VPSS\_VGS\_GRP\_NO](#ZH-CN_TOPIC_0000002441659549),  [OT\_VPSS\_MAX\_GRP\_NUM](#ZH-CN_TOPIC_0000002441699353)  - 1\]rangeNot supportedOutputOT\_VIDEO\_FORMAT\_TILE\_16x8.BlockProcessingwhenNot supportedOutputOT\_VIDEO\_FORMAT\_TILE\_16x8.
-   Hi3403V100 VPSSChannel1EnableLow latencyNot supportedOutputOT\_VIDEO\_FORMAT\_TILE\_16x8.
-   Hi3403V100 VPSSChannel1OutputOT\_VIDEO\_FORMAT\_TILE\_16x8when, Pixel formatCan OnlySet as OT\_PIXEL\_FORMAT\_YVU\_SEMIPLANAR\_420.
-   Hi3403V100 VPSSChannel1OutputOT\_VIDEO\_FORMAT\_TILE\_16x8when, Not supportedCoverEx, OverlayEx, MosaicEx, Line, Corner\_rectExAndGetChannelLuminanceand.
-   VPSSWorkOT\_VI\_ONLINE\_VPSS\_ONLINEmodewhen, Not supportedZoom.
-   Hi3403V100 VPSSWorkOT\_FRAME\_INTERRUPT\_EARLYmodewhen, Channel0compressionandflipcannot simultaneouslyEnable.
-   Channel AUTO mode and channel crop are mutually exclusive.
...
-   ChannelSet as AUTOmodewhen, Can OnlyAndoneReceive Bind.
...
<td class="cellrowborder" valign="top" width="62%" headers="mcps1.1.3.1.2 "><p id="p2721mcpsimp"><a name="p2721mcpsimp"></a><a name="p2721mcpsimp"></a>Used forHi3403V100NRStandardParameter.</p>
</td>
</tr>
</tbody>
</table>
...
-   denoisingRelatedfourParameterValuelarge, ThenPicturesecuritystatic, tf\_strength  toFlatRegiondenoisingEffectNon- clear/brightdisplay.RecommendedIfexitComparerelativelystrongMotionSmearingandPicture残影when, NeedwillwhendomainRelatedParameterreduce\(tf\_strength  And  ctf\_strength\).
-   sf\_strength (afterAbbreviation SFS)DirectlyAffectPictureMotionRegionnoisemeter tf\_strength(afterAbbreviation TFS)DirectlyAffectPictureStaticRegionnoisemeter, For examplelargechipFlatRegionnoisemeter.
    -   SFS IntensityHeightMay CauseOverallPictureEdgeBlur, finesavelose lost. TFS IntensityHeightMay CauseMotionWhenSmearing clear/brightdisplay.DebugwhenRecommendedStrategyIsPrioritypromptHeight SFS, IfPicturefinesavelose lost严heavyStaticRegionnoiseStillNoneValidDivide, ThenkeepClearBasiconsuitableWhenEnhance TFS Effect.ByachievetoSmearingandPicturesecuritystaticbalance as Optimal.
    -   csf\_strength(AbbreviationCSFS)Andctf\_strength(AbbreviationCTFS)twoParameterUsed toAdjustcolornoisecancapability.DebugStrategy as PriorityAdjustCSFSIntensity, IfCSFSNoneSuppressioncolornoiseThenAdjustCTFS, **ByPictureFlatRegioncolornoiserelativelysecuritystaticAndNone clear/brightdisplayColorSmearingResidual as Optimal**.

-   cvbs\_strength toFunctionComparerelativelyIndependent.cvbs\_strength useByProcessingmarkclearInputwhenGeneratebrightcolor串 扰Phenomenon.ProcessingIntensity as SetHeightThenbrightCrosstalkProcessingcancapabilitystrong.ButHeightclearInputwhenwillPicturestrongEdge placeCauseComparerelatively严heavycolorcolorResidual.ThereforeHeightclearInputwhenIfAdjust CTFS  as MinimumvaluewhenstillhoweverwillHas clear/brightdisplayResidual, Thenplease AdjustTheParameter.
...
DefinitionHi3403V1003DNRManualmodeParameter.
...
<td class="cellrowborder" valign="top" width="79%" headers="mcps1.1.3.1.2 "><p xml:lang="sv-SE" id="p7250mcpsimp"><a name="p7250mcpsimp"></a><a name="p7250mcpsimp"></a><span xml:lang="en-US" id="ph7251mcpsimp"><a name="ph7251mcpsimp"></a><a name="ph7251mcpsimp"></a>3DNRInterfaceStandardParameter.</span>ot_vpss_nrx_v2Definition见"<span xml:lang="en-US" id="ph7252mcpsimp"><a name="ph7252mcpsimp"></a><a name="ph7252mcpsimp"></a>Hi3403V100/Hi3519AV200</span> 3DNRParameterconfigured instructions".</p>
</td>
</tr>
</tbody>
</table>
...
This structure is used by Hi3403V100 only.
...
DefinitionHi3403V1003DNRAutomaticmodeParameter.
...
<td class="cellrowborder" valign="top" width="79%" headers="mcps1.1.3.1.2 "><p xml:lang="sv-SE" id="p563mcpsimp"><a name="p563mcpsimp"></a><a name="p563mcpsimp"></a><span xml:lang="en-US" id="ph564mcpsimp"><a name="ph564mcpsimp"></a><a name="ph564mcpsimp"></a>NRAutomaticmodeStandardParameternumberGroupPointer.</span>ot_vpss_nrx_v2Definition见"<span xml:lang="en-US" id="ph565mcpsimp"><a name="ph565mcpsimp"></a><a name="ph565mcpsimp"></a>Hi3403V100/Hi3519AV200</span> 3DNRParameterconfigured instructions".</p>
...
-   This structure is used by Hi3403V100 only.
-   isoandnrx\_param point to/indicatetowardnumberGroupMemberCountallMustAndparam\_numConsistent.
-   isoAndnrx\_paramExistsoneoneCorrespondingRelationship.param\_num, ISOvalueAndnrx\_paramParameter as Dynamic, UserCanBased onNeed requireself-byConfigure , where MaximumGroupcombineNumbernotGreater Than16, ISOvalueMustFromsmalltolargeConfigure , Range as \[100,  3276800\].
...
-   ServiceLine, IfVPSSGetISOvalue placeAtaonedistinguishinside, ActualConfigure NRParameter as ThedistinguishleftvalueCorrespondingStandardParameterAndrightvalueCorrespondingStandardParameterPerformsDifference算.
...
DefinitionHi3403V1003DNRStandardParameter.
...
-   CurrentstrongdenoisingChannelResultMainly used forEncodingOutput.CanMakeEncodingBitratetoValidControl.weakdenoisingChannelResultMainly used forPreviewPictureOutput, ConsiderPictureOverallImageQualityPriority.ThereforestrongdenoisingChannelValueGenerallySetwillCompareweakdenoisingChannelValuelarge.two Setetc.whenPictureImageQualitymeterWhen.
-   mdz useByDeterminesPictureWhether as Motionjudge determine.Algorithmwill认HeightAtTheThresholdjudge determineAll as MotionRegion, MainlyAdoptingemptydomainFilteringResult, lowAtTheThresholdjudge determine as StaticRegion, MainlyAdoptingwhendomainFilteringResult.ThereforeIf confirm/ensureBeforeIntensityValuebeforepromptunder, PictureSmearingnotheavyButStaticRegionnoiseStillCannotMake人full意, CanConsidersuitableWhenpromptHeightTheParameter ByNone clear/brightdisplaySmearing as Optimal.
-   edz useByDeterminesPictureWhether as strongEdge.HeightAtTheThresholdjudge determine as EdgeRegion, At this pointTheRegionemptydomaindenoisingcancapabilitywillHaspromptHeight, PicturewillchangeBlur.ThevalueIfSetsmallwillstrongEdgeRegionexitrelatively clear/brightdisplay锯齿, AndwhendomaindenoisingCauseSmearingalsowillIncreases.ThereforeRecommendedset value as 63, NoneNeeddotoo largeChange.
...
<td class="cellrowborder" valign="top" width="61%" headers="mcps1.1.3.1.2 "><p id="p2100mcpsimp"><a name="p2100mcpsimp"></a><a name="p2100mcpsimp"></a>Hi3403V100NRStandardParameter.</p>
</td>
</tr>
</tbody>
</table>
...
-   contrast and ie_strength are invalid for Hi3403V100 and no exception check is performed.
...
-   SS528V100/SS625V100/SS524V100/SS522V101BlocksavePointCountCanPressAs followsEstimation:
    ...
    -   compression outputwhen:
        ...
        NeedsavePointCountbyInputOutputWidthsame sharedDetermines, CanPressAs followsCalculatesEstimationMaximumNeedsavePointnumber\(min,max\).

        split\_node\_num \[min\] = input\_width\_align / 2580.

        split\_node\_num \[max\] = \(2 \* input\_width\_align / 2580\) – 1.

        -   Such asInputWidth as 3840when, split\_node\_nunNeedCountRange\[2, 2\], SpecificvaluebyOutputWidthchangeChanges Withchange convert.
        -   Such asInputWidth as 5120when, split\_node\_numNeedCountRange\[2, 3\], SpecificvaluebyOutputWidthchangeChanges Withchange convert.

-   Hi3403V100BlocksavePointCountCanPressAs followsFormulaEstimation:split\_node\_num = 2 \* input\_width / 3712 + 1.
...
-   BlockCountinsufficientwhen, please Via[ss\_mpi\_vpss\_set\_mod\_param](#ZH-CN_TOPIC_0000002408260114)InterfaceSetcombinearrangeBlocksavePointCount.
-   nr\_low\_delay\_enOnlytoHi3403V100Valid, OtherSolutionnotdoAbnormalChecks.

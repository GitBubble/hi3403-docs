...
>-   Hi3403V100 OT\_VI\_ONLINE\_VPSS\_ONLINEandOT\_VI\_OFFLINE\_VPSS\_ONLINEmode, ActualInputResolutionLimitGet决AtFrontendVIModule.
...
-   Not supported on Hi3403V100/SS626V100.此接口。
...
-   只对USERMode的通道有效，AUTOMode的通道不受影响。
...
-   UserVB大小根据VPSS通道Output图像计算，具体计算公式参考ot\_buffer.h。
-   绑定后，通道申请的VB均是从此VB池中获取，解绑后从公共VB池中获取。
-   buffer share和attach vb pool功能同时开启时，buffer share优先级更高，通道使用的VB为InputVB。
...
-   开启LDCI功能后，如果实际Input宽度超过7680，VPSS会自适应开关LDCI功能。
...
-   VPSS组号必须为0。
-   仅VI\_ONLINE\_VPSS\_ONLINEMode下有效。
-   该接口需要在设置ss\_mpi\_sys\_init之后调用。
-   修改中断Mode时必须先关闭所有Physical Channel。
...
-   该接口需要在设置ss\_mpi\_sys\_init之后调用。
-   Only Hi3403V100 supports this interface.
-   默认为OT\_FRAME\_INTERRUPT\_STARTMode，延时行号为0；销毁组后会将帧中断中断属性重置为默认状态。
...
-   配置鱼眼属性前必须先配置对应Physical Channel的属性。
-   Physical Channel开启了鱼眼矫正后，不支持动态改变通道Crop属性、通道旋转属性以及通道宽高属性。
-   如果Physical Channel非紧凑段压缩和鱼眼功能同时开启，会自适应关闭VPSS硬件压缩，鱼眼矫正后压缩Output。
-   如果Physical Channel非紧凑段压缩和鱼眼功能同时开启，只有当校正区域数为1，且该区域可完全覆盖Output范围时，可不打背景色，其他情况必须打上背景色，即bg\_color\_en参数必须设置为TD\_TRUE。
-   开启鱼眼功能时，Physical Channel像素格式不能设置为semi-planar422，压缩Mode仅支持非压和非紧凑段压缩。
-   开启鱼眼功能时，仅支持OT\_VIDEO\_FORMAT\_LINEAR格式Output。
-   Hi3403V100通道OutputVIDEO\_FORMAT\_TILE\_16x8格式时，不支持鱼眼。
...
-   通道AUTOMode下不支持鱼眼。
-   鱼眼矫正有最小的InputOutput宽高限制：OT\_FISHEYE\_MIN\_IN\_IMAGE\_HEIGHT等请参考“几何畸变矫正子系统”章节。
...
-   Only Hi3403V100 supports this interface.
-   The channel number must be a VPSS physical channel.
-   配置LDC属性前必须先配置对应Physical Channel的属性。
-   如果Physical Channel非紧凑段压缩和LDC功能同时开启，会自适应关闭VPSS硬件压缩，LDC处理后压缩Output。
-   开启LDC功能时，Physical Channel像素格式不能设置为semi-planar422，压缩Mode仅支持非压和非紧凑段压缩。
-   通道OutputVIDEO\_FORMAT\_TILE\_16x8格式时，不支持LDC。
-   启用LDC时，VPSS的COVER可能会变形，此时建议使用COVEREX来代替COVER。
-   当开启展宽时，OT\_LDC\_V1的distortion\_ratio取值范围只能为 \[0, 500\]，即只支持LDC的桶形Mode。
-   由于LDC会对图像进行几何拉伸或几何压缩，当Input图像存在明显噪声，在桶型矫正过后，可能存在图像四周噪声颗粒变大（四周被拉伸）的现象，可将LDC配置于之后改善此现象。噪声较大的场景建议在VPSS模块配置LDC，不建议在VI模块配置LDC。
...
-   Only Hi3403V100 supports this interface.
-   The channel number must be a VPSS physical channel.
-   配置展宽属性前必须先配置对应Physical Channel的属性。
-   如果Physical Channel非紧凑段压缩和展宽功能同时开启，会自适应关闭VPSS硬件压缩，展宽处理后压缩Output。
-   开启展宽功能时，Physical Channel像素格式不能设置为semi-planar422，压缩Mode仅支持非压和非紧凑段压缩。
-   通道OutputVIDEO\_FORMAT\_TILE\_16x8格式时，不支持展宽。
-   当开启展宽时，OT\_LDC\_V1的distortion\_ratio取值范围只能为  \[0, 500\]，即只支持LDC的桶形Mode。
...
-   如果裁剪区域超出图像范围，丢弃超出部分图像，优先保证裁剪Coordinates与所设置的参数相同。
-   分块处理时，通道0不支持裁剪。
-   设置通道裁剪前必须先设置通道属性。
-   通道AUTOMode与通道裁剪功能互斥。
...
-   SS626V100 VPSS通道USERMode时，后端绑定模块如果不是VPSS/VO，则自适应关闭通道Low Latency；VPSS通道AUTOMode时，后端绑定模块如果不是VO，或者绑定了VO，但VO不是SingleMode，则自适应关闭通道Low Latency。
-   Low Latency行号不能小于16，不能大于图像高度；如果是Physical Channel且开启了通道裁剪，则不能大于裁剪高度。
-   Hi3403V100 OT\_FRAME\_INTERRUPT\_EARLYModePhysical Channel不支持Low Latency，推荐使用OT\_FRAME\_INTERRUPT\_EARLY\_ENDMode开启Low Latency，以节省内存，降低通路延时。
-   Hi3403V100 VPSSPhysical Channel开启Low Latency，再开启固定角度旋转/MosaicEx/CoverEx/OverlayEx/Corner\_rectEx/Line，会自适应关闭VPSSPhysical ChannelLow Latency。
...
-   Hi3403V100 VPSS通道1OutputOT\_VIDEO\_FORMAT\_TILE\_16x8时不支持开启Low Latency。
...
-   Extension Channel的图像来源属性设置为OT\_EXT\_CHN\_SRC\_TYPE\_HEAD时，被绑定的Physical Channel不支持做Coverex/Mosaicex/Line/Overlayex/Corner\_rectex处理，即使设置Coverex/Mosaicex/Line/Overlayex/Corner\_rectex也不会生效。
...
-   The Group must be created first.
-   需在通道未使能时设置。
-   必须先调用SYS接口ss\_mpi\_sys\_set\_schedule\_mode（具体请参考“System Control”章节）并设置参数为OT\_SCHEDULE\_QUICK（具体请参考“System Control”章节），VPSS快速发送Mode才会生效。
-   VPSS通道绑定VO时，VPSS快速发送Mode才会生效。
...
-   SS626V100 开启快速发送Mode，通道Low Latency功能不生效。
-   SS528V100/SS625V100/SS524V100/SS522V101 VPSS后16组（[OT\_VPSS\_VGS\_GRP\_NO](#ZH-CN_TOPIC_0000002441659549)\~[OT\_VPSS\_MAX\_GRP\_NUM](#ZH-CN_TOPIC_0000002441699353)  - 1）不支持快速发送Mode，设置不生效。
-   SS528V100/SS625V100/SS524V100/SS522V101/SS626V100 建议Input图像宽度不超过3840，否则可能设置不生效。
...
-   The Group must be created first.
-   该接口适用于VPSS所有通道，包括Physical Channel与Extension Channel。
-   支持动态设置。

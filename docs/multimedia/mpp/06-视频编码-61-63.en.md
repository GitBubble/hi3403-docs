...
-   `src_frame_rate` should be set to the actual frame rate at which TimeRef is generated. The RC uses `src_frame_rate` to measure the actual frame rate and perform bitrate control. For details, see ot\_venc\_h264\_cbrPrecautions。
...
-   When the mosaic input mode is OT_VENC_MOSAIC_MODE_RECT, the user provides the mosaic region and the SDK internally applies rectangular mosaic overlay. Mosaic uses CPU overlay onto the image with internal NEON assembly optimization. Mosaic block size supports only OT_MOSAIC_BLK_SIZE_32 and OT_MOSAIC_BLK_SIZE_64. A maximum of 200 mosaic regions are supported; regions can overlap. Each mosaic region's start point (x, y) and end point (x1=x+w, y1=y+h) must not exceed the image boundary. Before applying mosaic, the start point (x, y) is aligned left/upward and the end point (x1, y1) is aligned right/downward to 32×32 or 64×64 boundaries.
...
-   高级参数主要由五个参数组成
    ...
    -   `mcu_per_ecs`: the number of MCUs contained in each ECS. When `mcu_per_ecs = 0` (system mode), all MCUs in the current frame are encoded as a single ECS. The minimum value is 0; the maximum value must not exceed min{(picwidth+15)>>4 × (picheight+15)>>4 × 2, 65535}. Setting `mcu_per_ecs` too small can easily cause excessively long CPU occupancy and similar issues; users must configure it appropriately based on actual requirements.
...
-   高级参数主要由四个参数组成
    ...
    -   `mcu_per_ecs`: the number of MCUs contained in each ECS. When `mcu_per_ecs = 0` (system mode), all MCUs in the current frame are encoded as a single ECS. The minimum value is 0; the maximum must not exceed min{(picwidth+15)>>4 × (picheight+15)>>4 × 2, 65535}. Setting `mcu_per_ecs` too small can easily cause excessively long CPU occupancy and similar issues; users must configure it appropriately based on actual requirements.
    -   ecs\_output\_en: onlyHi3403V100/SS626V100supports，not supported's default  valueis/for TD\_FALSE。
...
-   rate controller's advanced parametersby/from below parameterGroup become: 
    ...
    -   threshold\_i\[OT\_VENC\_TEXTURE\_THRESHOLD\_SIZE\], threshold\_p\[OT\_VENC\_TEXTURE\_THRESHOLD\_SIZE\], threshold\_b\[OT\_VENC\_TEXTURE\_THRESHOLD\_SIZE\]: A set of threshold values measuring the macroblock texture complexity for I-frames, P-frames, and B-frames respectively. The thresholds in this group are arranged in ascending order; each threshold value ranges from \[0, 255\]. This threshold group is used during macroblock-level bitrate control to fine-tune the Qp of each macroblock based on image texture complexity. For H.264/H.265, macroblock-level bitrate control supports both QP increase (max +16) and QP decrease (max -16). The direction of increase/decrease depends on the threshold index j: if the current macroblock complexity ≤ threshold\[j-1\], QP is reduced by x; if > threshold\[j-1\], QP is increased by y. Where C = complexity, x and y follow: C < threshold\[0\]: x=j; threshold\[j-1\]≤C≤threshold\[j\]: x=y=0; threshold\[j\]<C≤threshold\[j+1\]: y=1; and so on.
    ...
    -   `motion_sensitivity`: motion sensitivity. A larger value means the bitrate control responds to motion changes more quickly, but it is also more sensitive to noise.
    ...
    -   `max_bit_percent` & `min_bit_percent`: QVBR advanced parameters specifying the upper and lower bitrate floating percentages. When the scene is complex and encoding quality drops, the encoder increases the target bitrate within the configured upper percentage to maintain image quality. When the scene is simple and encoding quality improves, the encoder decreases the target bitrate within the configured lower percentage to save bandwidth.
...
-   VENC高级参数主要由六个参数决定。
    ...
    -   `crop_info`: sets the channel's crop attribute. The channel first performs image cropping, then compares the cropped image dimensions with the encoding channel dimensions to determine whether downscaling is needed.omparerelatively ，determine whether perform downscale 。
...
-   VENC高级参数主要由六个参数决定。
    ...
    -   threshold\_p\[OT\_VENC\_TEXTURE\_THRESHOLD\_SIZE\], threshold\_b\[OT\_VENC\_TEXTURE\_THRESHOLD\_SIZE\]: A set of threshold values (abbreviated thr below) measuring macroblock texture complexity for the current scene's P-frames and B-frames. The thresholds are arranged in ascending order; each ranges from \[0, 255\]. Used during macroblock-level bitrate control for the current scene to fine-tune each macroblock's Qp based on image texture complexity. For H.264/H.265, macroblock-level bitrate control supports QP increase (max +16) and decrease (max -16). The direction index j determines the adjustment: if complexity ≤ thr\[j-1\], QP is reduced by x; if > thr\[j-1\], QP is increased by y, following the same formula as threshold_i.
...
-   scenario  temporarilysupportsbelow three  typeMode: 
    ...
    -   OT_VENC_SCENE_1: high-bitrate motion scenarios, such as handheld cameras, action camcorders, aerial photography, etc.
...
**Figure 2**  smart\_p/adv\_smart\_pp Hierarchical QP Diagram<a name="fig16586145618411"></a>  

...
-   [ss\_mpi\_venc\_set\_search\_window](#ZH-CN_TOPIC_0000002408259126)：设置H.264/H.265channel搜索窗范围。
-   [ss\_mpi\_venc\_get\_search\_window](#ZH-CN_TOPIC_0000002408098754)：获取H.264/H.265channel搜索窗范围。
...
-   [ss\_mpi\_venc\_set\_h265\_entropy](#ZH-CN_TOPIC_0000002441698345)：设置H.265channel熵编码属性。
-   [ss\_mpi\_venc\_get\_h265\_entropy](#ZH-CN_TOPIC_0000002441698401)：获取H.265channel熵编码属性。
...
-   [ss\_mpi\_venc\_set\_mod\_param](#ZH-CN_TOPIC_0000002408099010)：设置编码相关的Module Parameter。
-   [ss\_mpi\_venc\_get\_mod\_param](#ZH-CN_TOPIC_0000002441697981)：获取编码相关的Module Parameter。
-   [ss\_mpi\_venc\_set\_sse\_rgn](#ZH-CN_TOPIC_0000002408258734)：设置H.264/H.265channelSSE属性。
-   [ss\_mpi\_venc\_get\_sse\_rgn](#ZH-CN_TOPIC_0000002441658541)：获取H.264/H.265channelSSE属性。
...
-   [ss\_mpi\_venc\_set\_fg\_protect](#ZH-CN_TOPIC_0000002441698337)：设置channel前景保护参数。
-   [ss\_mpi\_venc\_get\_fg\_protect](#ZH-CN_TOPIC_0000002408258646)：获取channel前景保护参数。
...
-   [ss\_mpi\_venc\_attach\_vb\_pool](#ZH-CN_TOPIC_0000002408098818)：将Encoding Channelbind to某个视频缓存VB池中。
-   [ss\_mpi\_venc\_detach\_vb\_pool](#ZH-CN_TOPIC_0000002408259130)：将Encoding Channel从某个视频缓存VB池中解绑定。
...
-   [ss\_mpi\_venc\_set\_rc\_adv\_param](#ZH-CN_TOPIC_0000002408259042)：设置RC模块的Advanced Parameters。
-   [ss\_mpi\_venc\_get\_rc\_adv\_param](#ZH-CN_TOPIC_0000002408099122)：获取RC模块的Advanced Parameters。
...
-   [ss\_mpi\_venc\_get\_param\_set\_id](#ZH-CN_TOPIC_0000002408258822)：获取H.264/H.265Parameter setID。
-   [ss\_mpi\_venc\_set\_param\_set\_id](#ZH-CN_TOPIC_0000002408258486)：设置H.264/H.265Parameter setID。
-   [ss\_mpi\_venc\_get\_h264\_poc](#ZH-CN_TOPIC_0000002408258554)：获取H.264协议Encoding Channel的POC类型。
-   [ss\_mpi\_venc\_set\_h264\_poc](#ZH-CN_TOPIC_0000002408259046)：设置H.264协议Encoding Channel的POC类型。
-   [ss\_mpi\_venc\_get\_jpeg\_dering\_level](#ZH-CN_TOPIC_0000002408099230)：获取JPEGEncoding Channel的强边去Ring效应强度等级。
-   [ss\_mpi\_venc\_set\_jpeg\_dering\_level](#ZH-CN_TOPIC_0000002441697877)：设置JPEGEncoding Channel的强边去Ring效应强度等级。
-   [ss\_mpi\_venc\_enable\_jpeg\_dblk](#ZH-CN_TOPIC_0000002441658381)：是否EnabledJPEGEncoding Channel的Block效应。
...
-   [ss\_mpi\_venc\_get\_jpeg\_roi\_adv\_attr](#ZH-CN_TOPIC_0000002441698113)：获取JPEG和MJPEGEncoding Channel的ROI高级属性。
-   [ss\_mpi\_venc\_set\_jpeg\_roi\_adv\_attr](#ZH-CN_TOPIC_0000002441698285)：设置JPEG和MJPEGEncoding Channel的ROI高级属性。
...
<td class="cellrowborder" valign="top" width="64.36%" headers="mcps1.1.4.1.2 "><p id="p18658mcpsimp"><a name="p18658mcpsimp"></a><a name="p18658mcpsimp"></a>Encoding Channel属性。</p>
</td>
...
-   Encoding Channel属性由三部分组成，编码器属性、码率控制器属性和帧结构类型属性，帧结构类型属性简称GOP类型属性。
-   编码器属性首先需要选择编码协议，然后分别对各种协议corresponding 属性进行赋值。
-   编码器属性最大宽高，通道宽高必须满足as follows约束：
...
-   编码器属性必须设置编码码流buffer大小，获取码流方式, 编码profile等，[表2](#_Ref196299180)详细Description了各种协议的各项属性的特性。
...
-   H.264/H.265，编码帧存由YHeaderSize、CHeaderSize、YSize、CSize、PmeSize、 PmeInfoSize和TmvSize共同构成，编码器默认根据最大宽高计算帧存进行内存分配，在设置通道宽高时需保证根据通道宽高计算的每Part of帧存大小都不能大于最大宽高计算的帧存大小。
...
-   对H.264/H.265编码协议，码率控制均SupportedSeven modes：CBR、VBR、AVBR、QVBR、CVBR、QPMAP和FIXQP。并且对于不同协议，相同RC模式的属性变量基本一致。
-   MJPEG的码率控制Supported三种模式：CBR、VBR和FIXQP。
-   src\_frame\_rate应该set to 产生TimeRef的实际帧率，RC需要根据src\_frame\_rate统计实际的帧率以及进行码率控制。详情请参看ot\_venc\_h264\_cbrNotes。
-   CBR除了上述的属性之外，还需要设置平均比特率。平均比特率的单位是kbps。平均比特率的设置与Encoding Channel宽高以及图像帧率都有关系。
...
-   当前NoneSolutionSupportedH.265 main 10 profile.
...
-   销毁并不存在的通道，returns failure。
-   销毁前必须停止接收图像，否则returns failure。
...
-   如果一个通道没有停止接收图像而reset通道，则returns failure。
...
-   需要接收的图像帧数set to -1时表示不指定帧数。
-   如果通道已经开始接收图像，没有停止接收图像前再一次调用this interface指定接收帧数，返回操作不允许。
-   如果通道指定接收帧数接收图像，没有停止接收图像前允许再一次调用this interface不指定帧数接收图像。
...
停止Encoding Channel接收input image。
...
-   this interface用于Encoding Channel停止接收图像来编码，在Encoding Channel销毁或复位前必须停止接收图像。
-   调用this interface仅停止接收原始数据编码，码流buffer并不会被清除。
...
<td class="cellrowborder" valign="top" width="57.99999999999999%" headers="mcps1.1.4.1.2 "><p id="p7377mcpsimp"><a name="p7377mcpsimp"></a><a name="p7377mcpsimp"></a>Encoding Channel属性。</p>
</td>
...
-   this interface只能Set the encoding channel属性中的Dynamic Attribute，If setStatic Attribute，则returns failure。编码协议、获取码流的方式（按帧还是按包获取码流）、编码图像最大宽高属性属于Static Attribute。另外，各个编码协议的Static Attribute由各个协议模块指定，具体see ot\_venc\_chn\_attr。
-   设置编码器属性中通道宽高属性时，需要首先停止Encoding Channel接收input image。通道宽高设置后channel优先级并不会恢复默认，Encoding Channel的其它有参数Configure 恢复Default value，关闭OSD，并且清空码流buffer和缓存图像队列。
...
<td class="cellrowborder" valign="top" width="62%" headers="mcps1.1.4.1.2 "><p id="p18981mcpsimp"><a name="p18981mcpsimp"></a><a name="p18981mcpsimp"></a>Encoding Channel属性。</p>
</td>
...
<td class="cellrowborder" valign="top" width="56.99999999999999%" headers="mcps1.1.4.1.2 "><p id="p10935mcpsimp"><a name="p10935mcpsimp"></a><a name="p10935mcpsimp"></a>获取码流Timeout。</p>
<p class="msonormal" id="p10936mcpsimp"><a name="p10936mcpsimp"></a><a name="p10936mcpsimp"></a>Valid range: [-1, +∞)</p>
<a name="ul10937mcpsimp"></a><a name="ul10937mcpsimp"></a><ul id="ul10937mcpsimp"><li xml:lang="it-IT">-1：<span xml:lang="en-US" id="ph10939mcpsimp"><a name="ph10939mcpsimp"></a><a name="ph10939mcpsimp"></a>阻塞。</span></li><li><span xml:lang="it-IT" id="ph10941mcpsimp"><a name="ph10941mcpsimp"></a><a name="ph10941mcpsimp"></a>0：</span>非阻塞。</li><li>大于0：Timeout。</li></ul>
</td>
...
-   码流结构体ot\_venc\_stream包含以下部分：
    ...
    -   码流特征信息，数据类型 as 联合体，包含了不同编码协议corresponding 码流特征信息h264\_info/ jpeg\_info/h265\_info，码流特征信息的Output用于Supported用户的上层应用。
    -   码流高级特征信息，数据类型 as 联合体，包含了不同编码协议corresponding 码流高级特征信息h264\_adv\_info/h265\_adv\_info，码流高级特征信息的Output用于Supported用户的上层应用。
...
-   插入用户数据，只SupportedH.264/H.265和MJPEG/JPEG编码协议。
...
<a name="ul7646mcpsimp"></a><a name="ul7646mcpsimp"></a><ul id="ul7646mcpsimp"><li xml:lang="it-IT">-1：<span xml:lang="en-US" id="ph7648mcpsimp"><a name="ph7648mcpsimp"></a><a name="ph7648mcpsimp"></a>阻塞。</span></li><li><span xml:lang="it-IT" id="ph7650mcpsimp"><a name="ph7650mcpsimp"></a><a name="ph7650mcpsimp"></a>0：</span>非阻塞。</li><li>&gt; 0：Timeout。</li></ul>
</td>
...
<a name="ul8623mcpsimp"></a><a name="ul8623mcpsimp"></a><ul id="ul8623mcpsimp"><li xml:lang="it-IT">-1：<span xml:lang="en-US" id="ph8625mcpsimp"><a name="ph8625mcpsimp"></a><a name="ph8625mcpsimp"></a>阻塞。</span></li><li><span xml:lang="it-IT" id="ph8627mcpsimp"><a name="ph8627mcpsimp"></a><a name="ph8627mcpsimp"></a>0：</span>非阻塞。</li><li>&gt;0：Timeout。</li></ul>
</td>
...
<p id="p971mcpsimp"><a name="p971mcpsimp"></a><a name="p971mcpsimp"></a>大于0：Timeout。</p>
</td>
...
-   VI帧请求，只SupportedH.264/H.265编码协议。
...
-   IDR帧请求，只SupportedH.264/H.265编码协议。
...
-   I/P/B帧的熵编码方式可以分别设置。
-   Baseline profileNot supportedcabac编码方式，Not supportedB帧，Supportedcavlc编码方式，main profile、high profile和svc-tSupportedcabac编码方式和cavlc编码方式。
...
将Encoding Channelbind to某个视频缓存VB池中。
...
将Encoding Channel从某个视频缓存VB池中解绑定。
...
获取H.264协议Encoding Channel的POC类型。
...
设置H.264协议Encoding Channel的POC类型。
...
获取JPEGEncoding Channel的强边去Ring效应强度等级。
...
设置JPEGEncoding Channel的强边去Ring效应强度等级。
...
是否EnabledJPEGEncoding Channel的Block效应。
...
获取JPEG和MJPEGEncoding Channel的ROI高级属性。
...
设置JPEG和MJPEGEncoding Channel的ROI高级属性。

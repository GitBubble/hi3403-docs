...
<td class="cellrowborder" valign="top" width="38%" headers="mcps1.2.4.1.2 "><p id="p7460mcpsimp"><a name="p7460mcpsimp"></a><a name="p7460mcpsimp"></a>任意不同于gop_mode before change</p>
</td>
...
设置channelslice分割属性，Only supported for H.264/H.265 channels.
...
-   This interface is used to obtain H.264/H.265channelslice分割属性。This API must be called after the channel is created and before it is destroyed.
-   This API is only supported for H.264/H.265 channels.
...
设置channel搜索窗范围，Only supported for H.264/H.265 channels.
...
获取channel搜索窗范围，Only supported for H.264/H.265 channels.
...
-   Only supported for H.265/H.264 encoding channels.H.264Encoding Channel推荐使用High profile。
...
获取cu/mb Skip mode selection preference.
...
Get the JPEG and MJPEG encoding channel ROI attributes，本接口仅适用于SupportedROI功能的Encoding Channel（如H.264/H.265）。
...
-   本接口Only supported for H.264 encoding channels.
...
-   This interface is used to obtain JPEGEncoding Channel的强边Deringing intensity level.
...
-   This interface is used to set JPEGEncoding Channel的强边Deringing intensity level.
-   This interface must be called after the encoding channel is created and before it is destroyed.
-   本接口只对JPEG类型的Encoding Channel有效且只在dering\_mode as 1的情况下才能够调用。

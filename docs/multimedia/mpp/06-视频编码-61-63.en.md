...
-   码率控制器属性首先需要配置RCMode，JPEG抓拍通道不需要配置码率控制器属性，其他协议类型通道（H.264/H.265/MJPEG）都必须配置。码率控制器属性RCMode必须与编码器属性协议类型匹配。
-   对H.264/H.265编码协议，码率控制均支持七种Mode：CBR、VBR、AVBR、QVBR、CVBR、QPMAP和FIXQP。并且对于不同协议，相同RCMode的属性变量基本一致。
-   MJPEG的码率控制支持三种Mode：CBR、VBR和FIXQP。
...
-   H.264/H.265 GOP结构属性首先需要配置Gop ModeMode，（JPEG/MJPEG通道不需要配置GOP结构属性）。
-   Jpege编码时，使能MPF后获取码流不支持单包Mode，调用接口[ss\_mpi\_venc\_set\_mod\_param](#ZH-CN_TOPIC_0000002408099010)设置one\_stream\_buf为多包Mode。
...
-   当前NoneSolution支持H.265 main 10 profile.
...
-   支持超时方式获取。支持select/poll系统调用。
    -   milli\_sec = 0时，则为非阻塞获取，即如果缓冲None数据，则返回FailureOT\_ERR\_VENC\_BUF\_EMPTY。
    -   milli\_sec = -1时，则为阻塞，即如果缓冲None数据，则会等待有数据时才返回获取Success.
    -   milli\_sec \> 0时，则为超时，即如果缓冲None数据，则会等待用户设定的超时时间，若在设定的时间内有数据则返回获取Success，否则返回超时FailureOT\_ERR\_VENC\_BUSY。
...
-   码流结构体ot\_venc\_stream包含以下部分：
    ...
    -   码流包个数pack\_cnt
        ...
        在Input时，此值指定pack中ot\_venc\_pack的个数。按包获取时，pack\_cnt必须不小于1；按帧获取时，pack\_cnt必须不小于当前帧的包个数。在函数调用Success后，pack\_cnt返回实际填充pack的包的个数。

    -   序列号seq

        按帧获取时是帧序列号；按包获取时为包序列号。

    -   码流特征信息，数据类型为联合体，包含了不同编码协议对应的码流特征信息h264\_info/ jpeg\_info/h265\_info，码流特征信息的Output用于支持用户的上层应用。
    -   码流高级特征信息，数据类型为联合体，包含了不同编码协议对应的码流高级特征信息h264\_adv\_info/h265\_adv\_info，码流高级特征信息的Output用于支持用户的上层应用。
...
5.  调用[ss\_mpi\_venc\_release\_stream](#ZH-CN_TOPIC_0000002408258886)Releases the stream buffer.
...
-   当one\_stream\_buf为1时，用户获取一帧码流时只会得到一个码流包（不包含用户数据）的地址，即pack\_cnt为1，地址为pack\[0\]. addr。而在ot\_venc\_pack结构体中增加offset，用来指出一帧码流中有效数据的地址和pack\[0\].addr的偏移。如图1-20所示（以H.264单包为例），pack\[0\]. data\_num = 3,即3种NAL包，分别为SPS、PPS、SEI。pack\[0\]. pack\_info\[0\]. pack\_type. h264\_type = OT\_VENC\_H264\_NALU\_SPS，pack\[0\]. pack\_info \[1\]. pack\_type. h264\_type = OT\_VENC\_H264\_NALU\_PPS，pack\[0\]. pack\_info \[2\]. pack\_type. h264\_type = OT\_VENC\_H264\_NALU\_SEI；其他协议以此类推，data\_num和pack\_info只在单包Mode才有意义。
    ...
    **图 1**  码流包结构图<a name="fig1125873818118"></a>  
    ...
    -   两种Mode可通过调用接口[ss\_mpi\_venc\_set\_mod\_param](#ZH-CN_TOPIC_0000002408099010)设置h.265e.ko/h264e.ko/jpege.ko模块参数one\_stream\_buf来选择。
        -   one\_stream\_buf=1表示单包Mode；
        -   one\_stream\_buf=0表示非单包Mode，系统默认one\_stream\_buf=0。
...
-   Video Input的原始图像大小必须不小于编码通道的大小。
-   调用该接口发送图像，用户需要保证编码通道已创建且开启接收Input图像。
...
-   用户发送原始图像格式要求：
    ...
    -   JPEG/MJPEG编码通道支持接收Semi-planar YVU 4:2:0/ Semi-planar YUV 4:2:0、Semi-planar YVU 4:2:2/ Semi-planar YUV 4:2:2或单分量Input\(PIXEL\_FORMAT\_YUV\_400\)图像。

-   Video Input的原始图像大小必须不小于编码通道的大小。
-   调用该接口发送图像，用户需要保证编码通道已创建且开启接收Input图像，若码控Mode为CBR/VBR/AVBR/QVBR/CVBR时，用户需调用[ss\_mpi\_venc\_set\_rc\_param](#ZH-CN_TOPIC_0000002441698377)接口使能QpMap功能，且只支持相对Mode。
-   Gop ModeMode为OT\_VENC\_GOP\_MODE\_ADV\_SMART\_P时不支持qpmapMode，调用此接口会返回不支持。
-   Rc为fixqpMode下不支持该接口。
...
-   马赛克的InputMode为OT\_VENC\_MOSAIC\_MODE\_RECT时，即用户Input马赛克区域，SDK内部执行矩形的马赛克叠加。马赛克使用CPU叠加到图像上，内部使用了neon汇编优化；马赛克块大小仅支持OT\_MOSAIC\_BLK\_SIZE\_32和OT\_MOSAIC\_BLK\_SIZE\_64两种；支持最多200个马赛克区域，区域可以重叠；每个马赛克区域起始点（x，y）、结束点（x1=x+w，y1=y+h）不能超出图像边界；在叠加马赛克前起始点（x，y）和结束点（x1，y1）会对齐到32x32或64x64，起始点（x，y）向左上方向对齐，结束点（x1，y1）向右下方向对齐；
...
-   复合编码使能后，RCMode支持CBR、VBR、AVBR、CVBR、QVBR、FixQP，不支持QpMapMode，也不支持在CBR等Mode中使用相对QpMap。
...
-   当GOP MODE为OT\_VENC\_GOP\_MODE\_SMART\_P，只支持Enables an IDR frame.
...
-   ROI参数主要由五个参数决定。
    ...
    -   rect：指定当前的ROI区域的位置Coordinates和区域的大小。ROI区域的起始点Coordinates必须在图像范围内，且必须16对齐；ROI区域的长宽必须是16对齐并且大于0；ROI区域必须在图像范围内。
...
-   仅支持GOPMode为OT\_VENC\_GOP\_MODE\_NORMAL\_P的编码通道。
...
-   SS528V100/SS625V100/SS524V100/SS522V101/Not supported on Hi3403V100/SS626V100.量化表。
...
-   如果前端Input图像的色域范围发生变化（如BT601切换为BT709）时，会立即编码一帧IDR帧。
...
-   本接口用于Gets the VUI configuration for the H.265 encoding channel.。
-   This interface must be called after the encoding channel is created and before it is destroyed.
-   It is recommended to call this interface after the channel is created and before encoding starts, to reduce the number of calls during encoding.
...
-   高级参数主要由五个参数组成
    ...
    -   mcu\_per\_ecs：每个Ecs中包含多少Mcu。系统Modemcu\_per\_ecs = 0，表示当前帧的所有的MCU被编码为一个ECS。mcu\_per\_ecs的最小值为0，最大值不超过\{\(picwidth+15\)\>\>4 \* \(picheight+15\)\>\>4 \* 2, 65535\}的较小值。当mcu\_per\_ecs配置较小时，容易引起CPU占用时间较长等现象，客户需要根据实际使用需求合理配置。
...
-   码率控制器的高级参数由以下参数组成：
    ...
    -   max\_qp&min\_qp：CBR高级参数，max\_qp、min\_qp表示的是当前帧的最大Qp和最小Qp。这个钳位效果最强烈，所有其他对图像Qp的调整，如宏块级码率控制，最终都会被约束到这个最大Qp和最小Qp。min\_qp，max\_qp内部有默认值，在对质量None特殊需求下，建议不更改此组参数。
    -   max\_i\_qp&min\_i\_qp：CBR高级参数，max\_i\_qp、min\_i\_qp表示的是当前序列IDR帧的最大Qp和最小Qp。这个钳位效果最强烈，所有其他对图像Qp的调整，如宏块级码率控制，最终都会被约束到这个最大Qp和最小Qp。min\_i\_qp，max\_i\_qp内部有默认值，在对质量None特殊需求下，建议不更改此组参数。
    ...
    -   max\_i\_qp&min\_i\_qp：VBR高级参数，max\_i\_qp、min\_i\_qp表示的是当前序列IDR帧的最大Qp和最小Qp。这个钳位效果最强烈，所有其他对图像Qp的调整，如宏块级码率控制，最终都会被约束到这个最大Qp和最小Qp。默认值min\_i\_qp为16，max\_i\_qp为51。在对质量None特殊需求下，建议不更改此组参数。
    ...
    -   max\_qp&min\_qp：AVBR高级参数，max\_qp、min\_qp表示的是当前帧的最大Qp和最小Qp。这个钳位效果最强烈，所有其他对图像Qp的调整，如宏块级码率控制，最终都会被约束到这个最大Qp和最小Qp。min\_qp和max\_qp内部有默认值，在对质量None特殊需求下，建议不更改此组参数。
    -   max\_i\_qp&min\_i\_qp：AVBR高级参数，max\_i\_qp、min\_i\_qp表示的是当前序列IDR帧的最大Qp和最小Qp。这个钳位效果最强烈，所有其他对图像Qp的调整，如宏块级码率控制，最终都会被约束到这个最大Qp和最小Qp。默认值min\_i\_qp为16，max\_i\_qp为51。min\_qp和max\_qp内部有默认值，在对质量None特殊需求下，建议不更改此组参数。
    ...
    -   max\_qp&min\_qp：QVBR高级参数，max\_qp、min\_qp表示的是当前帧的最大Qp和最小Qp。这个钳位效果最强烈，所有其他对图像Qp的调整，如宏块级码率控制，最终都会被约束到这个最大Qp和最小Qp。默认值min\_qp为16，max\_qp为51在对质量None特殊需求下，建议不更改此组参数。
    -   max\_i\_qp&min\_i\_qp：QVBR高级参数，max\_i\_qp、min\_i\_qp表示的是当前序列IDR帧的最大Qp和最小Qp。这个钳位效果最强烈，所有其他对图像Qp的调整，如宏块级码率控制，最终都会被约束到这个最大Qp和最小Qp。默认值min\_i\_qp为16，max\_i\_qp为51。在对质量None特殊需求下，建议不更改此组参数。
    ...
    -   max\_qp&min\_qp：CVBR高级参数，max\_qp、min\_qp表示的是当前帧的最大Qp和最小Qp。这个钳位效果最强烈，所有其他对图像Qp的调整，如宏块级码率控制，最终都会被约束到这个最大Qp和最小Qp。默认值min\_qp为22，max\_qp为47在对质量None特殊需求下，建议不更改此组参数。
    -   max\_i\_qp&min\_i\_qp：CVBR高级参数，max\_i\_qp、min\_i\_qp表示的是当前序列IDR帧的最大Qp和最小Qp。这个钳位效果最强烈，所有其他对图像Qp的调整，如宏块级码率控制，最终都会被约束到这个最大Qp和最小Qp。默认值min\_i\_qp为20，max\_i\_qp为47。在对质量None特殊需求下，建议不更改此组参数。
...
-   创建H.264/H.265协议编码通道时，默认跳帧参考Mode是1倍跳帧参考Mode。如果用户需要修改编码通道的跳帧参考，建议在创建通道之后，启动编码之前调用此接口，减少在编码过程中调用的次数。
-   本接口在编码过程中被调用时，等到下一个I帧时生效。
-   如果H.264协议编码通道的profile为svc-t，调用该接口则返回操作不允许错误。
-   如果用户设置1倍跳帧参考Mode，对应的配置为：pred\_en = TD\_TRUE，enhance = 0，base = 1； 如果设置2倍跳帧参考Mode，对应的配置为：pred\_en = TD\_TRUE，enhance = 1，base = 1；如果设置4倍跳帧参考Mode，对应的配置为：pred\_en = TD\_TRUE，enhance = 1，base = 2；
...
-   当编码帧存分配使用UserVB方式且打开帧节省Mode时，由于高级跳帧参考参数变化，导致参考帧个数增加，内部会判断已绑定的VB池存储Picture的VB个数是否满足需求，如果不够，则接口会返回Failure。
...
-   本接口用于Sets the snapshot mode for the JPEG snapshot channel.
-   This interface must be called after the encoding channel is created and before it is destroyed.
-   本接口只对JPEG类型的编码通道有效。
-   通道抓拍Mode有两种：
    -   OT\_VENC\_JPEG\_ENC\_ALLMode：通道启动接收图像之后，编码所有接收的图像；
...
-   SS528V100/SS625V100/SS524V100/SS522V101/Hi3403V100/SS626V100Solution上，对应绑定的VPSS 通道支持压缩和非压缩格式Output。
...
-   Slice分割属性主要由以下参数决定
    ...
    -   split\_mode：slice分割Mode，split\_mode = 0表示按照byte数来进行分割，split\_mode = 1表示按照宏块行或者LCU行进行分割。split\_mode \>=2不被支持。
    ...
    -   slice\_output\_en：仅Hi3403V100/SS626V100/SS524V100/SS528V100支持，不支持的默认值为TD\_FALSE，当slice\_output\_en为TD\_TRUE时，通道属性is\_by\_frame必须为TD\_FALSE，当slice\_output\_en为TD\_TRUE时仅支持split\_mode = 1的Mode。
...
-   搜索窗主要由三个参数决定：
    -   mode：Auto Mode：使用SDK内部默认值；手动Mode：当用户设置的搜索窗小于内部默认值时使用用户设置的值，当用户设置的搜索窗大小等于内部默认值时使用内部默认值。
...
-   Trans属性主要由以下参数决定
    ...
    -   scaling\_list\_en：表示量化表是否有效，None量化表时置为0。
...
-   SS528V100/SS625V100/SS524V100/SS522V101/Hi3403V100/SS626V100仅支持None量化表。
...
-   本接口在编码通道创建之后，编码通道销毁之前设置。此接口在编码过程中被调用时，等到下一帧时生效。如果在gopMode切换后调用该接口，需等到GOP生效后该接口生效。
-   p\_skip帧仅支持GOPMode为OT\_VENC\_GOP\_MODE\_NORMAL\_P的编码通道。
...
-   本接口仅支持H.264/H.265/Mjpeg 的CBR/VBR码率控制Mode、H.264/H.265 的AVBR/QVBR/CVBR码率控制Mode的参数的设置。
...
-   如果lost\_param为空，则返回Failure。
-   切换GOPMode时，获取的属性是已设置的最新的属性。
-   本接口仅支持H.264/H.265/Mjpeg 的CBR/VBR码率控制Mode、H.264/H.265 的AVBR/QVBR/CVBR码率控制Mode。
...
-   本接口仅支持H.264/H.265/Mjpeg 的CBR/VBR码率控制Mode、H.264/H.265的AVBR/QVBR/CVBR码率控制Mode的参数的设置。
...
-   本接口仅支持H.264/H.265/MJPEG的CBR/VBR码率控制Mode和H.264/H.265的AVBR/QVBR/CVBR码率控制Mode。
...
-   使能P帧刷I slice，不支持QPMapMode使能skip，内部会自适应关闭skip。
...
-   此接口在通道创建前调用，如果通道已经创建，则返回FailureOT\_ERR\_VENC\_NOT\_PERM。
...
-   SSE 参数主要由三个参数决定。
    ...
    -   rect：指定当前的SSE 区域的位置Coordinates和区域的大小。

        H.264的SSE区域的起始点Coordinates必须在图像范围内，且必须16对齐；SSE区域的长宽必须是16对齐，且区域必须在图像范围内。

        H.265的SSE区域的起始点Coordinates必须在图像范围内，且必须LCU对齐；SSE区域的长宽必须是LCU对齐，且区域必须在图像范围内。
...
-   VENC高级参数主要由六个参数决定。
    ...
    -   max\_stream\_cnt：用于设置编码通道的码流buffer中能够缓存的最大码流帧数
        ...
        在创建通道之后，销毁编码通道之前均可以被设置。在下一帧开始编码之前生效。此参数允许被多次设置，建议在创建编码通道Success之后，启动编码前进行设置，不建议在编码过程中动态调整。
    ...
    -   frame\_rate：设置通道帧率控制属性
        ...
        当Input帧率src\_frame\_rate大于或者等于Output帧率dst\_frame\_rate时为丢帧（减帧）Mode，取Input的部分帧进行编码。例如，src\_frame\_rate = 30，dst\_frame\_rate = 15时，表示每Input30帧图像，只取其中的15帧进行编码。

        丢帧（减帧）Mode下，当前端为固定频率的源时，例如VI，通道的帧率控制则准确；当前端为不固定频率的源时，例如VDEC，通道的帧率控制则不准确。

        JPEG不支持增帧Mode，Input帧率src\_frame\_rate必须大于或者等于Output帧率dst\_frame\_rate。
...
-   本接口属于高级接口，用户可以选择性调用，系统默认Scene Mode为OT\_VENC\_SCENE\_0。

-   Only H.265/H.264 encoding channels are supported.
-   设置Scene Mode为SCENE\_2时建议使能默认量化表，注意部分Solution不支持量化表。
...
-   用户必须调用接口ss\_mpi\_vb\_create\_pool（请参考”System Control”章节）创建一个视频缓存VB池，再通过调用接口ss\_mpi\_venc\_attach\_vb\_pool把当前编码通道绑定到固定PoolId的VB池中。可以把多个编码通道绑定到同一个VB池中，但是不能把同一个编码通道绑定到多个VB池中。
...
-   只有H.264/H.265编码支持UserVB池方式，如果当前编码帧存分配方式使用的不是编码UserVB池，则返回通道不支持的Error Code。
...
-   The channel must be created; otherwise an error code indicating the channel is not created is returned.
-   如果当前编码帧存分配方式使用的不是编码UserVB池，则返回通道不支持的Error Code。
-   调用该接口后，不保证内部已完成VB池占用的释放，如果要进行销毁VB池建议首先调用接口[ss\_mpi\_venc\_reset\_chn](#ZH-CN_TOPIC_0000002408258510)。
-   VB池如绑定了开启帧节省Mode的编码通道，则销毁VB池前，必须调用ss\_mpi\_venc\_detach\_vb\_pool接口解绑定，或销毁对应绑定的编码通道，VB池的占用才会释放。
...
-   The channel must be created; otherwise an error code indicating the channel is not created is returned.
-   本接口用于调试通道cu/mb的Mode选择的倾向性，支持调节inter和intra的倾向性配置以及编码块大小的倾向性配置。
-   This interface must be set after the encoding channel is created and before it is destroyed. When called during encoding, it takes effect on the next frame.
-   It is recommended to call this interface after the channel is created and before encoding starts, to reduce the number of calls during encoding.
-   建议用户在调用此接口之前，先调用[ss\_mpi\_venc\_get\_cu\_pred](#ZH-CN_TOPIC_0000002441658549)接口，获取CUMode选择的倾向性的配置，然后再进行设置。
-   该功能主要由如下九个参数决定。
    -   pred\_mode：倾向性选择Mode，支持auto和manual两种Mode。autoMode下由驱动内部完成倾向性的配置，manualMode允许用户根据实际场景调用该接口完成自定义的倾向性配置。
...
-   Only H.265/H.264 encoding channels are supported.H.264编码通道推荐使用High profile。
...
-   This interface must be set after the encoding channel is created and before it is destroyed. When called during encoding, it takes effect on the next frame.
-   It is recommended to call this interface after the channel is created and before encoding starts, to reduce the number of calls during encoding.
-   建议用户在调用此接口之前，先调用[ss\_mpi\_venc\_get\_skip\_bias](#ZH-CN_TOPIC_0000002408098726)接口，获取cu/mb选择SkipMode的倾向性配置，然后再进行设置。
-   该功能主要由如下五个参数决定。
    -   enable：skip倾向性使能控制。
    -   gain和offset是前景检测的控制参数，用于生成前景检测的阈值。阈值计算如下：\(\(gain \* madi + 8\) \>\> 4\) + offset，其中madi为当前块像素的平均绝对误差\(首先计算当前块像素的均值，然后计算当前块所有像素与该均值绝对差的均值\)。运动检测的方法：利用运动估计结果残差的SAD，跟这个阈值进行比较，若大于这个阈值则认为是前景部分。因此对于同一场景，阈值越小，检测出来前景区域越大，反之亦然。
...
-   VPSS-VENC处于卷绕Low Latency或单帧Low LatencyMode时，不支持去呼吸效应功能。如果用户同时开启了Low Latency和去呼吸效应功能，去呼吸效应功能None效。SS528V100/SS625V100/SS524V100/SS522V101/SS626V100不支持Low Latency，Hi3403V100支持非卷绕Low Latency。
-   支持同时开启帧节省Mode和去呼吸效应。
-   该功能主要由如下三个参数决定。
    -   enable：去除呼吸效应使能控制。
...
-   建议在GOPMode为OT\_VENC\_GOP\_MODE\_NORMAL\_P和OT\_VENC\_GOP\_MODE\_SMART\_P和OT\_VENC\_GOP\_MODE\_ADV\_SMART\_P时使用该功能。
-   该功能主要由如下参数决定。
    -   enable：分层qp功能使能配置。
...
-   仅H.264/H.265用于Gets the hierarchical QP parameters.
-   This interface must be called after the encoding channel is created and before it is destroyed.
...
-   本接口用于Gets the RC module advanced parameters.
-   本接口必须在通道创建之后，通道销毁之前调用。
-   此接口仅支持H.264/H.265/MJPEG通道。
...
-   ROI参数：
    ...
    -   rect：指定当前的ROI的起始Coordinates和区域的大小。ROI的起始点Coordinates必须在图像范围内，且必须16对齐；ROI区域的长宽必须是16对齐并且大于0；ROI区域必须在图像范围内。
...
-   支持码控Mode为CBR、VBR、AVBR、QVBR、CVBR下的智能编码。
    -   在CBRMode下，设置相同的目标码率时，开启智能编码可以增强前景图像质量；
    -   在VBR、QVBR、CVBRMode下，开启智能编码时，可以在保证前景质量的同时降低码率；或者在不改变背景质量的情况下，提升前景的质量。
    -   AVBRMode下，开启后对码率的影响视场景变化。

-   智能编码与QpMapMode、FixQpMode、ROI、PSkip功能、MD相关接口互斥。
...
-   This interface must be called after the encoding channel is created and before it is destroyed.本接口在编码过程中被调用时，等到下一个I帧时生效。
-   It is recommended to call this interface after the channel is created and before encoding starts, to reduce the number of calls during encoding.
...
-   本接口用于Gets the strong edge dering level for the JPEG encoding channel.
...
-   本接口用于Sets the strong edge dering level for the JPEG encoding channel.
-   This interface must be called after the encoding channel is created and before it is destroyed.
-   本接口只对JPEG类型的编码通道有效且只在dering\_mode为1的情况下才能够调用。
-   强边去RING效应强度等级有四种：

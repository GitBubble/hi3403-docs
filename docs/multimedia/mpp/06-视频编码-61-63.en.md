...
-   some/partial Solutionsupportsmultiple/moreencoderparallel encoding，H.265eencodingsupportsvertical  divide sub-Tile，based on channelwidth/heightand rate controllerOutputframe ratedetermine tile divide sub-count ；H.264eencodingsupports水平 divide sub-Tile\(actual is  divide sub-slice\)，based on channelwidth/heightand frame ratedetermine Tile divide sub-count 。SS528V100/SS625V100/SS524V100/SS522V101/Not supported on Hi3403V100/SS626V100.。
...
-   To ensure optimal encoding/decoding performance on this chip, when only 3 tiles are divided, the interior of each tile is evenly divided into 2 slices. However, if users callinterface[ss\_mpi\_venc\_set\_slice\_split](#ZH-CN_TOPIC_0000002441698373)perform  divide sub- whenwithin part will not self-动 divide sub-。
-   Recommended encoding dimensions: 7680x4320 (8K), 3840x2160 (4K), 1920x1080 (1080P), 1280x720 (720P).
-   encoderattributein/within except/divide bychannelwidth/height（pic\_widthand pic\_height）outside all is static attribute，one 旦createencoding channelSuccess，static attributenot supportedis modify ，except/divide by non-the channelis destroy，re-create。Sets  whenmustnote's 事 itemrefer to [ss\_mpi\_venc\_set\_chn\_attr](#ZH-CN_TOPIC_0000002441697697)interfaceDescription。
-   H.264/H.265，encodingframe bufferby/from YHeaderSize、CHeaderSize、YSize、CSize、PmeSize、 PmeInfoSizeand TmvSizetogether same构 become，encoderdefault based on maximum width/heightcalculate frame bufferperform memory allocate ，in/at Sets channelwidth/height when needensure based on channelwidth/heightcalculate 's each one some/partial frame buffersize all cannot greater than maximum width/heightcalculate 's frame buffersize 。
...
-   CVBR requires setting the following bitrate parameters:
    ...
    -   extra\_bit\_percent: encoderOutputstreammaximum overdraftbit numberrelative to long\_term\_max\_bit\_rate's percentage 。when bitrate is insufficient, to ensure imagequality ，encoderwill via overdraftone 定bit numberso as to improve imagequality ，this some/partial overdraft's bitratewill in/at encoder compress strength/powerrelatively small  whenperform 偿also/still 。

-   FIXQPIn addition to the above attributes, also requires settingi\_qp，p\_qp，b\_qp。i\_qp: Iframe when, imagefixed QPvalue.p\_qp: Pframe when, imagefixed QPvalue.b\_qp: Bframe when, imagefixed QPvalue.in/at Sets IframeQP，PframeQP when, can based on current's 带width limit for/to IframeQPand PframeQPsimultaneously perform to on/above or is to below/under  adjust whole/entire。is/for  reducefew/fewerbreathing effect，recommended IframeQP始终 comparePframe's QPsmall 2～3。
...
-   JPEGEencoding channelOT\_VENC\_PIC\_RECV\_MULTIModeused for encodingMPF/DCF，at this time encodingwill not callVGS moduleblock perform imagedownscale 。enablethe/this Modeafter mustcall ss\_mpi\_sys\_bindinterfacewill encoding channelbindto multiple Input source，ss\_mpi\_sys\_bind传递's Data Receiver's Device IDactual used for specified Input source，can use OT\_VENC\_RECV\_SRC0、OT\_VENC\_RECV\_SRC1、OT\_VENC\_RECV\_SRC2、OT\_VENC\_RECV\_SRC3枚举perform Input sourcespecified 。
...
-   The stream structure ot\_venc\_stream contains the following parts:
    -   Stream packet information pointer pack
         ...
         point to/indicateto one Groupot\_venc\_pack's memory space ，the/this space by/from call allocate 。ifis per packet Gets ，then this space not less than sizeof（ot\_venc\_pack）'s size ；if press/byframeGets ，then this space not less than N x sizeof（ot\_venc\_pack）'s size ，where N generation/represent tablecurrentframe之 in  packet/include's count ，can in/at selectafter via query interface获resulting in 。
...
-   If the user does not retrieve the stream for a long time, the stream buffer will become full.one encoding channelifoccurs streambuffer  region满，then will not again/then startencoding， directlyto user Gets stream，thereby has/have  sufficient enough/sufficient's streambuffer can used for encoding when, before start 继续encoding。
-   It is recommended to pair stream acquisition and release calls and release the stream as soon as possible，防止appears due to user 态Gets stream，releasenot and  when and/whilecauses 's streambuffer满，stopencoding。
...
-   When one\_stream\_buf is 1, retrieving one frame of stream returns only one stream packet address (not including user data), i.e., pack\_cnt is 1 and the address is pack\[0\].addr. The ot\_venc\_packstructure  body/structin/within  increase addoffset，used to  point to/indicate output/outone framestreamin/within valid/effective data 's address and pack\[0\].addr's 偏移。such as/if  figure1-20as shown（so as to H.264single-packet is/for  example），pack\[0\]. data\_num = 3,i.e., 3 typeNAL packet/include，respectively is/for SPS、PPS、SEI。pack\[0\]. pack\_info\[0\]. pack\_type. h264\_type = OT\_VENC\_H264\_NALU\_SPS，pack\[0\]. pack\_info \[1\]. pack\_type. h264\_type = OT\_VENC\_H264\_NALU\_PPS，pack\[0\]. pack\_info \[2\]. pack\_type. h264\_type = OT\_VENC\_H264\_NALU\_SEI；other protocolso as to this by analogy，data\_numand pack\_infoonlyin/at single-packet modebefore has/have 意义。
    ...
    **Figure 1** Stream packet structure diagram<a name="fig1125873818118"></a>  
    ...
    -   streamGets supportscachemethod/mode ， canby calling interface  [ss\_mpi\_venc\_set\_mod\_param](#ZH-CN_TOPIC_0000002408099010)  Sets vencmodule parameterbuf\_cache toselect 。
        ...
        when each framestreamonlyfrom streambuffer拷贝once  whennot recommendeduse cachemethod/mode 。
...
-   If the channel has not been created, failure is returned.
-   ifdatais/for  empty/空，then returns Failure。
...
-   H.264/H.265protocol，in/at callinterface[ss\_mpi\_venc\_set\_mod\_param](#ZH-CN_TOPIC_0000002408099010)will encodingstreamframeModeis/for single-packet mode when, users data 's SEI packet/includenot perform 拼 packet/includeprocessing 。
-   JPEG/MJPEGprotocolchannelallocate 1block 1k bytesize 's memory space ，used for bufferuser data ，can insert multiple/moreblock user data ，but  totalsize cannot exceeds memory space 's limit 。user data so as to APP segment（0xFFEF） shape style添 addto imagestreamin/within 。in/at user data is encodingsend after, JPEG/MJPEGchannelwithin bufferthis  segmentuser data 's memory space is  clear/HD zero，used for  store/exist enlargenew 's user data.
...
-   this interfacemustin/at enablecomposite encodingafter use ，supportsuser once sendtwo 张imageand corresponding mosaic region information  to/untilencoding channel。
-   If milli\_sec is less than -1, returns OT\_ERR\_VENC\_ILLEGAL\_PARAM.
-   user sendraw imagemustis/for Semi-planar YVU 4:2:0/Semi-planar YUV 4:2:0/PIXEL\_FORMAT\_YUV\_400image。
-   composite encodingonlysupports8bitpixel bit width 。Input's two 张imagemustis the same  when刻capture 's ，corresponding time\_refand ptsmust mutual same。composite encodingnot supportedencodinginternal CROPand VGSrotation  scale enlarge, etc.operation ，Inputimagesize mustequal to encoding channel's size 。ifmustfor/to Inputimageoverlaymosaic ，imageot\_venc\_multi\_frame\_info:frame\[0\]cannot use compressionMode。
...
-   when use  rectangle/frameMode when, due to mosaic use CPUoverlayto imageon/above ，ifoverlaymosaic 's region  surface/side积very large ，or   re-overlay's region very multiple/more，will  increase addCPUoccupied by/uses  rate。
-   mosaic 's InputModeis/for OT\_VENC\_MOSAIC\_MODE\_MAPMode when, i.e., user Inputmosaic  table；the/this ModesupportsOT\_MOSAIC\_BLK\_SIZE\_4、OT\_MOSAIC\_BLK\_SIZE\_8、OT\_MOSAIC\_BLK\_SIZE\_16、OT\_MOSAIC\_BLK\_SIZE\_32、OT\_MOSAIC\_BLK\_SIZE\_64and OT\_MOSAIC\_BLK\_SIZE\_128together6 typemosaic block size ，supportsso as to  lefton/above corner pixel 颜 colorfill/padding and specified YUV value纯 colorfill/padding two  typemosaic block fill/padding Mode。each block by/from 1 bitindicates whether  enable/openmosaic ，1indicates  enable/open，0indicates not  enable/open，strideis/for 128bit。mosaic\_map\_stride = align\(align\(width, blk\_size\) / blk\_size, 128\) / 8; mosaic\_map\_size = align\(height, blk\_size\) / blk\_size \* stride。
-   When calling this interface to send images, users must ensure the encoding channel has been created and is accepting input images.
...
-   enablecomposite encoding after, customer can Sets quality level 。quality level is/for 0 whenencoding channelmust increase addone  re-构frameVB， totalVBcount 's calculate method is : TotalVB = RefNum + 2；quality level is/for 1 whenencoding channelmust's reference framemust increase addone  times， totalVBcount 's calculate method is : TotalVB = RefNum \* 2 + 2。reference framecount RefNum retrieve/get determinein/at GOPstructure and  skipframereference parameter。
-   mosaic functionnot supportedsingle-独enable，in/at enablecomposite encodingbefore can 添 addmosaic 。
-   composite encodingenableafter except/divide bystreamlong  degreeoutside ，other streamreport information all onlystatistics 基础 layerimage's data.
...
-   composite encodingenable after, cannot use p\_skipencoding， affected byaffect 's has/have : 
    -   RCbitratepast/too 冲insert p\_skipframe；
...
-   due to this interfacenot  affected byframe ratecontrol affect ，each callonce i.e., encode outputone VI，call频繁will affect streamframe rateand bitrate's stable，use  whenmustnote。this outside ，below/under one frameencodingstartbefore multiple times interfacecallonlywill encode outputone VI。
-   please  requireVIframeonlyused for GOPModeis/for smart\_p。
-   when please  requirevirtual Iframe碰on/above currentframe encodeIDRframe，not affect IDRframeencoding，VIframe does not take effect.
...
-   due to this interfacenot  affected byframe ratecontrol affect ，each callonce i.e., encode outputone IDR，call频繁will affect streamframe rateand bitrate's stable，use  whenmustnote。this outside ，below/under one frameencodingstartbefore multiple times interfacecallonlywill encode outputone IDR。
-   when GOPModeis/for smart\_por BframeMode under, please  requireIDRframewill 延 takes effect.
...
-   if not enableIDRframe，then in/at below/under one frameafter all  encodenot  output/outIDRframeor Iframe， directlyto again enableis/for 止。
...
-   this interfacecannot with/and its/their it MPIinterfacesimultaneously call，user must ensurethis interfacewith/and its/their it interfacein/at time on/above is 串line call's 。
-   this interfaceused for closeVENCchannel文件handle ，recommendedin/at VENCchanneldestroycondition/casebelow/under use 。
-   use selectmethod/mode 监听stream when, not  capabilityclosecorresponding channelfd，otherwisewill affect stream监听。
...
-   ROIparametermainly by/from 五 parameterdetermine 。
...
Sets H.264/H.265channel's ROIconfigureheight  levelattribute。can  sub-开Sets Iframe、P/Bframe、VIframe's ROIattribute。
...
-   ROIparametermainly by/from 五 parameterdetermine 
    ...
    -   is\_abs\_qp: specified current's ROIregion adopt/use absoluteQPmethod/mode or is  mutualfor/to QP。
    -   qp: when is\_abs\_qp is/for true when, qp indicates ROIregion internal allmacro block adopt/use 's QP value，when is\_abs\_qp is/for false when, qpindicates ROIregion internal allmacro block adopt/use 's  mutualfor/to QPvalue.
    -   rect: specified current's ROI region'sbit  setCoordinatesand  region'ssize 。ROI region'sstarting pointCoordinatesmustin/at imagerange  within, and must16alignment；ROI region'slong width mustis 16alignment；ROIregion mustin/at imagerange within 。calculate imagerange  when, image's width/heightwill expand充to 16pixel integer  times，ensure image边界 placealso can Sets ROIregion 。
...
-   Sets the/this interface after, ifcurrentframedetermine encodingis/for p\_skipframe，so as to p\_skipframeeffect better/optimalfirst 。
-   this interfacecan  sub-开Sets Iframeand P、Bframe's ROIattribute， numberGroup标 number/IDis/for 0's  generation/represent tableIframe's ROIattribute， numberGroup标 number/IDis/for 1 generation/represent tableP、Bframe's ROIattribute， numberGroup标 number/IDis/for 2 generation/represent tableVIframe's ROIattribute。Iframe、P/Bframe、VIframe's ROIattributecan not  same， examplesuch as/if can IframeenableROI，Pframenot enableROI；Iframecan and PframeSets not  same's ROIregion 。
...
-   Sets  non-ROI region'sframe ratelow in/at ROIregion  when, not recommendeduse  skipframereference or  svc-tencoding。 skipframereference or svc-tencoding，ifSets  non-ROI region'sframe ratelow in/at ROIregion ，then actual encoding's  non-ROIregion target frame ratemay greater than user configure's target frame rate，因is/for base layer's  non-ROIregion cannot encodingis/for p\_skipblock 。
-   onlysupportsGOPModeis/for OT\_VENC\_GOP\_MODE\_NORMAL\_P's encoding channel。
-   GOPModeswitch  after, the/this interfaceinvalidate ，such as/if  need继续use mustre-Sets the/this interface。ifGOPModeis from  non-NORMAL\_P切to NORMAL\_P，Modeswitch after callthe/this interface needwait until GOPtakes effect after the/this interface generateeffect.
...
-   when Sets H.265protocolchannel non-ROI region'sframe ratelow in/at ROIregion  when, this  moduleblock within part will closeSAO滤波，故recommendedcustomer call[ss\_mpi\_venc\_set\_h265\_sao](#ZH-CN_TOPIC_0000002441658333)closeSAOfunction。
...
-   transform 、 amount/quantity convertattributemainly by/from 六 parameterGroup become: 
    ...
    -   inter\_scaling\_list8x8: for/to frameinter-prediction macroblock perform 8x8transform  when,  canby/from user via this  numberGroupprovides quantization table,保留， temporarily whenno/without use 。
    -   intra\_scaling\_list8x8: for/to frameintra-prediction macroblock perform 8x8transform  when,  canby/from user via this  numberGroupprovides quantization table,保留， temporarily whenno/without use 。
...
-   transform 、 amount/quantity convertattributemainly by/from 四 parameterGroup become: 
    ...
    -   cabac\_init\_idc: cabacinitialize 索引，system default is/for 0。specific meaning refer to H.264protocol。
...
-   Cabacencodingmethod/mode relative to cavlcencodingmethod/mode ，mustlarger 's calculate  amount/quantity， canis ，will 产 generatefewer 's stream。ifsystem performance not  enough/sufficient富裕，recommendeduser can 采 retrieve/getIframecavlcencoding，P/Bframecabacencoding。
-   this interfaceis an advanced interface，users may selectively call，not recommendedcall，system will has/have default value.system will based on not  same's profileSets default parameter。
...
-   ifuser 想closedeblockingfunction，can will disable\_deblocking\_filter\_idc setis/for 1。
...
-   advanced parametersmainly by/from 五 parameterGroup become
    -   qfactor: quantization table因sub-range is/for \[1, 99\]，qfactorthe more large ，quantization table in  amount/quantity convertcoefficient the more small ，resulting in to 's imagequality will more good ， same when, encodingcompression ratemore low 。 same理qfactorthe more small ，quantization table in  amount/quantity convertcoefficient the more large ，resulting in to 's imagequality will more  difference， same when, encodingcompression ratemore height 。specific 's qfactorwith/and quantization table's relationship please  seeRFC2435标 accurate/standard。
...
-   ifuser 想use self/own 's quantization table,in/at Sets quantization table's  same when, please will qfactorSets is/for 50。
...
-   advanced parametersmainly by/from 四 parameterGroup become
...
-   RCadvanced parameters现onlysupportsH.264/H.265/Mjpeg's CBR/VBRbitratecontrol Modeand H.264/H.265's AVBR/QVBR/CVBRbitratecontrol Modebelow/under line  leveland macro block  levelrate control 's parameter's Sets 。
-   rate controller's advanced parametersby/from below parameterGroup become: 
    ...
    -   row\_qp\_delta: in/at macro block  levelbitratecontrol  when, each one line macro block 's startingQprelative to framestartingQp's fluctuation幅 degreevalue.for bitratefluctuationrelatively 严 format/grid's scenario  under, can 尝试will this parameter adjustlarge ，implements more  add precise confirm/ensure's bitratecontrol ，but may will causes certain frameimageinternal imagequality has/have  difference different/abnormal。in/at height bitrate when, this  valuerecommended is/for 0；in/within bitrate whenrecommended the/this  valueis/for 0or 1；low bitrate whenrecommended the/this  valueis/for 2～5。
    -   first\_frame\_start\_qp: the one frame's startingQp value，CBR/VBR/AVBR/QVBR/CVBRhas/have effect.first\_frame\_start\_qp ifis/for -1then the one frame's startingQPby/from encoderwithin part calculate ，ifis/for its/their it valid  valuethen by/from user specified the/this valid  valueis/for the one framestartingQP。the/this  valuedefault is/for -1。this  placethe one frame's meaning is : channelcreate，GopModeswitch ，RCModeswitch ，or resolutionswitch  after,  sequencecolumn 's the one IDRframe。mustnoteifthe one frameencodingcomplete after 判定is/for  re- encode， re- encode's framenot is the one frame，not  affected byfirst\_frame\_start\_qp 's constraint 。
    -   direction: in/at macro block  levelbitratecontrol  when, used for control  reduce square/methodto threshold's below/under 标， examplesuch as/if when direction=8，Cindicates imagecomplex  degree，indicates from threshold\[0\]to threshold\[7\]is/for  reduce square/methodto 's threshold ，from threshold\[8\]to threshold\[15\]is  add square/methodto 's  thresholdvalue.
    ...
    -   min\_i\_proportion&max\_i\_proportion: CBRadvanced parameters，respectively represents minimum IPratio and maximum IPratio 。IPratio indicates Iframeand Pframe's Bits number's ratio 。this two  valueused for 钳bit IPratio 's range 。when min\_i\_proportionadjusted larger when, it will causes Iframesharp/clear,Pframeblurry.when max\_i\_proportionadjusted smaller when, it will causes Iframeblurry,Pframesharp/clear。in/at  positive/correct often/usualcondition/casebelow/under not recommendedfor/to IPsize  compareperform constraint ，to avoid breathing effectand bitratefluctuation,default min\_i\_proportionset to 1，max\_i\_proportionset to 100。in/at for/to Iframesize has/have constraint 's scenario  when, can based on for/to Iframesize fluctuation requirements toSets min\_i\_proportionand max\_i\_proportion's value.
    ...
    -   chg\_pos: VBRadvanced parameters，represents VBRstart  adjust whole/entireQp whenbitratewith/and maximum bitrate's  comparevalue.system default is/for 90，ifin/at Contentchanges剧烈 andrequirements not exceeds maximum bitrate's scenario ，recommendedreduce this  value，simultaneously increase delta\_qp，but bitratecontrol stable when's bitrate偏small and quality 偏 difference。
    -   min\_i\_proportion&max\_i\_proportion: VBRadvanced parameters，respectively represents minimum IPratio and maximum IPratio 。IPratio indicates Iframeand Pframe's Bits number's ratio 。this two  valueused for 钳bit IPratio 's range 。when min\_i\_proportionadjusted larger when, it will causes Iframesharp/clear,Pframeblurry.when max\_i\_proportionadjusted smaller when, it will causes Iframeblurry,Pframesharp/clear。in/at  positive/correct often/usualcondition/casebelow/under not recommendedfor/to IPsize  compareperform constraint ，to avoid breathing effectand bitratefluctuation,default min\_i\_proportionset to 1，max\_i\_proportionset to 100。in/at for/to Iframesize has/have constraint 's scenario  when, can based on for/to Iframesize fluctuation requirements toSets min\_i\_proportionand max\_i\_proportion's value.max\_qp&min\_qp: VBRadvanced parameters，max\_qp、min\_qprepresents currentframe maximumQp and minimumQp。This clamp has the strongest effect; allother adjustments to imageQp adjustments, such as macroblock-level bitrate control，will ultimately be constrained to this maximumQp and minimumQp。default  valuemin\_qpis/for 16，max\_qpis/for 51in/at for/to quality Noneunder special requirements, recommendeddo not change this Groupparameter。
    ...
    -   chg\_pos: AVBRadvanced parameters，represents AVBRstart  adjust whole/entireQp whenbitratewith/and maximum bitrate's  comparevalue.system default is/for 90，ifin/at Contentchanges剧烈 andrequirements not exceeds maximum bitrate's scenario ，recommendedreduce this  value，simultaneously increase delta\_qp，but bitratecontrol stable when's bitrate偏small and quality 偏 difference。
    -   min\_i\_proportion&max\_i\_proportion: AVBRadvanced parameters，respectively represents minimum IPratio and maximum IPratio 。IPratio indicates Iframeand Pframe's Bits number's ratio 。this two  valueused for 钳bit IPratio 's range 。when min\_i\_proportionadjusted larger when, it will causes Iframesharp/clear,Pframeblurry.when max\_i\_proportionadjusted smaller when, it will causes Iframeblurry,Pframesharp/clear。in/at  positive/correct often/usualcondition/casebelow/under not recommendedfor/to IPsize  compareperform constraint ，to avoid breathing effectand bitratefluctuation,default min\_i\_proportionset to 1，max\_i\_proportionset to 100。in/at for/to Iframesize has/have constraint 's scenario  when, can based on for/to Iframesize fluctuation requirements toSets min\_i\_proportionand max\_i\_proportion's value.
    ...
    -   min\_qp\_delta: frame levelQPminimum  valueand CU levelQPminimum  value's  difference value，in/at imageContent comparerelatively 简single- when, rate control will reduce Qp value，when frame levelQpadjust to minimum  value when, frame levelbitratecontrol then not again/then below/under  adjustQp，however CU/macro block  levelbitratecontrol 依 then/naturallytakes effect ，imagein/within 平坦region Qp capability enough/sufficientbelow/under  adjustto smaller value.

    QVBRmainly parameteras follows:

    -   min\_i\_proportion&max\_i\_proportion: QVBRadvanced parameters，respectively represents minimum IPratio and maximum IPratio 。IPratio indicates Iframeand Pframe's Bits number's ratio 。this two  valueused for 钳bit IPratio 's range 。when min\_i\_proportionadjusted larger when, it will causes Iframesharp/clear,Pframeblurry.when max\_i\_proportionadjusted smaller when, it will causes Iframeblurry,Pframesharp/clear。in/at  positive/correct often/usualcondition/casebelow/under not recommendedfor/to IPsize  compareperform constraint ，to avoid breathing effectand bitratefluctuation,default min\_i\_proportionset to 1，max\_i\_proportionset to 100。in/at for/to Iframesize has/have constraint 's scenario  when, can based on for/to Iframesize fluctuation requirements toSets min\_i\_proportionand max\_i\_proportion's value.
    ...
    -   min\_i\_proportion&max\_i\_proportion: CVBRadvanced parameters，respectively represents minimum IPratio and maximum IPratio 。IPratio indicates Iframeand Pframe's Bits number's ratio 。this two  valueused for 钳bit IPratio 's range 。
    ...
    -   min\_qp\_delta: CVBRadvanced parameters，frame levelQPminimum  valueand CU levelQPminimum  value's  difference value，in/at imageContent comparerelatively 简single- when, rate control will reduce Qp value，when frame levelQpadjust to minimum  value when, frame levelbitratecontrol then not again/then below/under  adjustQp，however CU/macro block  levelbitratecontrol 依 then/naturallytakes effect ，imagein/within 平坦region Qp capability enough/sufficientbelow/under  adjustto smaller value.
    -   max\_qp\_delta: CVBRadvanced parameters，frame levelQPmaximum  valueand CU levelQPmaximum  value's  difference value，in/at imageContent comparerelatively complex  when, rate control will increase Qp value，when frame levelQpadjust to maximum  value when, frame levelbitratecontrol then not again/then on/above  adjustQp，however CU/macro block  levelbitratecontrol 依 then/naturallytakes effect ，imagein/within  strong/enhance纹理region Qp capability enough/sufficienton/above 升to larger value.

-   It is recommended to call this interface after the channel is created and before encoding starts, to reduce the number of calls during encoding.
-   VPSS-VENC placein/at wrap-around Low Latencyor single-frameLow LatencyMode when, not supportedbitratecontrol  re- encodefunction，please customer Sets maximum  re- encode time numberis/for 0，otherwisewill causes channel频繁discard frame。SS528V100、SS625V100、SS524V100、SS522V101、SS626V100not supportedLow Latency，onlyHi3403V100supports non-wrap-around Low Latency。
...
-   ifrc\_paramis/for  empty/空，then returns Failure。
...
-   ifref\_paramis/for  empty/空，then returns Failure。
...
-   channel抓拍Modehas/have two  type: 
    -   OT\_VENC\_JPEG\_ENC\_ALLMode: channelstartreceiveimage after, encodingallreceive's image；
    -   OT\_VENC\_JPEG\_ENC\_SNAP Mode: channelstartreceiveimage after, onlyencodingis mark is/for 抓拍frame's image。

-   createJPEGchannel after, JPEGchanneldefault  placein/at OT\_VENC\_JPEG\_ENC\_ALLMode。when user mustonly抓拍is mark 's image when, can callthis interface，Sets JPEGchannelis/for OT\_VENC\_JPEG\_ENC\_SNAP Mode。
...
Gets JPEGencoding channel's 抓拍Mode。
...
-   This interface gets JPEGchannel's 抓拍Mode。
-   this interfaceonlyfor/to JPEGType's encoding channelhas/have effect.
...
-   Slice sub- splitattributemainly by/from below parameterdetermine 
    ...
    split\_modemustin/at enableis/for 真's 条件below/under valid/effective ，otherwise，Noneeffect.
...
-   when  press/bybyte  divide sub-slice when, if customer configure's slice\_sizerelatively small ，will may exist one frameis  sub- split becomerelatively multiple/moreslice's condition/case。at this time will causes streamcompression raterelatively low and CPUoccupied by/uses time relatively long , etc.phenomenon ，customer must结合actual  need require合理configure。
...
-   search window mainly by/from three  parameterdetermine : 
    ...
    -   hor: user Sets 's search window 水平range ，when modeis/for manual Mode whenvalid/effective ，minimum  valueis/for 32，not check on/above 限。
    -   ver: user Sets 's search window vertical range ，when modeis/for manual Mode whenvalid/effective ，minimum  valueis/for 32，not check on/above 限。
...
-   Transattributemainly by/from below parameterdetermine 
    -   cb\_qp\_offset: for the specific meaning, refer to H.265protocol关in/at pps\_cb\_qp\_offset's  decode/solve释。
    -   cr\_qp\_offset: for the specific meaning, refer to H.265protocol关in/at pps\_cr\_qp\_offset's  decode/solve释。
...
-   This interface sets H.265protocolencoding channel's Deblockingattributeand 边界滤波attribute。
...
-   this interfaceis an advanced interface，users may selectively call，recommendednot call，system default open deblockingfunctionand slice, tile边界滤波function，default slice\_deblocking\_filter\_disabled\_flag = 0，slice\_tc\_offset\_div2 = 0，slice\_beta\_offset\_div2 = 0，loop\_filter\_across\_tiles\_enabled\_flag=1，loop\_filter\_across\_slices\_enabled\_flag=1。
-   ifuser 想closedeblockingfunction，can will slice\_deblocking\_filter\_disabled\_flag setis/for 1。
-   ifenableslicewith/and tile边界滤波function，in/at  divide sub-sliceand tile takes effect.
-   This interface must be configured after the encoding channel is created and before it is destroyed.this interfacein/at encoding when called during encoding，Deblockingattributewait until below/under one frame whentakes effect ，边界滤波attributewait until below/under one Iframe generateeffect.
-   It is recommended to call this interface after the channel is created and before encoding starts, to reduce the number of calls during encoding.
...
-   Saoattributemainly by/from two parameterdetermine 
    -   slice\_sao\_luma\_flag: currentslicebrightness sub- amount/quantitywhether 作Sao滤波。
    -   slice\_sao\_chroma\_flag: currentslice color degree sub- amount/quantitywhether 作Sao滤波。
...
-   iflost\_paramis/for  empty/空，then returns Failure。
-   lost\_parammainly by/from 四 parameterdetermine 
    -   enable: discard frame开关
    -   bit\_rate\_threshold: discard framethreshold 
    -   frame\_gap: maximum allow 连续discard frameframe number。
...
-   frame\_gapcontrol whether all/each 匀discard frameor all/each 匀encodingp\_skipframe。such as/if [ figure1](#fig9874105285115)as shown。

**Figure 1**  discard frame示意 figure<a name="fig9874105285115"></a>  
...
-   switch GOPModeafter discard frame开关will default close，such as/if  need继续use the/this interfacemustre-configure。
...
-   when Sets H.265protocolchannelencodingp\_skipframe when, this  moduleblock within part will closeSAO滤波，故recommendedcustomer call[ss\_mpi\_venc\_set\_h265\_sao](#ZH-CN_TOPIC_0000002441658333)closeSAOfunction。
...
-   this interfaceonlysupportsH.264/H.265/Mjpeg 's CBR/VBRbitratecontrol Mode、H.264/H.265's AVBR/QVBR/CVBRbitratecontrol Mode's parameter's Sets 。
-   VPSS-VENC placein/at wrap-around Low LatencyMode when, not supported exceed/ultralarge frame re- encode， strong/enhance制configurewill causes channel频繁discard frame。SS528V100/SS625V100/SS524V100/SS522V101/Not supported on Hi3403V100/SS626V100.wrap-around Low Latency。
...
-   ifsuper\_frame\_paramis/for  empty/空，then returns Failure。
...
-   onlysupportsGOPModeis/for OT\_VENC\_GOP\_MODE\_NORMAL\_P's encoding channel。
-   GOPswitch after the/this interfaceself-动 losteffect.ifGOPModeis from  non-NORMAL\_P切to NORMAL\_P，Modeswitch after callthe/this interface needwait until GOPtakes effect after the/this interface generateeffect.
...
-   SSE parametermainly by/from three  parameterdetermine 。
    -   idx: system supportseach channel canSets 8  SSE region ，system within part according to 0～7 's index numberSSE region perform management，idx indicates 's user Sets SSE's index number.SSE region between can overlay each other, andwhen occurs overlay when, SSEregion between according to self/own corresponding region report SSE totaland ，互not affect 。
...
-   this interfaceis an advanced interface，system default 八 SSEregion enable，not callthis interface when, default all/each is/for the one macro block region 。user can callthis interfacechange SSEregion ，thereby in/at 水印information in/within resulting in to each  region'sSSE totaland 。
...
-   onlyHi3403V100's H.264encoding channelsupportsthe/this interface，H.265全supports。
...
-   VENCadvanced parametersmainly by/from 六 parameterdetermine 。
    -   color\_to\_grey\_en: enableor closeone channel's 彩转灰function，default is/for close。 When called during encoding,below/under one Iframeencoding takes effect.
    ...
    -   max\_stream\_cnt: used for Sets encoding channel's streambufferin/within  capability enough/sufficientbuffer maximumstreamframe number
        ...
        if bufferstreamframe numberalready  reach/achieveto maximum streamframe number，currentto-be-encodingimage因streambuffer满 and/whilenot is encoding。
    ...
    -   poll\_wake\_up\_frame\_cnt: when channeluse timeoutor blocking Gets stream，encodingspecified 's frameafter 唤醒blocking interface。

        default one frame唤醒once 。in/at encoding during whichSets  when, wait for below/under one frame takes effect.blocking Gets stream when, recommendedpoll\_wake\_up\_frame\_cnt applyless than max\_stream\_cnt。
    ...
    -   frame\_rate：Sets channelframe ratecontrol attribute
        ...
        when Inputframe ratesrc\_frame\_rateless than Outputframe ratedst\_frame\_rate whenis/for  increaseframeMode，so as to dst\_frame\_rate作is/for absoluteOutputframe rate。 examplesuch as/if ，src\_frame\_rate<dst\_frame\_rate，dst\_frame\_rateis/for 30 when, indicates encodingmost 终will Output30frame。

        > **Note:** 
        > increaseframeMode under, mainly  fitused for stableOutputframe rate's scenario ，when Input end断 stream/flow whenmuststop venc，otherwise断 stream/flowafter will repeated  encodemost after one frameso as to  reach/achieveto target frame rate。
        ...
        discard frame（ reduceframe）Mode under, current endis/for fixed 频 rate's  source when,  examplesuch as/if VI，channel's frame ratecontrol then  accurate/standard confirm/ensure；current endis/for not fixed 频 rate's  source when,  examplesuch as/if VDEC，channel's frame ratecontrol then not  accurate/standard confirm/ensure。
        ...
        VPSS-VENC placein/at wrap-around Low Latencyor single-bufferLow LatencyMode when, not supported increaseframeMode， strong/enhance制configurewill causes channel频繁discard frame。SS528V100、SS625V100、SS524V100、SS522V101not supportedVPSS-VENCLow Latency。

        JPEGEencoding channelenableOT\_VENC\_PIC\_RECV\_MULTIMode when, recommendedin/at Input source（ examplesuch as/if vpss）'s one channelperform frame ratecontrol ，encodingwill self-动discard other channelnot mustencoding's image。ifInput sourceperform frame ratecontrol ，not  applyagain/then in/at encoding channelperform frame ratecontrol ，may will causes discard frame。
...
-   This interface sets channelimagebefore scene 保护's configure。
-   This interface must be set after the encoding channel is created and before it is destroyed. When called during encoding, it takes effect on the next frame.
-   It is recommended to call this interface after the channel is created and before encoding starts, to reduce the number of calls during encoding.
-   It is recommended to, before calling this interface,，first call [ss\_mpi\_venc\_get\_fg\_protect](#ZH-CN_TOPIC_0000002408258646)interface，Gets currentencoding channel's before scene 保护's configure，then perform the configuration。
-   VENCadvanced parametersmainly by/from 六 parameterdetermine 。
...
-   This interface gets channelimagebefore scene 保护's configure。
-   This interface must be called after the encoding channel is created and before it is destroyed.
-   It is recommended to call this interface after the channel is created and before encoding starts, to reduce the number of calls during encoding.
...
-   bindVB pooland createencoding channel's operation ，mustin/at the same 进 processin/within 执line 。
...
-   when  requireswitch currentencoding channelbind's VB pool when, only needagain/then  adjustonce interfacess\_mpi\_venc\_attach\_vb\_poolcorrect configuremustbindto 's VB poolis sufficient。
    ...
    2.  framesave Modebelow/under : 
        ...
        interfacewill determine storage Picture's VB poolBlkSize and  BlkCnt's valid - property，if not satisfy  need require，returns Failure，within part 继续use already bind's VB poolperform encoding；if satisfy  need require，returns Success，within part switch to new  poolsub-encoding，dynamic switch VB poolwill 产 generateone Iframe。
...
-   framesave Modebelow/under use UserVBmethod/mode ，callinterfacess\_mpi\_venc\_attach\_vb\_pool whenwithin part will determine storage Picture's VB pool's BlkCnt and  BlkSizewhether 符合this channel's encoding need require（storage Pictureinformation 's VB pool's BlkCnt and  BlkSizewill not  provide/improvebefore determine ）。ifsatisfy  need requirethen interfacereturns Success，storage Picture's VBblock is sufficientis occupied by/uses ；otherwiseinterfacereturns Failure，Error CodeOT\_ERR\_VENC\_SIZE\_NOT\_ENOUGHindicates bind's Picture VB poolBlkSize not  sufficient，Error CodeOT\_ERR\_VENC\_NO\_MEMindicates bind's Picture VB pool BlkCnt not  sufficient。
...
-   callthe/this interface after, not ensure within part already complete VB pooloccupied by/uses 's release，if requireperform destroyVB poolrecommended首first call interface[ss\_mpi\_venc\_reset\_chn](#ZH-CN_TOPIC_0000002408258510)。
...
-   the/this functionmainly by/from as follows九 parameterdetermine 。
    ...
    -   intra32\_cost/intra16\_clost/intra8\_cost/intra4\_cost/inter64\_cost/inter32\_cost/inter16\_cost/inter8\_costrespectively Sets each self-Mode tendencysize 。default each cost valueis/for 8, i.e., not  dotendencyconfigure，each Mode's costSets the more large 意味着select the/this Mode tendencythe more small 。
...
-   The channel must be created; otherwise an error code indicating the channel is not created is returned.
-   this interfaceused for debug channelcu/mbselect SkipMode tendency，supportsbackground scene and before scene  tendencysingle-独configure。
-   This interface must be set after the encoding channel is created and before it is destroyed. When called during encoding, it takes effect on the next frame.
-   It is recommended to call this interface after the channel is created and before encoding starts, to reduce the number of calls during encoding.
...
-   the/this functionmainly by/from as follows五 parameterdetermine 。
...
-   enablethe/this interfaceencoderwill for/to image doprocessing ，has/have one 定's performance 损 lost，recommendeduser  press/by needenable，default is/for close。
...
-   the/this functionmainly by/from as followsthree  parameterdetermine 。
    ...
    -   strength0is/for Sets  removeexcept/divide bybreathing effect strong/enhance degreeadjust parameter0，default  valueis/for 6。strength0 valuethe more large ，breathing effectimprove will the more 弱， valuethe more small ，breathing effectimprove the more  clear/bright显，however in/at encoding compress strength/powervery large  andbitratevery low  when, it will causes imagemore  addblurry.
    -   strength1is/for Sets  removeexcept/divide bybreathing effect strong/enhance degreeadjust parameter1，default  valueis/for 16。its/their  valuethe more large ，Iframewill the more large ，its/their  valuethe more small ，Iframewill the more small 。ifIframe变large for/to customer application has/have affect ，customer can 尝试set strength1 adjustsmall one  point，will reduce Iframestream's size ， however may will  reduce弱breathing effect's improve  process degree。

-    originalthen on/above ，for/to not  same's scenario ，breathing effect's improve  require doto most 佳，mustadjust not  same's parameter。however 考虑to multiple/morescenario 's  fit apply- property，recommendedcustomer use default value.
...
-   the/this functionmainly by/from as followsparameterdetermine 。
    ...
    -   qp\_delta: Sets each one  layerframerelative to the 0 layerpframe's qp\_delta。due to 0 layerpframe's qpnot mustperform  adjust whole/entire，故qp\_delta\[0\]is/for the 1 layer，qp\_delta\[1\]is/for the 2 layer，so as to this by analogy。
...
-    sub- layerQP示意 figuresuch as/if [ figure1](#fig152253434317)and [ figure2](#fig16586145618411)as shown。

**Figure 1**  normal\_p sub- layerQP示意 figure<a name="fig152253434317"></a>  
![](figures/normal_p分层QP示意图.png "normal_p分层QP示意图")

**Figure 2**  smart\_p/adv\_smart\_pp sub- layerQP示意 figure<a name="fig16586145618411"></a>  
...
-   when Sets 's parameterchange statistics information memory size  when, must re-allocate memory or  algorithm may read to has not been is 赋 value's memory ，at this time None论clear\_stats\_after\_set\_attrSets 什么parameterall will clear statistics information 。
...
-   ROIparameter：
    ...
    -   level: specified current's ROIadopt/use 's  reduce/lowerbitrate, etc. level，0\~3provided in orderheight ，, etc. levelthe more height  reduce/lowerbitrate mutualfor/to the more multiple/more，imagequality 损 lostalso  mutualfor/to the more multiple/more。
...
-   This interface gets JPEGand MJPEGencoding channel索引is/for idx's ROIattribute。
-   This interface must be called after the encoding channel is created and before it is destroyed.
-   It is recommended to call this interface after the channel is created and before encoding starts, to reduce the number of calls during encoding.
...
-   supportsrate control Modeis/for CBR、VBR、AVBR、QVBR、CVBRbelow/under 's  smart capabilityencoding。
    ...
    -   AVBRMode under, enableafter for/to bitrate's affect 视scenario changes。
...
-   This interface applies to H.264/H.265 encoding channels.
-   典 typeapplication scenario **Note:**  fitused for _录 image/pixel engine/machine_fixed 's _video capture _scenario 。
...
-   POCType现provides pic\_order\_cnt\_type = 0/1/2togetherthree  typeType，system default pic\_order\_cnt\_typeis/for 0。
...
-   This interface must be called after the encoding channel is created and before it is destroyed.
-   this interfaceonlyfor/to JPEGType's encoding channelvalid/effective  andonlyin/at dering\_modeis/for 1 case/conditionbefore  enough/sufficientcall。
-    strong/enhance边 removeRINGeffect strong/enhance degree, etc. levelhas/have 四 type: 
...
-   createJPEGchannel after, JPEGchanneldefault  placein/at dering\_mode=1's condition/case。when user Must setJPEGencoding channel's  strong/enhance边 removering effect strong/enhance degree, etc. level，can callthis interface，Sets JPEGchannelis/for not  same's  strong/enhance degree, etc. level，thereby resulting in to must's effect。
...
-   典 typeapplication scenario **Note:**  fitused for 录 image/pixel engine/machinefixed 's video capture scenario 。
...
-   with/and ot\_venc\_deblur\_paramin/within adaptivedeblurfunctionmutually exclusive，onlyhas/have closeadaptive开关，before callthe/this interfaceSets advanced deblurparameter。
...
-   ROIparameter：
    -   idx: supportseach channel canSets 16 ROI，encoderaccording to 0～15's index numberROIperform management，idxindicates 's user Sets ROI's index number.ROIbetween can overlay each other, andwhen occurs overlay when, ROIbetween 's priority according to index number0～15provided in orderheight 。
    -   enable: specified current's ROIwhether enable。
    -   fg\_level: ROI region's reduce/lowerbitrate, etc. level，0\~15provided in orderheight ，, etc. levelthe more height  reduce/lowerbitrate mutualfor/to the more multiple/more，imagequality 损 lostalso  mutualfor/to the more multiple/more。
    -   bg\_level:  non-ROI region's reduce/lowerbitrate, etc. level，0\~15provided in orderheight ，, etc. levelthe more height  reduce/lowerbitrate mutualfor/to the more multiple/more，imagequality 损 lostalso  mutualfor/to the more multiple/more。
    -   rect: specified current's ROI's startingCoordinatesand  region'ssize 。ROI's starting pointCoordinatesmustin/at imagerange  within, and must16alignment；ROI region'slong width mustis 16alignmentand greater than 0；ROIregion mustin/at imagerange within 。

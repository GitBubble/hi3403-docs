...
-   解码帧存分配方式
    ...
    -   解码UserVB池：创建解码通道时不分配图像Buffer，而是由用户调用接口[ss\_mpi\_vb\_create\_pool](ss_mpi_vdec_attach_vb_pool.md#ss_mpi_vb_create_pool)创建一个视频缓存VB池，再通过调用接口[ss\_mpi\_vdec\_attach\_vb\_pool](#ZH-CN_TOPIC_0000002408260194)把某个解码通道绑定到固定的视频缓存VB池中。
...
-   调用此接口后，调用发送码流接口[ss\_mpi\_vdec\_send\_stream](#ZH-CN_TOPIC_0000002441699449)会返回[OT\_ERR\_VDEC\_NOT\_PERM](错误码.md#OT_ERR_VDEC_NOT_PERM)。
-   重复调用停止接收码流接口时，返回成功。
...
-   发送数据前必须保证已经调用[ss\_mpi\_vdec\_start\_recv\_stream](#ZH-CN_TOPIC_0000002408100234)接口启动接收码流，否则直接返回该操作不允许的错误码[OT\_ERR\_VDEC\_NOT\_PERM](错误码.md#OT_ERR_VDEC_NOT_PERM)。如果在发送数据过程中停止接收码流，就会立刻返回[OT\_ERR\_VDEC\_NOT\_PERM](错误码.md#OT_ERR_VDEC_NOT_PERM)。
...
-   各项参数如超过合法范围，会返回[OT\_ERR\_VDEC\_ILLEGAL\_PARAM](错误码.md#OT_ERR_VDEC_ILLEGAL_PARAM)的错误码，各参数范围见数据类型[ot\_vdec\_chn\_param](#ZH-CN_TOPIC_0000002441659561)的说明。
...
-   设置用户图片前必须先调用接口[ss\_mpi\_vdec\_disable\_user\_pic](#ZH-CN_TOPIC_0000002441699305)禁止使能用户图片，否则返回错误码[OT\_ERR\_VDEC\_NOT\_PERM](错误码.md#OT_ERR_VDEC_NOT_PERM)。

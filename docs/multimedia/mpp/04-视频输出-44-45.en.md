...
-  GatheredMethod: 
  ...
  -  After enabling the tiled display method, for video layers in MULTI mode, you can call the channel display position interface (ss\_mpi\_vo\_set\_chn\_display\_pos) to combine tiled layout with custom channel display positions.
...
-  On SS528V100/SS625V100/SS524V100/SS522V101, video layer VHD0 in SINGLE mode supports scaling: when img_size is smaller than display_rect, the image is scaled up from img_size to display_rect for display; when img_size is larger than display_rect, the intersection of img_size and display_rect is displayed. VHD1 and VHD2 do not support scaling; if img_size exceeds display_rect, the image is cropped for display.
...
-  When the WBC writeback mode is set to OT_VO_WBC_MODE_PROGRESSIVE_TO_INTERLACED and the HD device is displaying at 1080P@60fps, VI actually captures at 30fps. WBC converts the 60-frame progressive display image to 30-frame interlaced images, allowing the writeback image to achieve a relatively good display effect on a monitor. Therefore, when the HD device displays at 1080P@30fps, the maximum WBC writeback frame rate in this mode is 15fps, and frame-rate control settings are invalid. In this mode, the actual WBC writeback frame rate depends on the following scenarios: 

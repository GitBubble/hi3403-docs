---
title: "视频输入"
source: /sessions/sharp-sweet-allen/mnt/hi3403-build/pegasus/docs/zh-CN/MPP 媒体处理软件 V5.0 开发参考/03 视频输入.md
---
...
-   视频输入设备
    ...
    视频输入设备Supported若干种时序输入，负责对时序进行解析。
...
-   视频Physical channel
    ...
    Physical channel负责将最终处理后的数据输出到DDR，在真正将数据输出到DDR之前，它可以实现裁剪等功能。

-   PIPE的工作模式

    See“System Control”章节的“VI和VPSS”的工作模式描述。
...
-   DIS
    ...
    DIS模块通过比较当前图像与前两帧图像采用不同自由度的防抖算法, calculate 当前图像在各个轴方向上的抖动偏移向量，然后根据抖动偏移向量对当前图像进行校正，从而起到防抖的效果。

-   BAS

    Bayer scaling，, i.e., Bayer域缩放。

-   提前上报中断

    提前上报中断指图像写出指定的行数到DDR后，VIReport an interrupt，把图像发给后端模块处理，可以减少延时，但没有和Low Latency一样的硬件机制保证后端模块读图像不会出错。
...
**Figure 1**  VI在软件层次划分的4个部分<a name="fig2055755551718"></a>  
![](figures/VI在软件层次划分的4个部分.png "VI在软件层次划分的4个部分")

VI从软件上划分了输入设备（DEV），输入PIPE\(图示 as 物理PIPE，虚拟PIPE只包含ISP\_BE\)、Physical channel（PHY\_CHN）、Extension channel（EXT\_CHN）四个层级。各芯片的设备、PIPE、通道个数差异如[表1](#_Ref510970631)。

**Table 1**  设备/PIPE/channel个数
...
-   SS928V100视频输入通道功能As shown in [Figure 2](#fig12122195920190)。

**Figure 2**  SS928V100 VI通道功能框图<a name="fig12122195920190"></a>  
![](figures/SS928V100-VI通道功能框图.png "SS928V100-VI通道功能框图")
### 视频输入设备<a name="ZH-CN_TOPIC_0000002441657925"></a>

有VI设备都是相互独立的，Supported时序解析。

### 视频输入PIPE<a name="ZH-CN_TOPIC_0000002441658365"></a>

VI的PIPE包含了ISP的相关处理功能，主要是对图像数据进行流水线处理，输出YUV图像格式给通道。PIPE的工作模式See“System Control”章节的“VI和VPSS”的工作模式描述。

### 视频Physical channel<a name="ZH-CN_TOPIC_0000002441658401"></a>

-   SS928V100 VI只有一个Physical channel，Supported8个Extension channel。
-   SS928V100Physical channelSupported的典型分辨率如3840x2160@60fps、3840x2160@30fps、1080p@240fps、1080p@120fps、1080p@60fps、1080p@30fps等。

### 视频Extension channel<a name="ZH-CN_TOPIC_0000002441698193"></a>

Extension channel是物理channel扩展，Extension channel具备缩放、裁剪功能，它通过绑定Physical channel，将Physical channel输出作 as 自己的输入，然后输出用户设置的目标图像。

### BAS功能区分说明<a name="ZH-CN_TOPIC_0000002408098810"></a>

SS928V100有两个子模块SupportedBAS功能，如[表1](#_Ref48133205)。

**Table 1**  子模块具体功能描述

<a name="_Ref48133205"></a>
<table><thead align="left"><tr id="row12538mcpsimp"><th class="cellrowborder" valign="top" width="35%" id="mcps1.2.3.1.1"><p id="p12540mcpsimp"><a name="p12540mcpsimp"></a><a name="p12540mcpsimp"></a>功能属子模块</p>
</th>
...
<td class="cellrowborder" valign="top" width="65%" headers="mcps1.2.3.1.2 "><p id="p12547mcpsimp"><a name="p12547mcpsimp"></a><a name="p12547mcpsimp"></a>Supported1、1/2、1/3的缩放和相位调整。</p>
</td>
</tr>
...
<td class="cellrowborder" valign="top" width="65%" headers="mcps1.2.3.1.2 "><p id="p12552mcpsimp"><a name="p12552mcpsimp"></a><a name="p12552mcpsimp"></a>只Supported1/2的缩放，用户可以通过设置dump属性来获取bas帧。</p>
</td>
</tr>
</tbody>
</table>
...
-   SS928V100 DEV和时序输入接口的约束关系如[表1](#_Ref510971707)。

**Table 1**  SS928V100 DEV与MIPI/BT.1120/BT.656/BT.601/DC接口的绑定关系
...
“x”表示invalid设备号，数字表示有效的设备ID。

-   DEV和PIPE的绑定关系：每个PIPE都可以与任意Dev绑定，PIPE销毁后，PIPE可以和DEV解绑定。

### 从模式<a name="ZH-CN_TOPIC_0000002441697853"></a>

从模式SENSOR，需要使用VI的从模式模块。从模式与VI的物理PIPE对应关系是固定的。用户需要根据SENSOR管脚的连线和[表1](#_Ref486405463)确定使用哪个从模式模块，然后选择corresponding 物理PIPE号创建物理PIPE，否则会没有数据，详细步骤as follows：

1.  根据硬件原理图确认SENSOR管脚连接到了SENSOR\_HSx/ SENSOR\_VSx。
2.  根据[表1](#_Ref486405463)确定该SENSOR连接到从模式模块x。
3.  软件根据[表1](#_Ref486405463)确定在采集这个SENSOR的数据时可以使用编号 as x的PIPE。

    **Table 1**  从模式与PIPE的对应关系表
    ...
    >从模式和PIPE的对应关系默认是如[表1](#_Ref486405463)，如果需要修改，可以通过修改ISP相关的代码完成。

### 掩码Configure <a name="ZH-CN_TOPIC_0000002441698117"></a>

掩码的高12bit对应着硬件线路的12个pin脚连接（D0到D15之间的任意连续12个pin脚, i.e., 可，例如D4～D15），用户需要根据实际连接情况设置恰当的掩码Configure ，掩码的最高比特位corresponding pin as D15，例如10bit输入的Sensor连接的pin as D6\~D15，掩码configured as 0xFFC00000；同理如果是14bit输入时，corresponding 掩码configured as 0xFFFC0000。

VI接入Data线序 as 由低到高，例如单分量接入时，D0 as 数据低比特位，D15 as 数据高比特位。

-   1路/2路5M或1080p图像输入场景（12bit输入）

    1路/2路5M或1080p图像输入场景下，设置VI设备属性时，可根据[表1](#_Ref331669300)Configure 掩码。

**Table 1**  1路、2路5M或1080p场景下的掩码Configure （12bit）
...
-   1路/2路BT.1120高清输入场景（16bit输入）
    ...
    1路/2路BT.1120高清图像输入场景下，设置VI设备属性时，可根据[表2](#_Ref391628558)Configure 掩码。

**Table 2**  1路/2路BT.1120图像输入场景下的掩码Configure （16bit）
...
-   1路/2路D1图像输入场景（8bit输入）
    ...
    1路/2路 图像输入场景下，设置VI设备属性时，可根据[表3](#_Ref331669304)Configure 掩码。

**Table 3**  1路D1图像输入场景下的掩码Configure （8bit）
...
VI模块实现DevConfigure 和Enabled、Dev和Pipe绑定、GrpConfigure 、Pipe创建和Enabled、ChnConfigure 和Enabled等功能。
...
-   [ss\_mpi\_vi\_set\_thermo\_sns\_attr](#ZH-CN_TOPIC_0000002408258594)：设置热成像探测器的Configure 属性。
-   [ss\_mpi\_vi\_get\_thermo\_sns\_attr](#ZH-CN_TOPIC_0000002441697765)：获取热成像探测器的Configure 属性。
...
-   [ss\_mpi\_vi\_send\_dev\_frame](#ZH-CN_TOPIC_0000002408258974)：Configure 设备送帧的帧信息。
...
-   [ss\_mpi\_vi\_set\_pipe\_low\_delay\_attr](#ZH-CN_TOPIC_0000002408258530)：设置VI PIPELow Latency属性。
-   [ss\_mpi\_vi\_get\_pipe\_low\_delay\_attr](#ZH-CN_TOPIC_0000002408258458)：获取VI PIPELow Latency属性。
...
-   [ss\_mpi\_vi\_set\_pipe\_fisheye\_cfg](#ZH-CN_TOPIC_0000002408098862)：设置VI PIPEcorresponding 鱼眼镜头LMF参数Configure 。
-   [ss\_mpi\_vi\_get\_pipe\_fisheye\_cfg](#ZH-CN_TOPIC_0000002441658345)：获取VI PIPEcorresponding 鱼眼镜头LMF参数Configure 。
-   [ss\_mpi\_vi\_get\_pipe\_compress\_param](#ZH-CN_TOPIC_0000002408098426)：获取VI物理 PIPE的RAW压缩参数。
-   [ss\_mpi\_vi\_set\_user\_pic](#ZH-CN_TOPIC_0000002441698165)：设置用户图片，作 as None视频信号时的插入图片。
...
-   [ss\_mpi\_vi\_pipe\_attach\_vb\_pool](#ZH-CN_TOPIC_0000002441658197)：将VI的PIPEbind to某个视频缓存VB池中。
...
-   [ss\_mpi\_vi\_set\_chn\_attr](#ZH-CN_TOPIC_0000002408099002)：设置VIChannel attribute。
-   [ss\_mpi\_vi\_get\_chn\_attr](#ZH-CN_TOPIC_0000002441697789)：获取VIChannel attribute。
-   [ss\_mpi\_vi\_set\_ext\_chn\_attr](#ZH-CN_TOPIC_0000002408098582)：设置VIExtension channel属性。
-   [ss\_mpi\_vi\_get\_ext\_chn\_attr](#ZH-CN_TOPIC_0000002408098454)：获取VIExtension channel属性。
...
-   [ss\_mpi\_vi\_ldc\_pos\_query\_dst\_to\_src](#ZH-CN_TOPIC_0000002408258738)：根据镜头畸变校正（LDC）的输出图像Coordinate point查找输入图像的Coordinate point。
-   [ss\_mpi\_vi\_ldc\_pos\_query\_src\_to\_dst](#ZH-CN_TOPIC_0000002408098876)：根据镜头畸变校正（LDC）的输入图像Coordinate point查找输出图像的Coordinate point。
...
-   [ss\_mpi\_vi\_set\_chn\_fisheye](#ZH-CN_TOPIC_0000002441657997)：设置VI通道corresponding 鱼眼属性。
-   [ss\_mpi\_vi\_get\_chn\_fisheye](#ZH-CN_TOPIC_0000002441697773)：获取VI通道corresponding 鱼眼属性。
-   [ss\_mpi\_vi\_fisheye\_pos\_query\_dst\_to\_src](#ZH-CN_TOPIC_0000002408258834)：根据鱼眼校正输出图像Coordinate point查找源图像Coordinate point。
-   [ss\_mpi\_vi\_get\_chn\_rgn\_luma](#ZH-CN_TOPIC_0000002441697453)：获取指定图像区域的亮度总和。
-   [ss\_mpi\_vi\_set\_chn\_dis\_cfg](#ZH-CN_TOPIC_0000002408258606)：设置VIchannelDISConfigure 信息。
-   [ss\_mpi\_vi\_get\_chn\_dis\_cfg](#ZH-CN_TOPIC_0000002408098686)：获取VIchannelDISConfigure 信息。
-   [ss\_mpi\_vi\_set\_chn\_dis\_attr](#ZH-CN_TOPIC_0000002408098286)：设置VIchannelDIS属性。
-   [ss\_mpi\_vi\_get\_chn\_dis\_attr](#ZH-CN_TOPIC_0000002441657637)：获取VIchannelDIS属性。
-   [ss\_mpi\_vi\_set\_chn\_dis\_param](#ZH-CN_TOPIC_0000002441697485)：设置VIchannelDIS可选参数。
-   [ss\_mpi\_vi\_get\_chn\_dis\_param](#ZH-CN_TOPIC_0000002408258474)：获取VIchannelDIS的可选参数。
-   [ss\_mpi\_vi\_set\_chn\_dis\_wdr\_attr](#ZH-CN_TOPIC_0000002468735433)：设置VIchannelDIS WDR属性。
-   [ss\_mpi\_vi\_get\_chn\_dis\_wdr\_attr](#ZH-CN_TOPIC_0000002435176804)：获取VIchannelDIS WDR属性。
-   [ss\_mpi\_vi\_set\_chn\_fov\_correction\_attr](#ZH-CN_TOPIC_0000002408098698)：设置VIchannel视场角矫正属性。
-   [ss\_mpi\_vi\_get\_chn\_fov\_correction\_attr](#ZH-CN_TOPIC_0000002441697585)：获取VIchannel视场角矫正属性。
...
-   [ss\_mpi\_vi\_set\_chn\_low\_delay\_attr](#ZH-CN_TOPIC_0000002408098478)：设置VI通道Low Latency属性。
-   [ss\_mpi\_vi\_get\_chn\_low\_delay\_attr](#ZH-CN_TOPIC_0000002408098418)：获取VI通道Low Latency属性。
...
-   [ss\_mpi\_vi\_chn\_attach\_vb\_pool](#ZH-CN_TOPIC_0000002441658173)：将VI通道bind to某个视频缓存VB池中。
-   [ss\_mpi\_vi\_chn\_detach\_vb\_pool](#ZH-CN_TOPIC_0000002441658129)：将VI通道从某个视频缓存VB池中解绑定。
-   [ss\_mpi\_vi\_query\_chn\_status](#ZH-CN_TOPIC_0000002441697645)：查询VIchannel状态。
...
-   [ss\_mpi\_vi\_set\_mod\_param](#ZH-CN_TOPIC_0000002441658357)：设置VIModule Parameter。
-   [ss\_mpi\_vi\_get\_mod\_param](#ZH-CN_TOPIC_0000002441657753)：获取VIModule Parameter。
...
设置VI设备属性。基本设备属性默认了部分芯片Configure ，满足绝大部分的sensor对接要求。
...
-   Not supportedBT.1120隔行输入。
-   在调用前要保证VI设备处于禁用状态。如果VI设备已处于Enabled状态，可以使用[ss\_mpi\_vi\_disable\_dev](#ZH-CN_TOPIC_0000002441658061)来禁用设备。
-   参数dev\_attr主要用来Configure 指定VI设备的视频接口模式，用于与外围camera、sensor或codec对接，Supported的接口模式包括MIPI Rx（MIPI/LVDS/HISPI）、SLVS-EC。用户需要Configure 以下几类信息，具体属性意义参见“[数据类型](#ZH-CN_TOPIC_0000002408258714)”部分的Note:
    -   接口模式信息：接口mode isMIPI Rx（MIPI/LVDS/HISPI）等模式
    -   工作模式信息：1路、2路、4路复合模式
    -   数据布局信息：复合模式下多路数据的排布
    -   数据信息：逐行输入、YUV数据输入顺序
    -   同步时序信息：垂直、水平同步信号的属性
...
如果未设置VI设备属性，this interface将returns failure。
...
-   wdr模式下Not supportedbas功能。
-   this interface在[ss\_mpi\_vi\_enable\_dev](#ZH-CN_TOPIC_0000002408098518)之前Configure ，在[ss\_mpi\_vi\_set\_dev\_attr](#ZH-CN_TOPIC_0000002441697677)之后Configure 。
...
-   启用前必须已经设置设备属性，否则returns failure。
-   可重复启用，不returns failure。
...
-   需先销毁有与该VI设备绑定的物理PIPE后，再禁用VI设备。
-   可重复禁用，不returns failure。
-   Supported低功耗处理，禁用VI设备后将完全关闭该设备，需要重新设置属性，才能EnabledVI设备。
...
<td class="cellrowborder" valign="top" width="68%" headers="mcps1.1.4.1.2 "><p id="p1178mcpsimp"><a name="p1178mcpsimp"></a><a name="p1178mcpsimp"></a>热成像探测器的Configure 属性。</p>
</td>
...
<th class="cellrowborder" valign="top" width="56.99999999999999%" id="mcps1.1.3.1.2"><p id="p1215mcpsimp"><a name="p1215mcpsimp"></a><a name="p1215mcpsimp"></a>Supported的Dev ID</p>
</th>
</tr>
</thead>
...
<td class="cellrowborder" valign="top" width="56.99999999999999%" headers="mcps1.1.3.1.2 "><p id="p1221mcpsimp"><a name="p1221mcpsimp"></a><a name="p1221mcpsimp"></a>只有Dev1Supported热成像。</p>
</td>
</tr>
</tbody>
</table>
...
调用前必须已经Enabled设备，否则returns failure。
...
<td class="cellrowborder" valign="top" width="69%" headers="mcps1.1.4.1.2 "><p id="p1269mcpsimp"><a name="p1269mcpsimp"></a><a name="p1269mcpsimp"></a>热成像探测器的Configure 属性。</p>
</td>
...
调用前必须first call 了[ss\_mpi\_vi\_set\_thermo\_sns\_attr](#ZH-CN_TOPIC_0000002408258594)接口。
...
-   必须已经启用设备，才能Configure this interface送帧，否则会返回错误。
...
<p id="ot_video_frame_info"><a name="ot_video_frame_info"></a><a name="ot_video_frame_info"></a>ot_video_frame_info具体See“System Control”章节。</p>
</td>
...
<p id="p1488mcpsimp"><a name="p1488mcpsimp"></a><a name="p1488mcpsimp"></a>大于0表示超时模式，Timeout的unit is milliseconds (ms)。</p>
</td>
...
使用本接口时，需first call [ss\_mpi\_vi\_enable\_dev\_send\_frame](#ZH-CN_TOPIC_0000002441697685)Enabled送帧。
...
<td class="cellrowborder" valign="top" width="62%" headers="mcps1.1.4.1.2 "><p xml:lang="sv-SE" id="p1565mcpsimp"><a name="p1565mcpsimp"></a><a name="p1565mcpsimp"></a><span xml:lang="en-US" id="ph1566mcpsimp"><a name="ph1566mcpsimp"></a><a name="ph1566mcpsimp"></a>See</span>ot_vi_dev_timing_attr<span xml:lang="en-US" id="ph1568mcpsimp"><a name="ph1568mcpsimp"></a><a name="ph1568mcpsimp"></a>说明</span></p>
</td>
...
-   使用本接口前，需先Configure DEV属性，并Enabled设备，否则returns failure。
-   使用自产生时序功能灌RAW时，需Configure DEV的宽高与RAW文件的宽高保持一致。
-   Enabled自产生时序后，若不灌RAW，则None图像显示。
-   Enabled自产生时序后，从DEV灌RAW后VI输出帧率由Configure 自产生时序产生的有效帧率决定。
-   DEVConfigure X2速率时，Enabled自产生时序会导致时序异常。
...
-   必须先EnabledVI设备后才能绑定物理PIPE。
-   Not supported动态绑定。
-   Not supported绑定已经创建的PIPE。
...
必须先销毁物理PIPE再解绑定，否则returns failure。
...
<td class="cellrowborder" valign="top" width="61%" headers="mcps1.1.4.1.2 "><p id="p2110mcpsimp"><a name="p2110mcpsimp"></a><a name="p2110mcpsimp"></a>绑定PIPE的Device number.</p>
...
-   使用本接口，需first call DEV和PIPE绑定接口，且grp\_attr中的pipe id都必须是与同一个Dev绑定的。
-   本接口需要在创建pipe前调用。
-   本接口Supported更新grp\_attr的pipe\_id，如WDR模式和线性的切换，但Not supported同一个pipe\_idbind to两个grp\_attr。
-   WDR合成组的pipe\_id不能重复。
-   WDR模式下主pipedefaults to pipe id最小的pipe，例如，wdr合成组pipe id as 2、1、3，主pipe as pipe1；若Configure pipe\_reverse as TRUE，则主pipe as pipe\_id最大的pipe，, i.e., pipe3。
-   pipe\_reverse as TRUE仅Supported行模式WDR以及VIOffline mode。
...
<td class="cellrowborder" valign="top" width="69%" headers="mcps1.1.4.1.2 "><p id="p2344mcpsimp"><a name="p2344mcpsimp"></a><a name="p2344mcpsimp"></a>PIPE的Attribute Structure体指针。</p>
...
-   Not supported重复创建。
-   当VI离线且输入图像大于4096时，Compression not supported。
-   WDR模式下需要创建多个物理PIPEbind to同一个开了WDR的on the device，当进行切换时，需要把有bind to该设备的物理PIPE销毁再重建。不能使用上次使用过而未销毁的物理PIPE，否则可能造成错误。
...
-   使用本接口前，需first call [ss\_mpi\_vi\_stop\_pipe](#ZH-CN_TOPIC_0000002408099054)\(vi\_pipe\)停止PIPE，否则提示Failure.
...
<td class="cellrowborder" valign="top" width="69%" headers="mcps1.1.4.1.2 "><p id="p2495mcpsimp"><a name="p2495mcpsimp"></a><a name="p2495mcpsimp"></a>PIPE的Attribute Structure体指针。</p>
...
-   PIPE属性必须合法，where 部分Static Attribute不可动态设置，具体see [ot\_vi\_pipe\_attr](#ZH-CN_TOPIC_0000002441657721)。
-   当configured as OT\_VI\_PIPE\_BYPASS\_BE时，VI 通道类功能（如mirror/flip）不生效。
-   当Configure OT\_COMPRESS\_MODE\_FRAME时，PIPELow Latency、提前上报属性等功能不能使用。
...
<td class="cellrowborder" valign="top" width="62%" headers="mcps1.1.4.1.2 "><p id="p2580mcpsimp"><a name="p2580mcpsimp"></a><a name="p2580mcpsimp"></a>PIPE的Attribute Structure体指针。</p>
</td>
...
<p id="ot_crop_info"><a name="ot_crop_info"></a><a name="ot_crop_info"></a>ot_crop_info请查看“System Control”章节。</p>
</td>
...
-   本接口需在Start Pipe前Configure ，启动PIPE后，不可调用。
-   crop\_info.rect.x+width需大于等于pipe\_attr.size.width，且需小于等于BAS输出的宽度，高度同理。
-   Crop属性的x/y需2对齐，宽高需4对齐。
-   Configure pre\_crop后，需Configure ISP的PUB属性小于或者等于PreCrop的宽高。
...
<p id="p2954mcpsimp"><a name="p2954mcpsimp"></a><a name="p2954mcpsimp"></a>ot_crop_info请查看“System Control”章节。</p>
</td>
...
-   PIPE must be created, otherwise failure is returned。
-   VI离线VPSS离线、VI离线VPSS在线时，Not supported本接口。
-   Crop\_info.rect.x+width需小于等于pipe\_attr.size.width，且裁剪宽度需要大于等于32；高度同理。
-   Supported动态Configure pipe post crop属性。
-   Crop属性的x/y需2对齐，宽高需4对齐。
-   Virtual PIPE does not support this。
...
<p id="p3279mcpsimp"><a name="p3279mcpsimp"></a><a name="p3279mcpsimp"></a>大于0表示超时模式，Timeout的unit is milliseconds (ms)。</p>
</td>
...
-   PIPE must have been created。
-   调用[ss\_mpi\_vi\_set\_pipe\_frame\_dump\_attr](#ZH-CN_TOPIC_0000002408098630)设置dump属性、Enableddump、设置depth，否则获取不到帧数据。
-   Virtual PIPE does not support this。
...
-   PIPE must have been created。
-   FE输出帧是 FE写出的原始帧，VIOnline ModeNot supportedthis interface。
-   需要dump裁剪后的图像，可以在dump前设置pipe的裁剪。
-   需要dump压缩或者非压图像，可以在dump前设置pipe的属性以改变压缩类型。
-   Virtual PIPE does not support this。
...
-   PIPE must have been created。
-   Virtual PIPE does not support this。
-   VIOnline ModeNot supported调用本接口。
...
-   PIPE must have been created。
-   调用[ss\_mpi\_vi\_set\_pipe\_fe\_out\_frame\_dump\_attr](#ZH-CN_TOPIC_0000002441657853)设置dump属性、Enableddump、设置depth，否则获取不到帧数据。
...
<p id="p3603mcpsimp"><a name="p3603mcpsimp"></a><a name="p3603mcpsimp"></a>大于0表示超时模式，Timeout的unit is milliseconds (ms)。</p>
</td>
...
-   PIPE must have been created。
-   调用[ss\_mpi\_vi\_set\_pipe\_private\_data\_dump\_attr](#ZH-CN_TOPIC_0000002441658077)设置dump属性、Enableddump、设置depth，否则获取不到私有数据。
-   本接口Not supported使用select模式获取帧数据。
-   Virtual PIPE does not support this。
...
<p id="p3927mcpsimp"><a name="p3927mcpsimp"></a><a name="p3927mcpsimp"></a>大于0表示超时模式，Timeout的unit is milliseconds (ms)。</p>
</td>
...
-   PIPE must have been created。
-   调用[ss\_mpi\_vi\_set\_pipe\_bas\_frame\_dump\_attr](#ZH-CN_TOPIC_0000002441697701)设置dump属性、Enableddump、设置depth，否则获取不到raw数据。
-   本接口Not supported使用select模式获取帧数据。
-   本接口获取的图像帧Pixel formatfixed at raw12格式，不能修改。
-   Virtual PIPE does not support this。
...
<td class="cellrowborder" valign="top" width="62%" headers="mcps1.1.4.1.2 "><p id="p4233mcpsimp"><a name="p4233mcpsimp"></a><a name="p4233mcpsimp"></a>VI PIPE参数，See结构体定义。</p>
</td>
...
-   PIPE must have been created。
-   请务必first call [ss\_mpi\_vi\_get\_pipe\_param](#ZH-CN_TOPIC_0000002441697837)get current 参数，再进行设置操作。
...
<td class="cellrowborder" valign="top" width="62%" headers="mcps1.1.4.1.2 "><p id="p4311mcpsimp"><a name="p4311mcpsimp"></a><a name="p4311mcpsimp"></a>VI PIPE参数，See结构体定义。</p>
</td>
...
-   只SupportedVIOffline mode。
-   启用本接口之前，需要先设置pipe\_param 输出mode isstagger模式。
-   PIPE must have been created，且must be at start pipe前调用this interface。
-   调用本接口后，通过[ss\_mpi\_vi\_get\_pipe\_frame](#ZH-CN_TOPIC_0000002441658209)获取到的帧数据是stagger完整帧中的第一帧数据；Enabled本in mode，可使用[ss\_mpi\_vi\_get\_pipe\_fe\_out\_frame](#ZH-CN_TOPIC_0000002408099042)获取原始的完整帧数据。
...
-   只SupportedVIOffline mode。
-   调用本接口之前，需要先设置pipe\_param 输出mode isstagger模式。
-   PIPE must have been created，且must be at start pipe前调用this interface。
...
-   must be at pipe创建前使用，pipe创建后不可更改。
-   本接口默认可不调用，buffer个数Default value as  40。
-   VPSS在线时不需要buffer个数。
...
<td class="cellrowborder" valign="top" width="62%" headers="mcps1.1.4.1.2 "><p id="p4385mcpsimp"><a name="p4385mcpsimp"></a><a name="p4385mcpsimp"></a>VI帧信息结构体。具体描述see “System Control”章节。</p>
</td>
...
<p id="p4395mcpsimp"><a name="p4395mcpsimp"></a><a name="p4395mcpsimp"></a>大于0表示超时模式，Timeout的unit is milliseconds (ms)。</p>
</td>
...
-   PIPE must have been created且启动。
-   送YUV首先需要通过[ss\_mpi\_vi\_set\_pipe\_frame\_source](#ZH-CN_TOPIC_0000002441658049)来设置PIPE的数据来源 as OT\_VI\_PIPE\_FRAME\_SOURCE\_USER。
-   YUV数据的宽高必须与PIPE属性的size的宽高保持一致。
-   送下来的YUV帧信息must be 真实有效的VB。
-   YUV数据Pixel format必须与PIPE属性的Pixel format保持一致
-   YUV数据fieldmust be OT\_VIDEO\_FIELD\_FRAME。
-   YUV数据video\_formatmust be OT\_VIDEO\_FORMAT\_LINEAR。
-   YUV数据compress\_modemust be 非压OT\_COMPRESS\_MODE\_NONE。
-   YUV数据动态范围dynamic\_rangemust be OT\_DYNAMIC\_RANGE\_SDR8。YUV数据色域must be 合法值\[OT\_COLOR\_GAMUT\_BT601，OT\_COLOR\_GAMUT\_BUTT\)。
-   WDR模式Not supported本接口。
-   从BE送YUV时，来自SENSOR的数据不会送给BE处理，BE只会处理用户送下来的帧数据。
-   VI在线VPSSOnline Mode、VI在线VPSSOffline modeNot supported本接口。
...
<p id="p4498mcpsimp"><a name="p4498mcpsimp"></a><a name="p4498mcpsimp"></a>大于0表示超时模式，Timeout的unit is milliseconds (ms)。</p>
</td>
...
-   PIPE must have been created且启动。
-   vi\_pipemust be wdr融合的主pipe号。
-   送RAW首先需要通过[ss\_mpi\_vi\_set\_pipe\_frame\_source](#ZH-CN_TOPIC_0000002441658049)来设置PIPE的数据来源 as OT\_VI\_PIPE\_FRAME\_SOURCE\_USER。
-   送一帧处理一帧，不会循环处理。
-   WDR的有PIPE as Offline mode，每个PIPE的RAW存放的顺序必须是短中长帧的顺序，目的是保证长短曝光帧的顺序正确，每个PIPE的RAW的属性\(宽高，Pixel format，压缩等等\)要保持一致。
-   PIPE属性的IspBypassmust be TD\_FALSE。
-   送下来的RAW帧信息must be 真实有效的VB。
-   RAW数据Pixel format必须与PIPE属性的Pixel format保持一致。
-   RAW数据fieldmust be OT\_VIDEO\_FIELD\_FRAME。
-   RAW数据video\_formatmust be OT\_VIDEO\_FORMAT\_LINEAR。
-   RAW数据compress\_modemust be OT\_COMPRESS\_MODE\_NONE、OT\_COMPRESS\_MODE\_LINE、OT\_COMPRESS\_MODE\_FRAME。
-   RAW数据动态范围dynamic\_rangemust be OT\_DYNAMIC\_RANGE\_SDR8。RAW数据色域must be 合法值\[OT\_COLOR\_GAMUT\_BT601，OT\_COLOR\_GAMUT\_BUTT\)。
-   VI在线VPSSOnline Mode、VI在线VPSSOffline modeNot supported本接口。
...
-   当PIPE处于VI在线VPSSOnline Mode，VI在线VPSSOffline mode，Not supported使用本接口。
-   PIPE must have been created。
-   Virtual PIPE does not support this。
...
-   PIPE must have been created。
-   当PIPE处于VI在线，VPSSOnline Mode，Not supportedthis interface。
-   Virtual PIPE does not support this。
...
-   must be at PIPE创建后，Enabled之前调用。
-   Virtual PIPE does not support this。
...
<p id="ot_low_delay_info"><a name="ot_low_delay_info"></a><a name="ot_low_delay_info"></a>ot_low_delay_info具体See“System Control”章节。</p>
</td>
...
<td class="cellrowborder" valign="top" width="61%" headers="mcps1.1.4.1.2 "><p id="p4945mcpsimp"><a name="p4945mcpsimp"></a><a name="p4945mcpsimp"></a>Low Latency功能参数结构体。</p>
...
-   PIPE must have been created。
-   只有VIOffline mode下Supported该功能。
-   PIPELow Latency与PIPE帧压缩冲突，不能同时使用。
-   PIPELow Latency没有one\_buf功能，one\_buf\_enConfigure 不生效。
-   离线WDR模式下需保证多路pipe同时打开Low Latency开关，行号Configure 一致，否则会出现错帧融合的现象。
-   离线多路时，开启多路PIPELow Latency会影响系统性能，不建议开启多路PIPELow Latency。
...
<td class="cellrowborder" valign="top" width="60%" headers="mcps1.1.4.1.2 "><p id="p5024mcpsimp"><a name="p5024mcpsimp"></a><a name="p5024mcpsimp"></a>Low Latency功能参数结构体。</p>
</td>
...
<td class="cellrowborder" valign="top" width="60.4%" headers="mcps1.1.4.1.2 "><p id="ot_frame_interrupt_attr"><a name="ot_frame_interrupt_attr"></a><a name="ot_frame_interrupt_attr"></a>中断Attribute Structure体指针。<span xml:lang="sv-SE" id="ph5100mcpsimp"><a name="ph5100mcpsimp"></a><a name="ph5100mcpsimp"></a>ot_frame_interrupt_attr</span>详情请查看“System Control”章节。</p>
...
-   PIPE必须已经创建。
-   本接口must be at START PIPE前调用。
-   仅SupportedOT\_FRAME\_INTERRUPT\_START、OT\_FRAME\_INTERRUPT\_EARLY和OT\_FRAME\_INTERRUPT\_EARLY\_END三种类型。
-   early\_line用于设置VI硬件采集多少行后上报延时中断。
-   调用本接口用于节省内存占用：
    -   使用STARTin mode，VI驱动正常采集，没有省内存效果；
    -   使用EARLYin mode，可通过调大early\_line节省buffer占用时长，达到省内存的效果；
    -   early\_line设置越大，省一帧内存的概率越大，但early\_line调至接近高度时，可能会出现Configure 跨帧导致的效果异常或者丢帧问题，建议early\_lineconfigured as **采集高度-100**；同时，early模式下VI采集中断响应数会增加一倍左右；
    -   使用EARLY\_ENDin mode，Configure early\_line的效果同early模式；同时VI驱动会在采集END中断提前将帧信息送出，相比early模式更省内存，且减少了时序后消隐区的延时；early\_end模式下VI采集中断响应数会增加两倍左右。
    -   单路采集时建议使用early模式+Low Latency模式，替代early\_end模式，可起到, i.e., 省内存，又更多的降低通路延时的作用。

-   在线WDR模式下，离线pipeNot supported使用本接口。
-   离线WDR模式下Configure early\_end模式，由于改变了组帧时机，需保证多路pipe同时configured as early\_end，否则可能会出现错帧融合的现象。
...
<td class="cellrowborder" valign="top" width="60.4%" headers="mcps1.1.4.1.2 "><p id="p5182mcpsimp"><a name="p5182mcpsimp"></a><a name="p5182mcpsimp"></a>中断Attribute Structure体指针。<span xml:lang="sv-SE" id="ph5183mcpsimp"><a name="ph5183mcpsimp"></a><a name="ph5183mcpsimp"></a>ot_frame_interrupt_attr</span>详情请查看“System Control”章节。</p>
</td>
...
-   PIPE must have been created。
-   defaults to OT\_FRAME\_INTERRUPT\_START模式，延时行号 as 0；pipe销毁后会将中断属性重置 as 默认状态。
...
设置VI PIPEcorresponding 鱼眼镜头LMF参数Configure 。
...
<td class="cellrowborder" valign="top" width="59%" headers="mcps1.1.4.1.2 "><p id="p5260mcpsimp"><a name="p5260mcpsimp"></a><a name="p5260mcpsimp"></a>鱼眼镜头LMF参数结构体，具体描述See“几何畸变矫正子系统”章节。</p>
...
-   PIPE must have been created。
-   VI在线VPSSOnline Mode、VI离线VPSSOnline Mode，Not supported本接口。
...
获取VI PIPEcorresponding 鱼眼镜头LMF参数Configure 。
...
<td class="cellrowborder" valign="top" width="60%" headers="mcps1.1.4.1.2 "><p id="p5337mcpsimp"><a name="p5337mcpsimp"></a><a name="p5337mcpsimp"></a>鱼眼镜头LMF参数结构体，具体描述See“几何畸变矫正子系统”章节。</p>
</td>
...
-   物理PIPE must have been created。
-   若物理PIPE是非压，则returns failure。
-   Virtual PIPE does not support this。
...
-   只SupportedVIOffline mode。
-   WDR模式下从pipeNot supported设置用户图片。
-   this interface只SupportedYUV图像模式，纯色模式需要用户分配VB调用VGS打COVER实现。
-   设置VI用户图片，用于将VI通道输出的视频帧图像修改 as 用户Configure 的指定YUV图像数据，而不是codec的视频数据；this interface需要与[ss\_mpi\_vi\_enable\_user\_pic](#ZH-CN_TOPIC_0000002441657949)、[ss\_mpi\_vi\_disable\_user\_pic](#ZH-CN_TOPIC_0000002441697749)配合使用。
-   当前端None时序时，用户图片的帧率依赖于PIPE属性里面的目标帧率Configure ，如果PIPE属性的目标帧率设置成15帧，这个PIPE的用户图片的帧率则 as 15帧，通道帧率则根据Channel attribute的帧率来确定。如果PIPE属性的目标帧率设置成-1，用户图片的帧率则 as 30帧。当前端有时序时，用户图片的帧率由前端时序帧率和目标帧率Configure 共同决定。
-   设置完用户图片后，, i.e., 可调用ss\_mpi\_vi\_enable\_user\_pic接口对指定VI PIPE启用插入用户图片，此时VI PIPE的通道输出的数据, i.e.,  as Configure 的图片YUV数据；Generally used for视频信号丢失时，VI PIPE的通道输出NoVideo图片。

    Configure 的用户图片的宽高必须与VI PIPE大小相一致。

-   用户图片的视频帧信息结构中，需要设置图片的宽度、高度、行间隔、YUV格式以及Y分量和C分量数据的物理地址；可以从MPP公共缓冲池中获取一块相应大小的视频缓存块，从缓存块信息中得到存放YUV数据的物理地址，然后将物理地址映射到用户空间，, i.e., 可对这块内存进行YUV数据的填充操作；**填充数据时需要遵循先存Y分量数据，再存UV分量间插数据的存储顺序（小端字节序先V后U）。**
-   VI模块在启用插入用户图片时，会直接使用设置的用户图片帧信息中的物理地址，因此设置完用户图片后，不应该释放或销毁其视频缓存块，除非确认不再使用。可以通过再次调用this interface设置另外一块 VideoBuffer以修改图片信息。
-   此功能与灌图功能互斥。若开启用户图片的同时，设置frame\_source as user进行PIPE灌图，则实际输出的 as pipe灌图数据。
-   图像模式送下来的YUV帧信息must be 真实有效的VB。
-   图像模式YUV数据Pixel formatSupportedOT\_PIXEL\_FORMAT\_YVU\_SEMIPLANAR\_420、OT\_PIXEL\_FORMAT\_YVU\_SEMIPLANAR\_422、OT\_PIXEL\_FORMAT\_YUV\_400。
-   图像模式YUV数据fieldmust be OT\_VIDEO\_FIELD\_FRAME。
-   图像模式YUV数据video\_formatmust be OT\_VIDEO\_FORMAT\_LINEAR。
-   图像模式YUV数据compress\_modemust be 非压OT\_COMPRESS\_MODE\_NONE。
-   图像模式YUV数据动态范围dynamic\_rangemust be OT\_DYNAMIC\_RANGE\_SDR8。
-   图像模式YUV数据色域must be 合法值\[OT\_COLOR\_GAMUT\_BT601，OT\_COLOR\_GAMUT\_BUTT\)。
...
-   启用插入用户图片之后，当前VI PIPE的通道, i.e., 输出Configure 的用户图片帧数据。
-   PIPE must have been created。
-   this interface可以重复调用。
-   Enabled用户图片时，设置的图片数据会经过BE处理，会导致有些功能None法Supported.
...
-   只SupportedVIOffline mode。
-   VI通道不再需要输出用户图片时，应该调用this interface以恢复输出AD的原始视频数据。
-   this interface可以重复调用。
-   PIPE must have been created。
...
<p id="ss_vb_src"><a name="ss_vb_src"></a><a name="ss_vb_src"></a>ot_vb_src具体See “System Control章节”。</p>
</td>
...
-   本接口需要在创建PIPE之后调用。
-   Supported动态Configure vb来源。
-   vb来源只能set to OT\_VB\_SRC\_COMMON，OT\_VB\_SRC\_USER。
...
<td class="cellrowborder" valign="top" width="63.63999999999999%" headers="mcps1.1.4.1.2 "><p id="p5888mcpsimp"><a name="p5888mcpsimp"></a><a name="p5888mcpsimp"></a>视频缓存VB池信息。具体see “System Control”章节。</p>
</td>
...
-   PIPE must have been created。
-   用户通常调用接口ss\_mpi\_vb\_create\_pool（参见“System Control”章节）创建一个视频缓存VB池，再通过调用本接口把当前PIPEbind to固定PoolId的VB池中。Supported多个PIPEbind to同一个VB池中。
-   当要切换当前组绑定的VB池时，只需再调一次本接口正确Configure 需要bind to的VB池, i.e., 可。
...
-   需要配合[ss\_mpi\_vi\_pipe\_set\_vb\_src](#ZH-CN_TOPIC_0000002408098310)接口使用，可以动态切换。
    -   使用绑定用户VB池的方式，必须设置vb\_src as OT\_VB\_SRC\_USER。
    -   使用公共VB池的方式，必须设置vb\_src as OT\_VB\_SRC\_COMMON。

-   UserVB大小根据PIPE输出图像计算，具体Calculation Formula参考ot\_buffer.h。
...
<td class="cellrowborder" valign="top" width="75%" headers="mcps1.1.3.1.2 "><p id="p6065mcpsimp"><a name="p6065mcpsimp"></a><a name="p6065mcpsimp"></a>有效Return Value。</p>
</td>
</tr>
...
<td class="cellrowborder" valign="top" width="65%" headers="mcps1.1.4.1.2 "><p id="p6128mcpsimp"><a name="p6128mcpsimp"></a><a name="p6128mcpsimp"></a>VI通道Attribute Structure体指针。</p>
</td>
...
Channel attribute的各项Configure 限制as follows：

-   目标图像大小size：
    -   必须Configure ，且大小需要在VISupported的范围。
    -   通道Not supported缩放，通道size必须和pipe属性Configure 的size一致。

-   Pixel formatpixel\_format：
    -   输出Pixel formatSupportedsemi-planar YVU420/semi-planar YVU422/YUV400和raw8/raw10/raw12/raw14/raw16格式。
    -   Rotation功能开启时，输出Pixel format只Supportedsemi-planar YVU420和YUV400。
    -   PIPE格式 as semi-planar  YUV时，通道输出格式不能 as raw格式。
    -   通道输出格式 as raw格式时，不能Supportedmirror/flip。
    -   通道输出格式 as raw格式时，Not supported有的通道后处理功能（DIS/LDC/Rotate等）。
    -   VPSS在线时Not supportedconfigured as raw格式。

-   动态范围dynamic\_range：
    -   DYNAMIC\_RANGE\_SDR8表示的是8bit数据，其余 as 10bit数据。
    -   VI只SupportedDYNAMIC\_RANGE\_SDR8。
    -   色域信息由ISP的CSC控制。

-   视频数据格式video\_format：

    有通道都SupportedOT\_VIDEO\_FORMAT\_LINEAR格式。

-   压缩模式compress\_mode：

    Supported段压缩OT\_COMPRESS\_MODE\_SEG；

    Supported紧凑段压缩OT\_COMPRESS\_MODE\_SEG\_COMPACT；

    Supported非压缩OT\_COMPRESS\_MODE\_NONE。

    通道图像宽度超过4096Not supported紧凑段压缩。

    输出格式 as raw格式时Compression not supported。

-   帧率控制frame\_rate\_ctrl：
    -   原始帧率src\_frame\_rate：如果不进行帧率控制，则该值set to -1。
    -   目标帧率dst\_frame\_rate：如果不进行帧率控制，则该值set to -1，否则必须设置原始帧率src\_frame\_rate的值，且目标帧率必须小于或等于原始帧率。

-   PIPE must be created, otherwise failure is returned。
-   Physical channel开启紧凑段压缩后，Not supported做CoverEX与OverlayEX的处理，以及LDC，SPREAD等GDC的处理。
-   通道FLIPEnabled与通道Low Latency功能互斥，Not supported同时使用。
-   通道图像宽度超过4096Not supportedMIRROREnabled。
...
<td class="cellrowborder" valign="top" width="67%" headers="mcps1.1.4.1.2 "><p id="p6262mcpsimp"><a name="p6262mcpsimp"></a><a name="p6262mcpsimp"></a>VI通道Attribute Structure体指针。</p>
</td>
...
<td class="cellrowborder" valign="top" width="68%" headers="mcps1.1.4.1.2 "><p id="p6355mcpsimp"><a name="p6355mcpsimp"></a><a name="p6355mcpsimp"></a>Extension channelAttribute Structure体指针。</p>
</td>
...
-   PIPE must be created, otherwise failure is returned。
-   扩展channel输入绑定源must be Physical channel。
...
-   Extension channel开启紧凑段压缩后，Not supported做CoverEX与OverlayEX的处理，, i.e., 使设置CoverEX/OverlayEX也不会生效。
-   扩展channel图像来源属性set to OT\_EXT\_CHN\_SRC\_TYPE\_HEAD（See”System Control”章节）时，被绑定的Physical channelNot supported做CoverEX与OverlayEX处理，, i.e., 使设置CoverEX/OverlayEX也不会生效。
...
<td class="cellrowborder" valign="top" width="54%" headers="mcps1.1.4.1.2 "><p id="p6452mcpsimp"><a name="p6452mcpsimp"></a><a name="p6452mcpsimp"></a>Extension channelAttribute Structure体指针。</p>
</td>
...
-   PIPE must be created, otherwise failure is returned。
-   必须先设置Channel attribute。
-   可重复启用VI通道，不returns failure。
-   VI在线VPSSOnline Mode、VI离线VPSSOnline Mode下，启动VI通道不生效，直接返回Success.
...
-   PIPE must be created, otherwise failure is returned。
-   可重复禁用VI通道，不returns failure。
-   VI在线VPSSOnline Mode、VI离线VPSSOnline Mode下，禁用VI通道不生效，直接返回Success.
...
-   PIPE must be created, otherwise failure is returned。
-   Physical channel开启Gme Dis/Hybrid Dis/Ldc/Spread/Rotation/Fisheye后， as 防止这些功能的输入图像大小发生变化，要求不能再设置裁剪属性。
-   This interface is not supported in VI online VPSS online mode or VI offline VPSS online mode。
-   通道开启压缩且图像宽度大于4096时，Not supported本接口。
...
<td class="cellrowborder" valign="top" width="62%" headers="mcps1.1.4.1.2 "><p id="p6877mcpsimp"><a name="p6877mcpsimp"></a><a name="p6877mcpsimp"></a>VI物理Channel number.</p>
...
<td class="cellrowborder" valign="top" width="62%" headers="mcps1.1.4.1.2 "><p id="p6888mcpsimp"><a name="p6888mcpsimp"></a><a name="p6888mcpsimp"></a>旋转属性，具体描述see “System Control”章节。</p>
...
-   PIPE must have been created。
-   must be at 设置Channel attribute后才能设置此属性。
-   Supported非场输出的semi-planar  420和YUV400图像的旋转。
-   仅Supported90度、180度、270度的旋转、任意角度旋转和高精度任意角度旋转。
-   设置旋转后，Physical channel输出的宽高可能会发生变化，但获取的Channel attribute中的通道宽高仍 as 用户设置值。比如720p输入的图像，在旋转90度、270度时，实际输出的图像大小 as 720x1280，但[ss\_mpi\_vi\_get\_chn\_attr](#ZH-CN_TOPIC_0000002441697789)获取的DestSize宽高仍 as 1280x 720。
-   开启固定角度旋转时，Physical channel属性的压缩模式必须configured as 非压缩。
-   This interface is not supported in VI online VPSS online mode or VI offline VPSS online mode。
...
<td class="cellrowborder" valign="top" width="62%" headers="mcps1.1.4.1.2 "><p id="p6974mcpsimp"><a name="p6974mcpsimp"></a><a name="p6974mcpsimp"></a>VI物理Channel number.</p>
...
<td class="cellrowborder" valign="top" width="62%" headers="mcps1.1.4.1.2 "><p id="p6985mcpsimp"><a name="p6985mcpsimp"></a><a name="p6985mcpsimp"></a>旋转属性，具体描述see “System Control”章节。</p>
</td>
...
<td class="cellrowborder" valign="top" width="68%" headers="mcps1.1.4.1.2 "><p id="p7062mcpsimp"><a name="p7062mcpsimp"></a><a name="p7062mcpsimp"></a>VI物理Channel number.</p>
...
<td class="cellrowborder" valign="top" width="68%" headers="mcps1.1.4.1.2 "><p id="p7073mcpsimp"><a name="p7073mcpsimp"></a><a name="p7073mcpsimp"></a>LDCAttribute Structure体指针。</p>
<p id="p7074mcpsimp"><a name="p7074mcpsimp"></a><a name="p7074mcpsimp"></a>Dynamic Attribute</p>
<p id="p7075mcpsimp"><a name="p7075mcpsimp"></a><a name="p7075mcpsimp"></a><span xml:lang="sv-SE" id="ph7076mcpsimp"><a name="ph7076mcpsimp"></a><a name="ph7076mcpsimp"></a>ot_ldc_attr</span>具体See“System Control”章节。</p>
</td>
...
-   PIPE must have been created。
-   this interfacemust be at 设置Channel attribute后调用。
-   仅Supported非场输出的图像。
-   仅Supported物理channelLDC设置。
-   This interface is not supported in VI online VPSS online mode or VI offline VPSS online mode。
-   当开启展宽（Spread）时候，LDC V1的distortion\_ratioValid range只能 as   \[0, 500\]，, i.e., 只SupportedLDC的桶形模式。
-   由于LDC会对图像进行几何拉伸或几何压缩，当输入图像存在明显噪声，在桶型矫正过后，可能存在图像四周噪声颗粒变大（四周被拉伸）的现象。可将LDCConfigure 于3DNR之后改善此现象，, i.e., 调用VPSS的接口ss\_mpi\_vpss\_set\_chn\_ldc\_attr\(this interface具体See“视频处理子系统”章节\)代替this interface来Configure LDC。
-   SS928V100开启LDC V2功能，需要加载ss928v100\_gyrodis.ko文件；
-   LDC V2不能跟4轴或6轴DIS同时使用；
-   使用Gyro DIS时，必须设置LDC V2参数，See《图像质量调试工具使用指南》“DIS标定工具使用说明”章节。
    -   场景一：LDC V2+Gyro DIS效果，DISset to TRUE，LDCset to TRUE。
    -   场景二：单独Gyro DIS效果，DISset to TRUE，LDCset to FALSE。
    -   场景三：单独LDC V2效果，DISset to FALSE，LDCset to TRUE。

-   LDC V2和LDC V3的参数需使用标定工具的参数，而不能用随意Configure 的参数。
-   LDC矫正有最小的宽高限制：OT\_LDC\_MIN\_IMAGE\_WIDTH，OT\_LDC\_MIN\_IMAGE\_HEIGHTSee“几何畸变矫正子系统”。
...
<td class="cellrowborder" valign="top" width="60%" headers="mcps1.1.4.1.2 "><p id="p7174mcpsimp"><a name="p7174mcpsimp"></a><a name="p7174mcpsimp"></a>VI物理Channel number.</p>
...
<td class="cellrowborder" valign="top" width="60%" headers="mcps1.1.4.1.2 "><p id="p7185mcpsimp"><a name="p7185mcpsimp"></a><a name="p7185mcpsimp"></a>LDCAttribute Structure体指针。</p>
<p id="p7186mcpsimp"><a name="p7186mcpsimp"></a><a name="p7186mcpsimp"></a>具体See“System Control”章节。</p>
</td>
...
根据镜头畸变校正（LDC）的输出图像Coordinate point查找输入图像的Coordinate point。
...
<td class="cellrowborder" valign="top" width="59%" headers="mcps1.1.4.1.2 "><p id="p7274mcpsimp"><a name="p7274mcpsimp"></a><a name="p7274mcpsimp"></a>镜头畸变校正输出图像上需要查找映射关系的Coordinate point</p>
</td>
...
<td class="cellrowborder" valign="top" width="59%" headers="mcps1.1.4.1.2 "><p id="p7281mcpsimp"><a name="p7281mcpsimp"></a><a name="p7281mcpsimp"></a>镜头畸变校正输入图像上查找到的Coordinate point</p>
<p id="p7282mcpsimp"><a name="p7282mcpsimp"></a><a name="p7282mcpsimp"></a>ot_point具体See“System Control”章节。</p>
</td>
...
-   PIPE must be created, otherwise failure is returned。
-   this interface仅SupportedOT\_LDC\_V1版本的LDC矫正。
-   必须LDCEnabled之后再调用，否则会returns failure。
-   EnabledLDC后，如果需要变动参数，要遵循参数变动、获取校正图像和启动Coordinate point查询的先后原则。如果在LDC参数变动之前就获取了矫正图像，可能导致查找到错误的Coordinate point。
-   This interface is not supported in VI online VPSS online mode or VI offline VPSS online mode。
...
根据镜头畸变校正（LDC）的输入图像Coordinate point查找输出图像的Coordinate point。
...
<td class="cellrowborder" valign="top" width="59%" headers="mcps1.1.4.1.2 "><p id="p7374mcpsimp"><a name="p7374mcpsimp"></a><a name="p7374mcpsimp"></a>镜头畸变校正输入图像上需要查找映射关系的Coordinate point</p>
</td>
...
<td class="cellrowborder" valign="top" width="59%" headers="mcps1.1.4.1.2 "><p id="p7381mcpsimp"><a name="p7381mcpsimp"></a><a name="p7381mcpsimp"></a>镜头畸变校正输出图像上查找到的Coordinate point</p>
<p id="p7382mcpsimp"><a name="p7382mcpsimp"></a><a name="p7382mcpsimp"></a>ot_point具体See“System Control”章节。</p>
</td>
...
<td class="cellrowborder" valign="top" width="59%" headers="mcps1.1.4.1.2 "><p id="p7463mcpsimp"><a name="p7463mcpsimp"></a><a name="p7463mcpsimp"></a>VI物理Channel number.</p>
...
<td class="cellrowborder" valign="top" width="59%" headers="mcps1.1.4.1.2 "><p id="p7474mcpsimp"><a name="p7474mcpsimp"></a><a name="p7474mcpsimp"></a>展宽Attribute Structure体指针，具体See“System Control”章节。</p>
...
-   this interfacemust be at 设置Channel attribute后调用。
-   This interface is not supported in VI online VPSS online mode or VI offline VPSS online mode。
-   当开启展宽（Spread）时候，LDC的distortion\_ratioValid range只能 as   \[0, 500\]，, i.e., 只SupportedLDC的桶形模式。
-   展宽有最小输出宽高限制：OT\_SPREAD\_MIN\_IMAGE\_WIDTH等See“几何畸变矫正子系统”。
...
<td class="cellrowborder" valign="top" width="59%" headers="mcps1.1.4.1.2 "><p id="p7563mcpsimp"><a name="p7563mcpsimp"></a><a name="p7563mcpsimp"></a>VI物理Channel number.</p>
...
<td class="cellrowborder" valign="top" width="59%" headers="mcps1.1.4.1.2 "><p id="p7574mcpsimp"><a name="p7574mcpsimp"></a><a name="p7574mcpsimp"></a>展宽Attribute Structure体指针，具体See“System Control”章节。</p>
</td>
...
<td class="cellrowborder" valign="top" width="59%" headers="mcps1.1.4.1.2 "><p id="p7667mcpsimp"><a name="p7667mcpsimp"></a><a name="p7667mcpsimp"></a>VIchannel鱼眼Attribute Structure体指针，具体描述See“几何畸变矫正子系统”章节。</p>
...
-   PIPE must be created, otherwise failure is returned
-   调用本接口前Physical channel属性已Configure 。
-   开启了鱼眼矫正后，Not supported动态改变通道Crop属性、通道旋转属性以及通道宽高属性。
-   This interface is not supported in VI online VPSS online mode or VI offline VPSS online mode。
-   鱼眼校正三种校正模式：
    -   360全景模式Supported地装和顶装；
    -   180全景模式Supported壁装；
    -   normal模式Supported地装，顶装，壁装。

-   鱼眼矫正有最小的输入输出宽高限制：OT\_FISHEYE\_MIN\_IN\_IMAGE\_HEIGHT等See“几何畸变矫正子系统”。
...
<td class="cellrowborder" valign="top" width="59%" headers="mcps1.1.4.1.2 "><p id="p7764mcpsimp"><a name="p7764mcpsimp"></a><a name="p7764mcpsimp"></a>VIchannel鱼眼Attribute Structure体指针，具体描述See“几何畸变矫正子系统”章节。</p>
</td>
...
根据鱼眼校正输出图像Coordinate point查找源图像Coordinate point。
...
<p id="p7856mcpsimp"><a name="p7856mcpsimp"></a><a name="p7856mcpsimp"></a>OT_FISHEYE_MAX_RGN_NUM具体描述See“几何畸变矫正子系统”章节。</p>
</td>
...
<td class="cellrowborder" valign="top" width="59%" headers="mcps1.1.4.1.2 "><p id="p7863mcpsimp"><a name="p7863mcpsimp"></a><a name="p7863mcpsimp"></a>鱼眼校正图上需要查找映射关系的Coordinate point</p>
</td>
...
<td class="cellrowborder" valign="top" width="59%" headers="mcps1.1.4.1.2 "><p id="p7870mcpsimp"><a name="p7870mcpsimp"></a><a name="p7870mcpsimp"></a>鱼眼原图上查找到的Coordinate point</p>
<p id="p7871mcpsimp"><a name="p7871mcpsimp"></a><a name="p7871mcpsimp"></a>ot_point具体See“System Control”章节。</p>
</td>
...
-   PIPE must be created, otherwise failure is returned。
-   必须鱼眼校正启动之后再调用，否则会returns failure。
-   启动鱼眼矫正后，如果需要变动参数，要遵循参数变动、获取校正图像和启动Coordinate point查询的先后原则。如果在鱼眼参数变动之前就获取了矫正图像，可能导致查找到错误的Coordinate point。
-   多区域时，每个区域的Coordinate point按照每个区域单独计算，不能按照拼接之后的图像来计算。
-   This interface is not supported in VI online VPSS online mode or VI offline VPSS online mode。
...
<td class="cellrowborder" valign="top" width="62%" headers="mcps1.1.4.1.2 "><p id="p7969mcpsimp"><a name="p7969mcpsimp"></a><a name="p7969mcpsimp"></a>区域信息，, i.e., 起始位置、宽、高。</p>
</td>
...
<td class="cellrowborder" valign="top" width="62%" headers="mcps1.1.4.1.2 "><p id="p7976mcpsimp"><a name="p7976mcpsimp"></a><a name="p7976mcpsimp"></a>接收区域亮度和statistics的内存指针，该内存大小应该大于或等于sizeof(td_u64)×num。</p>
</td>
...
<p id="p7986mcpsimp"><a name="p7986mcpsimp"></a><a name="p7986mcpsimp"></a>大于0表示超时模式，Timeout的unit is milliseconds (ms)。</p>
</td>
...
-   PIPE must be created, otherwise failure is returned。
-   this interface针对的是VIchannel输出图像。获取亮度statistics的时机是这些图像被用户获取或被发送到后端模块之前。
-   统计区域的起始坐标的确定分两种情况：如果Not enabledCROP，则起始坐标相对于原始图像的左上角坐标；如果EnabledCROP，则起始坐标相对于CROP以后的图像的左上角坐标。统计区域不得超出原始图像或CROP之后的图像区域。
-   This interface is not supported in VI online VPSS online mode or VI offline VPSS online mode。
...
<td class="cellrowborder" valign="top" width="62%" headers="mcps1.1.4.1.2 "><p id="p8077mcpsimp"><a name="p8077mcpsimp"></a><a name="p8077mcpsimp"></a>DISConfigure 信息结构体。</p>
</td>
...
-   PIPE must be created, otherwise failure is returned。
-   在DISEnabled且scale as TRUE时，crop\_ratio参数可以动态修改，其他情况要修改dis\_cfg中参数时，必须先关闭DIS再修改参数，不可动态修改。
-   1路pipe只Supported1路通道防抖。
-   最多Supported4路陀螺仪防抖。
-   This interface is not supported in VI online VPSS online mode or VI offline VPSS online mode。
...
<td class="cellrowborder" valign="top" width="62%" headers="mcps1.1.4.1.2 "><p id="p8169mcpsimp"><a name="p8169mcpsimp"></a><a name="p8169mcpsimp"></a>DISConfigure 信息结构体。</p>
</td>
...
<td class="cellrowborder" valign="top" width="62%" headers="mcps1.1.4.1.2 "><p id="p8260mcpsimp"><a name="p8260mcpsimp"></a><a name="p8260mcpsimp"></a>DISAttribute Structure体指针。</p>
...
-   PIPE must be created, otherwise failure is returned。
-   必须先EnabledVI通道和设置DIS Configure 后，才能设置DIS属性。
-   1个PIPE只能Enabled一个channelDIS功能。
-   最多Supported4路陀螺仪防抖。
-   陀螺仪防抖场景不建议开启通道crop功能，否则会有效果问题。
-   EnabledGME DIS后，Not supported开启对应物理channelMirror和Flip功能。
-   EnabledDIS，corresponding 通道不能Enabled裁剪功能，如果要裁剪后的图像做DIS，建议在前端MIPI或者ISP做裁剪。
-   EnabledDIS，同时DIS参数中scale=FALSE时候，Not supported同时开启自由角度旋转属性。
-   修改DISConfigure 或修改channelsize和dynamic\_range属性，必须先关闭DIS。
-   This interface is not supported in VI online VPSS online mode or VI offline VPSS online mode。
-   GME DISNot supportedLow Latency，VI开启通道Low Latency再打开GME DIS，会将VI输出Low Latency自适应关闭。
-   使用HYBRID\_DIS需要调用DSP相关接口加载corresponding DSP Bin，具体参考《SVP2.0 API 参考》；使用HYBRID\_DIS功能时不能操作DSP下电和去EnabledDSP核，否则会导致挂死。
...
<td class="cellrowborder" valign="top" width="62%" headers="mcps1.1.4.1.2 "><p id="p8361mcpsimp"><a name="p8361mcpsimp"></a><a name="p8361mcpsimp"></a>DISAttribute Structure体指针。</p>
</td>
...
-   PIPE must be created, otherwise failure is returned。
-   This interface is not supported in VI online VPSS online mode or VI offline VPSS online mode。
-   this interface as 可选调用，可以调整GME模式下的防抖效果，不调用使用Default value。
-   参数只在GMEvalid in mode果，GYRO模式下invalid。
...
<td class="cellrowborder" valign="top" width="62%" headers="mcps1.1.4.1.2 "><p id="p8260mcpsimp"><a name="p8260mcpsimp"></a><a name="p8260mcpsimp"></a>DIS WDRAttribute Structure体指针。</p>
<p id="p8261mcpsimp"><a name="p8261mcpsimp"></a><a name="p8261mcpsimp"></a>Dynamic Attribute。</p>
</td>
<td class="cellrowborder" valign="top" width="18%" headers="mcps1.1.4.1.3 "><p id="p8263mcpsimp"><a name="p8263mcpsimp"></a><a name="p8263mcpsimp"></a>输入</p>
</td>
...
-   PIPE must be created, otherwise failure is returned。
-   This interface is not supported in VI online VPSS online mode or VI offline VPSS online mode。
-   参数只在GYROvalid in mode果，GME模式下invalid。参数只在wdr\_mode as 2to1 wdr、3to1 wdrvalid in mode果。
-   在fswdr\_mode as OT\_ISP\_FSWDR\_LONG\_FRAME\_MODEin mode固定匹配长帧，参数设置invalid；fswdr\_mode as OT\_ISP\_FSWDR\_AUTO\_LONG\_FRAME\_MODEin mode，若切换到了长帧模式，也会固定匹配长帧，此时参数设置invalid。
-   this interface as 可选调用，不调用使用Default value，开启WDR和GYRO\_DIS后默认 match\_frame  as 长帧。
-   使用dis\_wdr\_attr功能时需要调用ss\_mpi\_vb\_set\_supplement\_cfg（See“System Control”章节）接口添加VB的图像运动附加信息，corresponding MASK as OT\_VB\_SUPPLEMENT\_MOTION\_DATA\_MASK。
-   match\_frame 参数含义：WDRmode is2TO1时，0 as 匹配短帧，1 as 匹配长帧；WDRmode is3TO1时，0 as 短帧，1 as 中帧，2 as 长帧。
...
<td class="cellrowborder" valign="top" width="62%" headers="mcps1.1.4.1.2 "><p id="p8361mcpsimp"><a name="p8361mcpsimp"></a><a name="p8361mcpsimp"></a>DIS WDRAttribute Structure体指针。</p>
</td>
<td class="cellrowborder" valign="top" width="18%" headers="mcps1.1.4.1.3 "><p id="p8363mcpsimp"><a name="p8363mcpsimp"></a><a name="p8363mcpsimp"></a>输出</p>
</td>
</tr>
...
-   PIPE must be created, otherwise failure is returned。
-   This interface is not supported in VI online VPSS online mode or VI offline VPSS online mode。
-   参数只在GYROvalid in mode果，GME模式下invalid。
...
<p id="p8642mcpsimp"><a name="p8642mcpsimp"></a><a name="p8642mcpsimp"></a>Valid rangeSee《黑白彩色双路融合 开发参考》文档。</p>
</td>
...
-   PIPE must be created, otherwise failure is returned。
-   This interface is not supported in VI online VPSS online mode or VI offline VPSS online mode。
-   开启视场角矫正后，channelGME DIS、ROTATION、SPREAD功能不生效。
-   开启视场角矫正后，SupportedGME DIS的still crop模式。
...
<td class="cellrowborder" valign="top" width="57.00000000000001%" headers="mcps1.1.4.1.2 "><p id="p8823mcpsimp"><a name="p8823mcpsimp"></a><a name="p8823mcpsimp"></a>VI帧信息结构体。具体描述see “System Control”章节。</p>
</td>
...
<p id="p8833mcpsimp"><a name="p8833mcpsimp"></a><a name="p8833mcpsimp"></a>大于0表示超时模式，Timeout的unit is milliseconds (ms)。</p>
</td>
...
-   PIPE must be created, otherwise failure is returned。
-   this interface可以获取指定VIchannel视频图像信息。图像信息主要包括：图像的宽度、高度、Pixel format、时间戳以及YUV各分量的物理地址等等。
-   this interface需在通道已启用，且通道队列深度不 as 0，才能获取到图像。
-   Supported多次获取后再释放，但建议获取和释放接口配对使用。
...
-   调用this interface获取图像，不会对后端绑定的模块有影响。如后端绑定VO显示，可以在显示过程中获取图像，VO仍正常显示，不会受到影响。
-   当milli\_sec设 as -1时，表示阻塞模式，程序一直等待，直到获取到图像才返回。如果milli\_sec大于0时，表示非阻塞模式，参数的单位是毫秒，指Timeout，在此时间内如果没有获取到图像，则超时返回。
-   This interface is not supported in VI online VPSS online mode or VI offline VPSS online mode。
...
<p id="p8917mcpsimp"><a name="p8917mcpsimp"></a><a name="p8917mcpsimp"></a>具体描述see “System Control”章节。</p>
</td>
...
<p id="p8929mcpsimp"><a name="p8929mcpsimp"></a><a name="p8929mcpsimp"></a>具体描述see “System Control”章节。</p>
</td>
...
<p id="p8937mcpsimp"><a name="p8937mcpsimp"></a><a name="p8937mcpsimp"></a>具体描述see “System Control”章节。</p>
</td>
...
-   this interface必须与[ss\_mpi\_vi\_get\_chn\_frame](#ZH-CN_TOPIC_0000002441698001)配对使用。
-   用户必须保证frame\_info结构中的信息与获取时一致，否则可能造成释放不Success.
...
<td class="cellrowborder" valign="top" width="62.629999999999995%" headers="mcps1.1.4.1.2 "><p id="p9027mcpsimp"><a name="p9027mcpsimp"></a><a name="p9027mcpsimp"></a>Low Latency功能参数结构体。</p>
</td>
...
-   PIPE must have been created。
-   只有VI在线VPSS离线、VI离线VPSSOffline mode下Supported该功能。
-   Low Latency按最小延时的策略来做自适应。
-   当通道设置chn\_crop时，Low Latency行号应小于通道crop的高度。
-   chnLow Latency没有one\_buf功能，one\_buf\_enConfigure 不生效。
...
<td class="cellrowborder" valign="top" width="60%" headers="mcps1.1.4.1.2 "><p id="p9117mcpsimp"><a name="p9117mcpsimp"></a><a name="p9117mcpsimp"></a>Low Latency功能参数结构体。</p>
</td>
...
<td class="cellrowborder" valign="top" width="69%" headers="mcps1.1.4.1.2 "><p id="p9204mcpsimp"><a name="p9204mcpsimp"></a><a name="p9204mcpsimp"></a>VI通道输出YUV数据的行stride对齐，单位 as Byte。</p>
...
-   PIPE must have been created。
-   align as 0时stride对齐使用自动模式，对齐要求跟随系统。
-   align as 非0时使用指定的stride对齐。
-   当align小于芯片要求的stride对齐时，系统分配buffer和计算stride还是以芯片要求stride对齐 as 准。
-   当align不等于芯片要求的stride对齐倍数时，系统分配buffer和计算stride向上对齐到芯片要求stride对齐的倍数。

    例：alignset to 48，芯片要求32对齐时，系统分配buffer和计算stride时使用64对齐。

-   拍照后处理算法输入的YUV数据stride需要128Bytes对齐，因此需要调用本接口Configure VI输出的YUV数据stride对齐。
-   This interface is not supported in VI online VPSS online mode or VI offline VPSS online mode。
...
<td class="cellrowborder" valign="top" width="69%" headers="mcps1.1.4.1.2 "><p id="p9299mcpsimp"><a name="p9299mcpsimp"></a><a name="p9299mcpsimp"></a>VI通道输出YUV数据的行stride对齐，单位 as Byte。</p>
...
-   PIPE must have been created。
-   用户通常调用接口ss\_mpi\_vb\_create\_pool（参见“System Control”章节）创建一个视频缓存VB池，再通过调用本接口把当前CHNbind to固定PoolId的VB池中。Supported多个CHNbind to同一个VB池中。
-   当要切换当前组绑定的VB池时，只需再调一次接口本接口正确Configure 需要bind to的VB池, i.e., 可。
-   vb\_pool必须保证是已创建VB池的有效PoolId。
...
-   UserVB大小根据CHN输出图像计算，具体Calculation Formula参考ot\_buffer.h。
...
<td class="cellrowborder" valign="top" width="75%" headers="mcps1.1.3.1.2 "><p id="p9850mcpsimp"><a name="p9850mcpsimp"></a><a name="p9850mcpsimp"></a>有效Return Value。</p>
</td>
</tr>
...
<td class="cellrowborder" valign="top" width="65.66%" headers="mcps1.1.4.1.2 "><p id="p9903mcpsimp"><a name="p9903mcpsimp"></a><a name="p9903mcpsimp"></a>拼接组的Attribute Structure体指针。</p>
</td>
...
-   must be at PIPE创建之前设置VI拼接组属性。
-   拼接组属性中的stitch\_en表示拼接组的Enabled，不允许重复set to TD\_TRUE，如果已经 as TD\_TRUE，重新设置拼接组属性，必须先set to TD\_FALSE，再重新设置。
-   拼接组属性的PIPE必须不重复。
-   拼接组的PIPE必须是相同的工作模式，比如都是VI离线，VPSSOffline mode。
-   拼接的sensor时序必须保持同步，如果相差大于max\_pts\_gap，则会出现同步不上的现象。
-   当set to 同步模式的时候，拼接的sensor时序必须一致\(同时上报帧起始\)，否则效果可能不一致。
-   当set to 同步模式的时候，拼接组的PIPE必须是相同的工作模式且必须是VI离线，VPSS离线，或者VI离线，VPSS在线。
-   使用送帧+isp\_runonce做多pipe拼接时，需要调用者自己保证多帧间的同步；使用pipe送帧的场景下Not supported拼接sync模式，会导致通路异常。
...
<td class="cellrowborder" valign="top" width="62%" headers="mcps1.1.4.1.2 "><p id="p9986mcpsimp"><a name="p9986mcpsimp"></a><a name="p9986mcpsimp"></a>拼接组的Attribute Structure体指针。</p>
</td>
...
<td class="cellrowborder" valign="top" width="62%" headers="mcps1.1.4.1.2 "><p id="p10049mcpsimp"><a name="p10049mcpsimp"></a><a name="p10049mcpsimp"></a>VIModule Parameter结构体。</p>
</td>
...
-   must be at 有VI设备设置，PIPE创建之前设置VIModule Parameter。
-   VI控制是否丢帧，PIPE丢帧取决于PIPE是否有数据写出，如果有数据写出则丢帧策略有效，否则丢帧策略invalid。
-   当PIPE处于VI在线VPSS在线时，由于VI通道None帧数据输出，因此VI通道丢帧失效。
-   暂Not supportedthis interface。
...
<td class="cellrowborder" valign="top" width="62%" headers="mcps1.1.4.1.2 "><p id="p10118mcpsimp"><a name="p10118mcpsimp"></a><a name="p10118mcpsimp"></a>VIModule Parameter结构体。</p>
</td>
...
this interfaceNot supported多线程调用。
...
-   若crop\_mode as OT\_COORD\_RATIO（相对坐标模式）：

    使用crop\_rect的成员时应进行转换，Calculation Formula as ：

    x = 起始点坐标\*原始图像宽度/1000，合法Valid range: \[0, 999\]。计算完成后将进行取整和对齐操作。公式同样适用于纵坐标计算，x as 转换后的坐标。

    width=区域宽度\*实际图像宽度/1000，区域宽度Valid range: \[1, 1000\]。计算完成后将进行取整和对齐操作。公式同样适用于区域高度计算，width as 转换后的宽度。

-   在相对模式进行裁剪时，裁剪区域的坐标Valid range as \[0, 999\]，裁剪区域的宽高Valid range as \[1, 1000\]。
-   如果裁剪放大区域超出图像范围，则自动裁剪至图像的Valid range区域。
-   裁剪区域的起始点坐标和宽高需满足2像素对齐要求。

-   None论是相对还是绝对裁剪放大，实际裁剪放大区域的宽高不能小于输入图像宽高的1/8，起始点必须位于图像范围内，不允许使用负坐标。
-   zoom\_in\_windowNot supported与gyro组合。
-   当channel宽高改变时，需要去重新设置zoom属性，否则可能导致效果异常。
...
视频输入相关数据类型定义as follows：

-   [OT\_VI\_MAX\_SNS\_CFG\_NUM](#ZH-CN_TOPIC_0000002441657781): 时序Configure 数据的最大个数。
...
-   [OT\_VI\_MAX\_WDR\_FRAME\_NUM](#ZH-CN_TOPIC_0000002441657761): 定义VI WDR融合组合成maximum frame 个数。
-   [OT\_VI\_MAX\_PHYS\_CHN\_NUM](#ZH-CN_TOPIC_0000002408099078)：定义VI物理channel最大个数。
-   [OT\_VI\_MAX\_EXT\_CHN\_NUM](#ZH-CN_TOPIC_0000002408258522)：定义VI扩展channel最大个数。
-   [OT\_VI\_EXT\_CHN\_START](#ZH-CN_TOPIC_0000002441698257)：定义VI扩展channel起始Channel number.
-   [OT\_VI\_MAX\_EXTCHN\_BIND\_PER\_CHN](#ZH-CN_TOPIC_0000002408098646)：定义VI单个通道可绑定的Extension channel最大个数。
-   [OT\_VI\_MAX\_CHN\_NUM](#ZH-CN_TOPIC_0000002441657673)：定义VIPhysical channel和扩展channel总个数。
...
-   [OT\_VI\_PIPE\_OFFLINE\_MAX\_WIDTH](#ZH-CN_TOPIC_0000002408258562)：Offline mode下VI PIPE处理图像的最大宽度。
-   [OT\_VI\_PIPE\_OFFLINE\_MAX\_HEIGHT](#ZH-CN_TOPIC_0000002408258274)：VI PIPE离线处理图像的最大高度。
-   [OT\_VI\_PIPE\_ONLINE\_MIN\_WIDTH](#ZH-CN_TOPIC_0000002441697937)：Online Mode下VI PIPE处理图像的最小宽度。
-   [OT\_VI\_PIPE\_ONLINE\_MIN\_HEIGHT](#ZH-CN_TOPIC_0000002408098734)：Online Mode下VI PIPE处理图像的最小高度。
-   [OT\_VI\_PIPE\_ONLINE\_MAX\_WIDTH](#ZH-CN_TOPIC_0000002408258354)：Online Mode下VI PIPE处理图像的最大宽度。
-   [OT\_VI\_PIPE\_ONLINE\_MAX\_HEIGHT](#ZH-CN_TOPIC_0000002441658429)：Online Mode下VI PIPE处理图像的最大高度。
-   [OT\_VI\_PHYS\_CHN\_OFFLINE\_MIN\_WIDTH](#ZH-CN_TOPIC_0000002408098614)：VIPhysical channel离线Supported的最小宽度。
-   [OT\_VI\_PHYS\_CHN\_OFFLINE\_MIN\_HEIGHT](#ZH-CN_TOPIC_0000002441698265)：VIPhysical channel离线Supported的最小高度。
-   [OT\_VI\_PHYS\_CHN\_OFFLINE\_MAX\_WIDTH](#ZH-CN_TOPIC_0000002441658245)：VIPhysical channel离线Supported的最大宽度。
-   [OT\_VI\_PHYS\_CHN\_OFFLINE\_MAX\_HEIGHT](#ZH-CN_TOPIC_0000002441657701)：VIPhysical channel离线Supported的最大高度。
-   [OT\_VI\_PHYS\_CHN\_ONLINE\_MIN\_WIDTH](#ZH-CN_TOPIC_0000002441658109)：VIPhysical channel在线Supported的最小宽度。
-   [OT\_VI\_PHYS\_CHN\_ONLINE\_MIN\_HEIGHT](#ZH-CN_TOPIC_0000002408258638)：VIPhysical channel在线Supported的最小高度。
-   [OT\_VI\_PHYS\_CHN\_ONLINE\_MAX\_WIDTH](#ZH-CN_TOPIC_0000002441657773)：VIPhysical channel在线Supported的最大宽度。
-   [OT\_VI\_PHYS\_CHN\_ONLINE\_MAX\_HEIGHT](#ZH-CN_TOPIC_0000002441658417)：VIPhysical channel在线Supported的最大高度。
-   [OT\_VI\_EXT\_CHN\_MIN\_WIDTH](#ZH-CN_TOPIC_0000002408258546)：VIExtension channelSupported的最小宽度。
-   [OT\_VI\_EXT\_CHN\_MIN\_HEIGHT](#ZH-CN_TOPIC_0000002441658437)：VIExtension channelSupported的最小高度。
-   [OT\_VI\_EXT\_CHN\_MAX\_WIDTH](#ZH-CN_TOPIC_0000002441698085)：VIExtension channelSupported的最大宽度。
-   [OT\_VI\_EXT\_CHN\_MAX\_HEIGHT](#ZH-CN_TOPIC_0000002441697513)：VIExtension channelSupported的最大高度。
-   [OT\_VI\_EXT\_CHN\_MAX\_ZOOMIN](#ZH-CN_TOPIC_0000002441697909)：VIExtension channelSupported的最大放大倍数。
-   [OT\_VI\_EXT\_CHN\_MAX\_ZOOMOUT](#ZH-CN_TOPIC_0000002441697885)：VIExtension channelSupported的最大缩小倍数。
...
-   [OT\_VI\_INVALID\_FRAME\_RATE](#ZH-CN_TOPIC_0000002408099030)：VI帧率控制的宏，当原始帧率和目标帧率都 as 该宏时，不做帧率控制。
...
-   [ot\_vi\_thermo\_sns\_attr](#ZH-CN_TOPIC_0000002441698205): 定义热成像探测器的Configure 属性。
...
-   [ot\_vi\_chn\_attr](#ZH-CN_TOPIC_0000002441697741)：定义VIchannel属性。
-   [ot\_vi\_ext\_chn\_attr](#ZH-CN_TOPIC_0000002408258994)：定义VI扩展channel属性。
-   [ot\_vi\_crop\_info](#ZH-CN_TOPIC_0000002441658185)：定义VIchannelcrop信息。
-   [ot\_vi\_chn\_status](#ZH-CN_TOPIC_0000002441697821)：定义VIchannel运行状态。
-   [ot\_vi\_stitch\_cfg\_mode](#ZH-CN_TOPIC_0000002441657821)：定义VI拼接的Configure 模式。
-   [ot\_vi\_stitch\_grp\_attr](#ZH-CN_TOPIC_0000002408098978)：定义VI拼接组的属性。
-   [ot\_vi\_mod\_param](#ZH-CN_TOPIC_0000002408259018)：定义VI的Module Parameter。
...
-   [ot\_dis\_cfg](#ZH-CN_TOPIC_0000002441698229)：定义DIS的Configure 信息。
...
定义VI单个通道可绑定的Extension channel最大个数。
...
定义VIPhysical channel和扩展channel总个数。
...
VIPhysical channel离线Supported的最小宽度。
...
VIPhysical channel离线Supported的最小高度。
...
VIPhysical channel离线Supported的最大宽度。
...
VIPhysical channel离线Supported的最大高度。
...
VIPhysical channel在线Supported的最小宽度。
...
VIPhysical channel在线Supported的最小高度。
...
VIPhysical channel在线Supported的最大宽度。
...
VIPhysical channel在线Supported的最大高度。
...
VIExtension channelSupported的最小宽度。
...
VIExtension channelSupported的最小高度。
...
VIExtension channelSupported的最大宽度。
...
VIExtension channelSupported的最大高度。
...
VIExtension channelSupported的最大放大倍数。
...
VIExtension channelSupported的最大缩小倍数。
...
VI帧率控制的宏，当原始帧率和目标帧率都 as 该宏时，不做帧率控制。
...
<td class="cellrowborder" valign="top" width="56.99999999999999%" headers="mcps1.1.3.1.2 "><p id="p11015mcpsimp"><a name="p11015mcpsimp"></a><a name="p11015mcpsimp"></a>输入数据的协议符合标准BT.656协议，端口数据输入mode is亮度色度分离模式，分量mode is单分量。</p>
</td>
</tr>
...
<td class="cellrowborder" valign="top" width="56.99999999999999%" headers="mcps1.1.3.1.2 "><p id="p11020mcpsimp"><a name="p11020mcpsimp"></a><a name="p11020mcpsimp"></a>输入数据的协议符合标准BT.601协议，端口数据输入mode is亮度色度复合模式，分量mode is单分量。</p>
</td>
</tr>
...
<td class="cellrowborder" valign="top" width="56.99999999999999%" headers="mcps1.1.3.1.2 "><p id="p11025mcpsimp"><a name="p11025mcpsimp"></a><a name="p11025mcpsimp"></a>输入数据的协议 as Digital camera协议，端口数据输入mode is亮度色度复合模式，分量mode is单分量。</p>
</td>
</tr>
...
<td class="cellrowborder" valign="top" width="56.99999999999999%" headers="mcps1.1.3.1.2 "><p id="p11030mcpsimp"><a name="p11030mcpsimp"></a><a name="p11030mcpsimp"></a>输入数据的协议符合标准BT.1120协议（BT656+双分量），端口数据输入mode is亮度色度分离模式，分量mode is双分量。</p>
</td>
</tr>
...
<td class="cellrowborder" valign="top" width="56.99999999999999%" headers="mcps1.1.3.1.2 "><p xml:lang="sv-SE" id="p11055mcpsimp"><a name="p11055mcpsimp"></a><a name="p11055mcpsimp"></a>热成像工作mode isOT_VI_THERMO_WORK_MODE_T0或者OT_VI_THERMO_WORK_MODE_T2时，用于传输热成像探测器的数据。</p>
</td>
</tr>
</tbody>
</table>
...
<td class="cellrowborder" valign="top" width="51%" headers="mcps1.1.3.1.2 "><p id="p11098mcpsimp"><a name="p11098mcpsimp"></a><a name="p11098mcpsimp"></a>2路复合工作模式，输入数据的协议must be 标准BT.656协议。</p>
</td>
</tr>
...
<td class="cellrowborder" valign="top" width="51%" headers="mcps1.1.3.1.2 "><p id="p11103mcpsimp"><a name="p11103mcpsimp"></a><a name="p11103mcpsimp"></a>4路复合工作模式，输入数据的协议must be 标准BT.656协议。</p>
</td>
</tr>
</tbody>
</table>
...
当该项设 as 2路或4路复合工作in mode，设备输入的协议必须是BT.656协议。1路复合工作模式没有限制。
...
<td class="cellrowborder" valign="top" width="55.00000000000001%" headers="mcps1.1.3.1.2 "><p id="p11137mcpsimp"><a name="p11137mcpsimp"></a><a name="p11137mcpsimp"></a>VI输入 as 逐行图像。</p>
</td>
</tr>
...
<td class="cellrowborder" valign="top" width="55.00000000000001%" headers="mcps1.1.3.1.2 "><p id="p11142mcpsimp"><a name="p11142mcpsimp"></a><a name="p11142mcpsimp"></a>VI输入 as 隔行图像。</p>
</td>
</tr>
</tbody>
</table>
...
仅SupportedOT\_VI\_SCAN\_PROGRESSIVE。
...
<td class="cellrowborder" valign="top" width="64%" headers="mcps1.1.3.1.2 "><p id="p11181mcpsimp"><a name="p11181mcpsimp"></a><a name="p11181mcpsimp"></a>YUV数据通过分离模式输入时，C分量的输入排列顺序 as VUVU。</p>
</td>
</tr>
...
<td class="cellrowborder" valign="top" width="64%" headers="mcps1.1.3.1.2 "><p id="p11186mcpsimp"><a name="p11186mcpsimp"></a><a name="p11186mcpsimp"></a>YUV数据通过分离模式输入时，C分量的输入排列顺序 as UVUV。</p>
</td>
</tr>
...
<td class="cellrowborder" valign="top" width="64%" headers="mcps1.1.3.1.2 "><p id="p11191mcpsimp"><a name="p11191mcpsimp"></a><a name="p11191mcpsimp"></a>YUV数据通过复合模式输入时，顺序 as <span xml:lang="sv-SE" id="ph11192mcpsimp"><a name="ph11192mcpsimp"></a><a name="ph11192mcpsimp"></a>UYVY</span>。</p>
</td>
</tr>
...
<td class="cellrowborder" valign="top" width="64%" headers="mcps1.1.3.1.2 "><p id="p11197mcpsimp"><a name="p11197mcpsimp"></a><a name="p11197mcpsimp"></a>YUV数据通过复合模式输入时，顺序 as <span xml:lang="sv-SE" id="ph11198mcpsimp"><a name="ph11198mcpsimp"></a><a name="ph11198mcpsimp"></a>VYUY</span>。</p>
</td>
</tr>
...
<td class="cellrowborder" valign="top" width="64%" headers="mcps1.1.3.1.2 "><p id="p11203mcpsimp"><a name="p11203mcpsimp"></a><a name="p11203mcpsimp"></a>YUV数据通过复合模式输入时，顺序 as <span xml:lang="sv-SE" id="ph11204mcpsimp"><a name="ph11204mcpsimp"></a><a name="ph11204mcpsimp"></a>YUYV</span>。</p>
</td>
</tr>
...
<td class="cellrowborder" valign="top" width="64%" headers="mcps1.1.3.1.2 "><p id="p11209mcpsimp"><a name="p11209mcpsimp"></a><a name="p11209mcpsimp"></a>YUV数据通过复合模式输入时，顺序 as <span xml:lang="sv-SE" id="ph11210mcpsimp"><a name="ph11210mcpsimp"></a><a name="ph11210mcpsimp"></a>YVYU</span>。</p>
</td>
</tr>
</tbody>
</table>
...
<td class="cellrowborder" valign="top" width="69%" headers="mcps1.1.3.1.2 "><p xml:lang="sv-SE" id="p11244mcpsimp"><a name="p11244mcpsimp"></a><a name="p11244mcpsimp"></a>垂直同步翻转模式，, i.e., 翻转一次表示一场。BT.601模式The table below示场号，DC模式The table below示行有效信号。</p>
</td>
</tr>
...
<td class="cellrowborder" valign="top" width="69%" headers="mcps1.1.3.1.2 "><p xml:lang="sv-SE" id="p11249mcpsimp"><a name="p11249mcpsimp"></a><a name="p11249mcpsimp"></a>垂直同步脉冲模式，, i.e., 一个脉冲到来表示新的一帧或一场。</p>
</td>
</tr>
</tbody>
</table>
...
<td class="cellrowborder" valign="top" width="64%" headers="mcps1.1.3.1.2 "><p xml:lang="sv-SE" id="p11284mcpsimp"><a name="p11284mcpsimp"></a><a name="p11284mcpsimp"></a>若ot_vi_vsync = OT_VI_VSYNC_FIELD，则表示偶数场的vsync信号 as 高电平；</p>
<p xml:lang="sv-SE" id="p11285mcpsimp"><a name="p11285mcpsimp"></a><a name="p11285mcpsimp"></a>若ot_vi_vsync = OT_VI_VSYNC_PULSE，则表示vsync同步脉冲 as 正脉冲。</p>
</td>
</tr>
...
<td class="cellrowborder" valign="top" width="64%" headers="mcps1.1.3.1.2 "><p xml:lang="sv-SE" id="p11290mcpsimp"><a name="p11290mcpsimp"></a><a name="p11290mcpsimp"></a>若ot_vi_vsync = OT_VI_VSYNC_FIELD，则表示偶数场的vsync信号 as 低电平；</p>
<p xml:lang="sv-SE" id="p11291mcpsimp"><a name="p11291mcpsimp"></a><a name="p11291mcpsimp"></a>若ot_vi_vsync = OT_VI_VSYNC_PULSE，则表示vsync同步脉冲 as 负脉冲。</p>
</td>
</tr>
</tbody>
</table>
...
DC/BT.656/BT.1120模式下需要Configure 该项。
...
<td class="cellrowborder" valign="top" width="55.00000000000001%" headers="mcps1.1.3.1.2 "><p xml:lang="sv-SE" id="p11447mcpsimp"><a name="p11447mcpsimp"></a><a name="p11447mcpsimp"></a>若ot_vi_vsync_valid = OT_VI_VSYNC_VALID_SIG，则表示高电平 as 有效信号。</p>
</td>
</tr>
...
<td class="cellrowborder" valign="top" width="55.00000000000001%" headers="mcps1.1.3.1.2 "><p xml:lang="sv-SE" id="p11452mcpsimp"><a name="p11452mcpsimp"></a><a name="p11452mcpsimp"></a>若ot_vi_vsync_valid = OT_VI_VSYNC_VALID_SIG，则表示低电平 as 有效信号。</p>
</td>
</tr>
</tbody>
</table>
...
<td class="cellrowborder" valign="top" width="75%" headers="mcps1.1.3.1.2 "><p xml:lang="sv-SE" id="p11523mcpsimp"><a name="p11523mcpsimp"></a><a name="p11523mcpsimp"></a>隔行输入时偶场垂直前消隐区高度（帧输入时invalid）。</p>
</td>
</tr>
...
<td class="cellrowborder" valign="top" width="75%" headers="mcps1.1.3.1.2 "><p xml:lang="sv-SE" id="p11528mcpsimp"><a name="p11528mcpsimp"></a><a name="p11528mcpsimp"></a>隔行输入时偶场垂直有效高度（帧输入时invalid）。</p>
</td>
</tr>
...
<td class="cellrowborder" valign="top" width="75%" headers="mcps1.1.3.1.2 "><p xml:lang="sv-SE" id="p11533mcpsimp"><a name="p11533mcpsimp"></a><a name="p11533mcpsimp"></a>隔行输入时偶场垂直后消隐区高度（帧输入时invalid）。</p>
</td>
</tr>
</tbody>
</table>
...
-   此部分同步信息详细Configure 可参考《MPP 媒体处理软件V5.0 FAQ》2.2小节“VI YUV时序Configure ”。
-   BT.601模式下，垂直同步信号只需Configure vsync和vsync\_neg, i.e., 可，它们分别表示管脚OT\_VI\_P\_VSYNC\_FIELD的垂直同步信号类型和垂直同步信号的极性，两者决定了采用何种垂直同步时序。
    -   vsyncconfigured as OT\_VI\_VSYNC\_FIELD，BT.601模式The table below示场号。此时，如果vsync\_negconfigured as OT\_VI\_VSYNC\_NEG\_HIGH，则高电平表示偶场，低电平表示奇场；如果vsync\_negconfigured as OT\_VI\_VSYNC\_NEG\_LOW，则低电平表示偶场，高电平表示奇场。
    -   vsyncconfigured as OT\_VI\_VSYNC\_PULSE，表示垂直同步脉冲，, i.e., 一个脉冲到来表示新的一场或一帧。此时，如果vsync\_negconfigured as OT\_VI\_VSYNC\_NEG\_HIGH，则正脉冲表示垂直同步脉冲；如果vsync\_negconfigured as OT\_VI\_VSYNC\_NEG\_LOW，则负脉冲表示垂直同步脉冲。

-   DC/BT.656/BT.1120模式下，垂直同步信号需要Configure vsync、vsync\_valid和 vsync\_valid\_neg三项，它们分别表示管脚OT\_VI\_P\_VSYNC\_FIELD的垂直同步信号类型、垂直同步信号标识和垂直有效信号极性。vsync\_valid\_neg只有在vsync\_valid配 as OT\_VI\_VSYNC\_VALID\_SIG时有效。
    -   vsyncconfigured as OT\_VI\_VSYNC\_PULSE，表示垂直时序脉冲方式。如果在垂直消隐区内hsync信号仍有效，则vsync\_valid需configured as OT\_VI\_VSYNC\_VALID\_SIG选择垂直有效标识，同时vsync\_valid\_neg需要configured as OT\_VI\_VSYNC\_VALID\_NEG\_HIGH（或OT\_VI\_VSYNC\_VALID\_NEG\_LOW），以表示高（低）电平有效，其他情况不需关注此属性。
    -   如果输入时序 as 垂直时序行有效方式，只需将vsyncconfigured as OT\_VI\_VSYNC\_FIELD, i.e., 可。
    -   标准BT.656、BT.1120同步信息Configure 可参考《MPP 媒体处理软件V5.0 FAQ》2.2小节“VI YUV时序Configure ”。

-   hsync和  hsync\_neg分别表示管脚VI\_P\_HSYNC\_VD的水平同步信号类型和水平同步信号的极性，两者决定了采用何种水平同步时序。
    -   hsyncconfigured as OT\_VI\_HSYNC\_VALID\_SIG，表示数据有效信号。此时，如果hsync\_negconfigured as OT\_VI\_HSYNC\_NEG\_HIGH，则高电平表示数据有效；如果hsync\_negconfigured as OT\_VI\_HSYNC\_NEG\_LOW，则低电平表示数据有效。
    -   hsyncconfigured as OT\_VI\_HSYNC\_PULSE，表示水平同步脉冲信号，, i.e., 一个脉冲到来表示新一行的开始。此时，如果hsync\_negconfigured as OT\_VI\_HSYNC\_NEG\_HIGH，则正脉冲表示水平同步脉冲；如果hsync\_negconfigured as OT\_VI\_HSYNC\_NEG\_LOW，则负脉冲表示水平同步脉冲。

对于消隐信息，水平或垂直的有效数据方式不需要配消隐区。

SSxxSupported的具体时序介绍see 芯片手册。
...
<td class="cellrowborder" valign="top" width="65%" headers="mcps1.1.3.1.2 "><p id="p11689mcpsimp"><a name="p11689mcpsimp"></a><a name="p11689mcpsimp"></a>输入数据类型 as RAW，VI前端一般接的是sensor。</p>
</td>
</tr>
...
<td class="cellrowborder" valign="top" width="65%" headers="mcps1.1.3.1.2 "><p id="p11694mcpsimp"><a name="p11694mcpsimp"></a><a name="p11694mcpsimp"></a>输入数据类型 as YUV，VI前端一般接的是AD。</p>
</td>
</tr>
</tbody>
</table>
...
<td class="cellrowborder" valign="top" width="70%" headers="mcps1.1.3.1.2 "><p xml:lang="sv-SE" id="p12606mcpsimp"><a name="p12606mcpsimp"></a><a name="p12606mcpsimp"></a>1、2、4路复合工作模式，必须set to OT_VI_WORK_MODE_MULTIPLEX_1<span xml:lang="en-US" id="ph12607mcpsimp"><a name="ph12607mcpsimp"></a><a name="ph12607mcpsimp"></a>。</span></p>
</td>
</tr>
...
<td class="cellrowborder" valign="top" width="70%" headers="mcps1.1.3.1.2 "><p id="p12612mcpsimp"><a name="p12612mcpsimp"></a><a name="p12612mcpsimp"></a>分量掩码Configure 。当intf_mode = OT_VI_INTF_MODE_BT1120时，需要Configure Y（对应该变量数组的0下标）和C（对应该变量数组的1下标）的分量掩码，其他in modeConfigure 单分量掩码（对应该变量数组的0下标），另一个分量（对应该变量数组的1下标）配成0x0。</p>
</td>
</tr>
...
<td class="cellrowborder" valign="top" width="70%" headers="mcps1.1.3.1.2 "><p xml:lang="sv-SE" id="p12622mcpsimp"><a name="p12622mcpsimp"></a><a name="p12622mcpsimp"></a>Valid range: [-1, 3]，推荐统一set to Default value-1，invalid参数，parameter is not checked。</p>
</td>
</tr>
...
<td class="cellrowborder" valign="top" width="70%" headers="mcps1.1.3.1.2 "><p xml:lang="sv-SE" id="p12627mcpsimp"><a name="p12627mcpsimp"></a><a name="p12627mcpsimp"></a>输入数据顺序 (仅Supportedyuv格式)</p>
</td>
</tr>
...
<td class="cellrowborder" valign="top" width="70%" headers="mcps1.1.3.1.2 "><p xml:lang="sv-SE" id="p12632mcpsimp"><a name="p12632mcpsimp"></a><a name="p12632mcpsimp"></a>同步时序Configure 。</p>
</td>
</tr>
...
<td class="cellrowborder" valign="top" width="70%" headers="mcps1.1.3.1.2 "><p xml:lang="sv-SE" id="p12637mcpsimp"><a name="p12637mcpsimp"></a><a name="p12637mcpsimp"></a>输入数据类型，Sensor输入GenerallyRaw，AD输入GenerallyYUV。</p>
</td>
</tr>
...
<p xml:lang="sv-SE" id="p12658mcpsimp"><a name="p12658mcpsimp"></a><a name="p12658mcpsimp"></a>宽度：[<a href="#ZH-CN_TOPIC_0000002408258678">OT_VI_DEV_MIN_WIDTH</a>, <span xml:lang="de-DE" id="ph12661mcpsimp"><a name="ph12661mcpsimp"></a><a name="ph12661mcpsimp"></a>OT_VI_DEV_MAX_WIDTH</span></a>]</p>
<p xml:lang="sv-SE" id="p12662mcpsimp"><a name="p12662mcpsimp"></a><a name="p12662mcpsimp"></a>高度：[<a href="#ZH-CN_TOPIC_0000002441658141">OT_VI_DEV_MIN_HEIGHT</a>, <span xml:lang="de-DE" id="ph12665mcpsimp"><a name="ph12665mcpsimp"></a><a name="ph12665mcpsimp"></a>OT_VI_DEV_MAX_HEIGHT</span></a>]</p>
</td>
</tr>
...
<p id="p12671mcpsimp"><a name="p12671mcpsimp"></a><a name="p12671mcpsimp"></a>参数See“System Control”章节。</p>
</td>
</tr>
</tbody>
</table>
...
-   只有dev0SupportedOT\_DATA\_RATE\_X2。data\_rate的值需与mipi\_data\_rate\_t（SeeMIPI\_RX章节）保持一致。
-   OT\_DATA\_RATE\_X2通路Configure ：MIPI0configured as MIPI\_DATA\_RATE\_X2，DEV0Configure OT\_DATA\_RATE\_X2，DEV0绑定PIPE0，其他通路若需要Two pixels per beat写出，需要bypass isp；VI在线且非WDRin mode，Not supportedTwo pixels per beat通路，会导致通道采集Failure.
-   当接口mode isOT\_VI\_INTF\_MODE\_MIPI\_YUV420\_NORM，OT\_VI\_INTF\_MODE\_MIPI\_YUV420\_LEGACY，OT\_VI\_INTF\_MODE\_MIPI\_YUV422时data\_reversemust be TD\_FALSE，且掩码的设置must be component\_mask\[0\] = 0xFF000000，component\_mask\[1\] = 0x00FF0000，, i.e., 高8bit输入Y数据，低8bit输入C数据，否则会导致图像异常或None图像等现像。
...
当前端sensor进行了相应的调整后， as 了保持好的图像质量就需要在采集的时候进行相应的调整。
...
-   WDR模式下Not supportedBAS。
-   前端输入YUV数据，则Not supportedBAS。
-   前端RAW数据位宽大于14bitNot supportedBAS。
...
<td class="cellrowborder" valign="top" width="62%" headers="mcps1.1.3.1.2 "><p xml:lang="sv-SE" id="p12839mcpsimp"><a name="p12839mcpsimp"></a><a name="p12839mcpsimp"></a>Bayer scale功能是否Enabled。</p>
</td>
</tr>
...
-   BAS只Supported1倍，2倍或者3倍缩小。
-   Rephase限制：BAS 打开rephase功能时（rephase的调整类型不是设置成OT\_VI\_REPHASE\_MODE\_NONE时），前端输入的宽度有限制，SS928V100最大输入宽度 as 2114，否则图像会出现异常。
-   当DEV0开BAS功能时，只有PIPE0绑定在DEV0上才会得到BAS后的数据，其他PIPEbind toDEV0NoneBAS效果。
...
每一类热成像探测器只能Configure 对应类型的管脚，如T0类型的热成像探测器只能Configure OT\_VI\_SD\_MUX\_T0\_SD0和OT\_VI\_SD\_MUX\_T0\_SD1，其余管脚configured as OT\_VI\_SD\_MUX\_BUTT。
...
<td class="cellrowborder" valign="top" width="62%" headers="mcps1.1.3.1.2 "><p xml:lang="sv-SE" id="p13024mcpsimp"><a name="p13024mcpsimp"></a><a name="p13024mcpsimp"></a>时序Configure 数据的个数。</p>
</td>
</tr>
...
<td class="cellrowborder" valign="top" width="62%" headers="mcps1.1.3.1.2 "><p xml:lang="sv-SE" id="p13029mcpsimp"><a name="p13029mcpsimp"></a><a name="p13029mcpsimp"></a>时序Configure 具体的数据。</p>
</td>
</tr>
...
<td class="cellrowborder" valign="top" width="62%" headers="mcps1.1.3.1.2 "><p xml:lang="sv-SE" id="p13034mcpsimp"><a name="p13034mcpsimp"></a><a name="p13034mcpsimp"></a>输出帧率，此参数暂时Not supported.</p>
</td>
</tr>
...
<td class="cellrowborder" valign="top" width="62%" headers="mcps1.1.3.1.2 "><p xml:lang="sv-SE" id="p13039mcpsimp"><a name="p13039mcpsimp"></a><a name="p13039mcpsimp"></a>输出管脚的乱序Configure 。</p>
</td>
</tr>
</tbody>
</table>
...
-   OOC校正数据只Supportedraw数据输入。
-   Configure 的cfg\_num最大值不能超过[OT\_VI\_MAX\_SNS\_CFG\_NUM](#ZH-CN_TOPIC_0000002441657781)。
...
<td class="cellrowborder" valign="top" width="62%" headers="mcps1.1.3.1.2 "><p xml:lang="sv-SE" id="p13075mcpsimp"><a name="p13075mcpsimp"></a><a name="p13075mcpsimp"></a>自产生时序Enabled开关。</p>
</td>
</tr>
...
-   使用此功能时，需Configure Sensor类型与RAW数据文件的宽高相等，如均 as 4000px\*3000px或者3840px\*2160px。
-   当Enabled自产生时序后，若用户设置的帧率超过设备最大帧率时，系统自动以设备最大帧率 as 有效值；同时用户设置的帧率必须大于0，否则返回错误。
-   设备最大帧率由设备corresponding 时钟clk、设备宽高共同决定。
...
<td class="cellrowborder" valign="top" width="62%" headers="mcps1.1.3.1.2 "><p xml:lang="sv-SE" id="p13110mcpsimp"><a name="p13110mcpsimp"></a><a name="p13110mcpsimp"></a>自产生数据Enabled开关。</p>
</td>
</tr>
</tbody>
</table>
...
<td class="cellrowborder" valign="top" width="59%" headers="mcps1.1.3.1.2 "><p id="p13142mcpsimp"><a name="p13142mcpsimp"></a><a name="p13142mcpsimp"></a>该VI Dev绑定的PIPE数目，Valid range[1, <a href="#ZH-CN_TOPIC_0000002441698249">OT_VI_MAX_PHYS_PIPE_NUM</a>]。</p>
</td>
</tr>
...
<td class="cellrowborder" valign="top" width="59%" headers="mcps1.1.3.1.2 "><p id="p13149mcpsimp"><a name="p13149mcpsimp"></a><a name="p13149mcpsimp"></a>该VI Dev绑定的PIPE number.</p>
</td>
</tr>
</tbody>
</table>
...
<td class="cellrowborder" valign="top" width="66%" headers="mcps1.1.3.1.2 "><p id="p13182mcpsimp"><a name="p13182mcpsimp"></a><a name="p13182mcpsimp"></a>WDR工作模式，分 as 帧模式、行模式、非WDR等三大类。</p>
</td>
</tr>
...
<td class="cellrowborder" valign="top" width="66%" headers="mcps1.1.3.1.2 "><p id="p184854599122"><a name="p184854599122"></a><a name="p184854599122"></a>WDR融合组主pipe反转标记，set to TRUE时最大的pipe_id as 融合主pipe，FALSE时最小的pipe_id as 融合主pipe。</p>
</td>
</tr>
</tbody>
</table>
...
<th class="cellrowborder" valign="top" width="46%" id="mcps1.1.4.1.2"><p id="p13206mcpsimp"><a name="p13206mcpsimp"></a><a name="p13206mcpsimp"></a>Supported的WDR模式</p>
</th>
<th class="cellrowborder" valign="top" width="22%" id="mcps1.1.4.1.3"><p id="p13208mcpsimp"><a name="p13208mcpsimp"></a><a name="p13208mcpsimp"></a>Supported缓存行数</p>
</th>
</tr>
</thead>
...
-   若Configure 后出现图像效果异常时，则需调节Configure 的卷绕行数。推荐configured as DEV高度的一半。
-   在线WDR时，会创建一个私有VB；离线WDR时，使用公共VB轮转。
-   WDR卷绕的Configure 可减小在线WDR私有VB的大小，仅在线WDR行模式才Supported.
-   线性模式可不Configure WDR组，若Configure ，则cache\_line不能configured as 0，否则会Configure Failure.
-   pipe\_reverse须配合[ss\_mpi\_vi\_set\_pipe\_vc\_number](#ZH-CN_TOPIC_0000002408258378)接口使用，在需要将长短帧交换FE处理的场景下使用：如2To1模式下FE0处理长帧，FE1处理短帧，需设置pipe\_reverse as TRUE，同时交换两路pipe的vc\_number。
-   Configure WDR组属性后，获取对应ispstatistics需要从corresponding WDR主pipe获取；如pipe0, 1做wdr融合，若Configure 了pipe\_reverse，则需要从pipe1\(isp1\)上获取statistics，此时isp通路也只需要起isp1，不需要Configure isp0。
...
<p id="p13365mcpsimp"><a name="p13365mcpsimp"></a><a name="p13365mcpsimp"></a>Static Attribute<span xml:lang="sv-SE" id="ph13366mcpsimp"><a name="ph13366mcpsimp"></a><a name="ph13366mcpsimp"></a>，</span>创建<span xml:lang="sv-SE" id="ph13367mcpsimp"><a name="ph13367mcpsimp"></a><a name="ph13367mcpsimp"></a>PIPE</span>时设定<span xml:lang="sv-SE" id="ph13368mcpsimp"><a name="ph13368mcpsimp"></a><a name="ph13368mcpsimp"></a>，</span>不可更改。</p>
</td>
</tr>
...
<p id="p13378mcpsimp"><a name="p13378mcpsimp"></a><a name="p13378mcpsimp"></a>Online Mode下宽高Valid range<span xml:lang="sv-SE" id="ph13379mcpsimp"><a name="ph13379mcpsimp"></a><a name="ph13379mcpsimp"></a>：</span></p>
...
<p xml:lang="sv-SE" id="p13390mcpsimp"><a name="p13390mcpsimp"></a><a name="p13390mcpsimp"></a>Offline mode下Valid range: </p>
...
<td class="cellrowborder" valign="top" width="74%" headers="mcps1.1.3.1.2 "><p id="p13410mcpsimp"><a name="p13410mcpsimp"></a><a name="p13410mcpsimp"></a>数据压缩格式。具体信息See“System Control”章节。</p>
</td>
</tr>
...
<p id="p13416mcpsimp"><a name="p13416mcpsimp"></a><a name="p13416mcpsimp"></a>Static Attribute<span xml:lang="sv-SE" id="ph13417mcpsimp"><a name="ph13417mcpsimp"></a><a name="ph13417mcpsimp"></a>，</span>创建<span xml:lang="sv-SE" id="ph13418mcpsimp"><a name="ph13418mcpsimp"></a><a name="ph13418mcpsimp"></a>PIPE</span>时设定<span xml:lang="sv-SE" id="ph13419mcpsimp"><a name="ph13419mcpsimp"></a><a name="ph13419mcpsimp"></a>，</span>不可更改，仅当Pixel formatpixel_format as YUVPixel format时有效。</p>
</td>
</tr>
...
<p id="p13430mcpsimp"><a name="p13430mcpsimp"></a><a name="p13430mcpsimp"></a>Valid rangeSee“System Control”章节。</p>
</td>
</tr>
</tbody>
</table>
...
<th class="cellrowborder" valign="top" width="19.801980198019802%" id="mcps1.1.5.1.2"><p id="p13443mcpsimp"><a name="p13443mcpsimp"></a><a name="p13443mcpsimp"></a>pixel_format的Valid range差异：</p>
</th>
<th class="cellrowborder" valign="top" width="26.732673267326728%" id="mcps1.1.5.1.3"><p id="p13445mcpsimp"><a name="p13445mcpsimp"></a><a name="p13445mcpsimp"></a>compress_mode<span xml:lang="sv-SE" id="ph13446mcpsimp"><a name="ph13446mcpsimp"></a><a name="ph13446mcpsimp"></a>的Valid range差异：</span></p>
</th>
<th class="cellrowborder" valign="top" width="33.663366336633665%" id="mcps1.1.5.1.4"><p id="p13448mcpsimp"><a name="p13448mcpsimp"></a><a name="p13448mcpsimp"></a>pipe_bypass_mode<span xml:lang="sv-SE" id="ph13449mcpsimp"><a name="ph13449mcpsimp"></a><a name="ph13449mcpsimp"></a>的Valid range差异：</span></p>
</th>
</tr>
</thead>
...
<td class="cellrowborder" valign="top" width="19.801980198019802%" headers="mcps1.1.5.1.2 "><p id="p13455mcpsimp"><a name="p13455mcpsimp"></a><a name="p13455mcpsimp"></a>SupportedRAW格式和YUV数据格式。</p>
</td>
<td class="cellrowborder" valign="top" width="26.732673267326728%" headers="mcps1.1.5.1.3 "><p xml:lang="sv-SE" id="p13457mcpsimp"><a name="p13457mcpsimp"></a><a name="p13457mcpsimp"></a>线性in mode，Supported非压、行压缩和帧压缩模式。</p>
<p xml:lang="sv-SE" id="p13458mcpsimp"><a name="p13458mcpsimp"></a><a name="p13458mcpsimp"></a>WDRin mode，Supported非压和行压缩模式；帧压缩模式只有WDR离线Supported，WDR在线Not supported.</p>
</td>
<td class="cellrowborder" valign="top" width="33.663366336633665%" headers="mcps1.1.5.1.4 "><p xml:lang="sv-SE" id="p13460mcpsimp"><a name="p13460mcpsimp"></a><a name="p13460mcpsimp"></a>虚拟PIPE只SupportedOT_VI_PIPE_BYPASS_FE。</p>
</td>
</tr>
</tbody>
</table>
...
-   pixel\_format定义当前PIPE接收的图像Pixel format，当 as Raw数据时，可以动态修改 as 不同位宽的Pixel format（位宽低于实际输入Raw位宽时有精度损失），当 as YUV格式数据时，可以动态修改 as SP420，SP422，但是建议与输入保持一致，否则可能导致图像异常。
-   isp\_bypass表示是否bypass ISP的算法。
    -   当输入 as Raw数据时，须set to TD\_FALSE才能正常输出通道YUV数据。
    -   当输入 as YUV时，则可以set to TD\_TRUE或者TD\_FALSE。
    -   当YUV需要获取AFstatistics和YUV图像处理时，isp\_bypass参数设置成TD\_FALSE；
    -   当YUV不需要经过ISP处理时，isp\_bypass参数设置成TD\_TRUE，同时强烈建议不起ISP业务，避免内存等资源的浪费。

-   当参数pipe\_bypass\_mode设置成OT\_VI\_PIPE\_BYPASS\_BE时，Not supported从该PIPE灌YUV。
-   当参数pipe\_bypass\_mode设置成OT\_VI\_PIPE\_BYPASS\_BE时，PIPE只能处于OT\_VI\_OFFLINE\_VPSS\_OFFLINE模式。
-   compress\_mode表示压缩类型，有Supported的数据类型都Supported非压缩。当输入数据Pixel format as OT\_PIXEL\_FORMAT\_RGB\_BAYER\_10BPP，OT\_PIXEL\_FORMAT\_RGB\_BAYER\_12BPP，OT\_PIXEL\_FORMAT\_RGB\_BAYER\_14BPP时，线性模式下压缩类型SupportedOT\_COMPRESS\_MODE\_LINE和OT\_COMPRESS\_MODE\_FRAME，WDR模式下SupportedOT\_COMPRESS\_MODE\_LINE，当输入数据 as YUV数据时Compression not supported。
-   Pixel format as OT\_PIXEL\_FORMAT\_RGB\_BAYER\_16BPP和OT\_PIXEL\_FORMAT\_RGB\_BAYER\_8BPP时，Compression not supported。
-   参数bit\_width表示输入数据类型 as YUV时的数据位宽，不能动态设置，SS928V100Not supported10bit的YUV数据输入。
-   参数size需与ot\_isp\_pub\_attr的wnd\_rect宽高保持一致，宽高必须保持4对齐。
-   PIPE的帧率控制只有离线或者Enabled了dump属性才有效。
-   bit\_align\_mode通常场景下都设置成OT\_VI\_BIT\_ALIGN\_MODE\_HIGH，VI离线场景下如果需要设置成OT\_VI\_BIT\_ALIGN\_MODE\_LOW，需要设置frame\_source as OT\_VI\_PIPE\_FRAME\_SOURCE\_USER，不能送给pipe处理，否则后端图像会出现异常（因 as 低位对齐后数据变小而成黑色图像）。
...
<td class="cellrowborder" valign="top" width="75%" headers="mcps1.1.3.1.2 "><p id="p13512mcpsimp"><a name="p13512mcpsimp"></a><a name="p13512mcpsimp"></a>是否Enableddump图像帧。</p>
</td>
</tr>
...
<td class="cellrowborder" valign="top" width="75%" headers="mcps1.1.3.1.2 "><p id="p13517mcpsimp"><a name="p13517mcpsimp"></a><a name="p13517mcpsimp"></a>Dump图像帧的队列深度。Valid range: [0, 8]。</p>
</td>
</tr>
</tbody>
</table>
...
<td class="cellrowborder" valign="top" width="75%" headers="mcps1.1.3.1.2 "><p id="p13600mcpsimp"><a name="p13600mcpsimp"></a><a name="p13600mcpsimp"></a>是否Enableddump私有数据。</p>
</td>
</tr>
...
<td class="cellrowborder" valign="top" width="75%" headers="mcps1.1.3.1.2 "><p id="p13610mcpsimp"><a name="p13610mcpsimp"></a><a name="p13610mcpsimp"></a>Dump私有数据的队列深度。Valid range: [0, 8]。</p>
</td>
</tr>
...
-   out\_mode 仅Supported在start pipe之前修改，pipe 启动后再修改out\_mode将返回报错。
-   out\_mode configured as stagger模式，与pipe pre\_crop功能、设备bas功能互斥。
-   VIOnline ModeNot supported修改out\_mode as stagger模式。
...
<td class="cellrowborder" valign="top" width="55.00000000000001%" headers="mcps1.1.3.1.2 "><p id="p13832mcpsimp"><a name="p13832mcpsimp"></a><a name="p13832mcpsimp"></a>当前PIPE是否Enabled。</p>
</td>
</tr>
...
-   当PIPE处于VI离线，VPSSOffline mode，有字段都有效。
-   当PIPE处于VI离线，VPSSOnline Mode，有字段都有效。
-   当Enabled了用户图片，frame\_rateinvalid。
-   用户从BE送RAW，送YUV不会统计到frame\_rate中。
-   用户从FE送RAW，当PIPE处于VI在线，VPSS离线或者VI在线，VPSS在线，如果未Enableddump功能，由于None帧数据输出，则frame\_rateinvalid。
...
<p id="p13950mcpsimp"><a name="p13950mcpsimp"></a><a name="p13950mcpsimp"></a>Online Mode下宽度<span xml:lang="de-DE" id="ph13951mcpsimp"><a name="ph13951mcpsimp"></a><a name="ph13951mcpsimp"></a>：</span></p>
...
<p id="p13955mcpsimp"><a name="p13955mcpsimp"></a><a name="p13955mcpsimp"></a>Online Mode下高度<span xml:lang="de-DE" id="ph13956mcpsimp"><a name="ph13956mcpsimp"></a><a name="ph13956mcpsimp"></a>：</span></p>
...
<p id="p13960mcpsimp"><a name="p13960mcpsimp"></a><a name="p13960mcpsimp"></a>Offline mode下宽度<span xml:lang="de-DE" id="ph13961mcpsimp"><a name="ph13961mcpsimp"></a><a name="ph13961mcpsimp"></a>：</span></p>
...
<p id="p13965mcpsimp"><a name="p13965mcpsimp"></a><a name="p13965mcpsimp"></a>Offline mode下高度<span xml:lang="de-DE" id="ph13966mcpsimp"><a name="ph13966mcpsimp"></a><a name="ph13966mcpsimp"></a>：</span></p>
...
<td class="cellrowborder" valign="top" width="75%" headers="mcps1.1.3.1.2 "><p id="p13974mcpsimp"><a name="p13974mcpsimp"></a><a name="p13974mcpsimp"></a><span xml:lang="sv-SE" id="ph13975mcpsimp"><a name="ph13975mcpsimp"></a><a name="ph13975mcpsimp"></a>目标图像Pixel format。</span>具体描述see “System Control”章节。</p>
...
<p id="p13983mcpsimp"><a name="p13983mcpsimp"></a><a name="p13983mcpsimp"></a>Static Attribute，设置CHN时设定，不可更改。</p>
<p id="p13984mcpsimp"><a name="p13984mcpsimp"></a><a name="p13984mcpsimp"></a>具体描述see “System Control”章节。</p>
</td>
</tr>
...
<p id="p13990mcpsimp"><a name="p13990mcpsimp"></a><a name="p13990mcpsimp"></a>具体描述see “System Control”章节。</p>
</td>
</tr>
...
<p id="p13996mcpsimp"><a name="p13996mcpsimp"></a><a name="p13996mcpsimp"></a>具体描述see “System Control”章节。</p>
</td>
</tr>
...
<td class="cellrowborder" valign="top" width="75%" headers="mcps1.1.3.1.2 "><p xml:lang="sv-SE" id="p14001mcpsimp"><a name="p14001mcpsimp"></a><a name="p14001mcpsimp"></a>MirrorEnabled开关。</p>
...
<td class="cellrowborder" valign="top" width="75%" headers="mcps1.1.3.1.2 "><p xml:lang="sv-SE" id="p14008mcpsimp"><a name="p14008mcpsimp"></a><a name="p14008mcpsimp"></a>FlipEnabled开关。</p>
...
<td class="cellrowborder" valign="top" width="75%" headers="mcps1.1.3.1.2 "><p xml:lang="sv-SE" id="p14015mcpsimp"><a name="p14015mcpsimp"></a><a name="p14015mcpsimp"></a>用户获取图像的队列深度，Valid range: [0, 8]</p>
</td>
</tr>
...
<p xml:lang="sv-SE" id="p14021mcpsimp"><a name="p14021mcpsimp"></a><a name="p14021mcpsimp"></a>源帧率Valid range: (0, 240]，以及-1。</p>
<p xml:lang="sv-SE" id="p14022mcpsimp"><a name="p14022mcpsimp"></a><a name="p14022mcpsimp"></a>目标帧率Valid range: [-1, 240]</p>
<p id="p14023mcpsimp"><a name="p14023mcpsimp"></a><a name="p14023mcpsimp"></a>当源帧率 as -1时，目标帧率must be -1(不进行帧率控制)，其他情况下，目标帧率不能大于源帧率<span xml:lang="sv-SE" id="ph14024mcpsimp"><a name="ph14024mcpsimp"></a><a name="ph14024mcpsimp"></a>。</span></p>
<p id="p14025mcpsimp"><a name="p14025mcpsimp"></a><a name="p14025mcpsimp"></a>具体描述see “System Control”章节。</p>
</td>
</tr>
</tbody>
</table>
...
-   若LDC/ROTATION\_FREE/SPREAD/DISEnabled，size不可动态更改，视频数据格式 as 线性。
-   如果ROTATION已经设置成90/180/270旋转，Channel attribute不能设置成压缩。
-   若GME DISEnabled，不能EnabledMIRROR，FLIP，缩放，CROP。
-   宽高必须4对齐。
...
<td class="cellrowborder" valign="top" width="78%" headers="mcps1.1.3.1.2 "><p xml:lang="sv-SE" id="p14074mcpsimp"><a name="p14074mcpsimp"></a><a name="p14074mcpsimp"></a>绑定的源Physical channel。</p>
</td>
</tr>
...
<td class="cellrowborder" valign="top" width="78%" headers="mcps1.1.3.1.2 "><p xml:lang="sv-SE" id="p14079mcpsimp"><a name="p14079mcpsimp"></a><a name="p14079mcpsimp"></a>定义扩展channel图像来源。</p>
<p id="p14080mcpsimp"><a name="p14080mcpsimp"></a><a name="p14080mcpsimp"></a>具体描述see “System Control”章节。</p>
</td>
</tr>
...
<td class="cellrowborder" valign="top" width="78%" headers="mcps1.1.3.1.2 "><p id="p14106mcpsimp"><a name="p14106mcpsimp"></a><a name="p14106mcpsimp"></a><span xml:lang="sv-SE" id="ph14107mcpsimp"><a name="ph14107mcpsimp"></a><a name="ph14107mcpsimp"></a>目标图像Pixel format。</span>具体描述see “System Control”章节。</p>
</td>
</tr>
...
<td class="cellrowborder" valign="top" width="78%" headers="mcps1.1.3.1.2 "><p id="p14112mcpsimp"><a name="p14112mcpsimp"></a><a name="p14112mcpsimp"></a><span xml:lang="sv-SE" id="ph14113mcpsimp"><a name="ph14113mcpsimp"></a><a name="ph14113mcpsimp"></a>目标图像视频数据格式。</span>具体描述see “System Control”章节。</p>
</td>
</tr>
...
<td class="cellrowborder" valign="top" width="78%" headers="mcps1.1.3.1.2 "><p id="p14118mcpsimp"><a name="p14118mcpsimp"></a><a name="p14118mcpsimp"></a><span xml:lang="sv-SE" id="ph14119mcpsimp"><a name="ph14119mcpsimp"></a><a name="ph14119mcpsimp"></a>目标图像动态范围</span>。具体描述see “System Control”章节。</p>
</td>
</tr>
...
<td class="cellrowborder" valign="top" width="78%" headers="mcps1.1.3.1.2 "><p id="p14124mcpsimp"><a name="p14124mcpsimp"></a><a name="p14124mcpsimp"></a><span xml:lang="sv-SE" id="ph14125mcpsimp"><a name="ph14125mcpsimp"></a><a name="ph14125mcpsimp"></a>目标图像压缩格式。</span>具体描述see “System Control”章节。</p>
</td>
</tr>
...
<td class="cellrowborder" valign="top" width="78%" headers="mcps1.1.3.1.2 "><p xml:lang="sv-SE" id="p14130mcpsimp"><a name="p14130mcpsimp"></a><a name="p14130mcpsimp"></a>用户获取图像的队列深度，Valid range: [0, 8]。</p>
</td>
</tr>
...
<p xml:lang="sv-SE" id="p14136mcpsimp"><a name="p14136mcpsimp"></a><a name="p14136mcpsimp"></a>源帧率Valid range: (0, 240]，以及-1。</p>
<p xml:lang="sv-SE" id="p14137mcpsimp"><a name="p14137mcpsimp"></a><a name="p14137mcpsimp"></a>目标帧率Valid range: [-1, 240]。</p>
<p id="p14138mcpsimp"><a name="p14138mcpsimp"></a><a name="p14138mcpsimp"></a>当源帧率 as -1时，目标帧率must be -1(不进行帧率控制)，其他情况下，目标帧率不能大于源帧率<span xml:lang="sv-SE" id="ph14139mcpsimp"><a name="ph14139mcpsimp"></a><a name="ph14139mcpsimp"></a>。</span></p>
<p id="p14140mcpsimp"><a name="p14140mcpsimp"></a><a name="p14140mcpsimp"></a>具体描述see “System Control”章节。</p>
</td>
</tr>
</tbody>
</table>
...
<td class="cellrowborder" valign="top" width="75%" headers="mcps1.1.3.1.2 "><p id="p14177mcpsimp"><a name="p14177mcpsimp"></a><a name="p14177mcpsimp"></a>CROPEnabled开关。</p>
</td>
</tr>
...
<p xml:lang="sv-SE" id="p14189mcpsimp"><a name="p14189mcpsimp"></a><a name="p14189mcpsimp"></a>OT_COORD_ABS<span xml:lang="en-US" id="ph14190mcpsimp"><a name="ph14190mcpsimp"></a><a name="ph14190mcpsimp"></a>模式下的Valid rangeas follows</span>：</p>
...
-   若crop\_mode as OT\_COORD\_RATIO（相对坐标模式），使用rect的成员时应做转换，Calculation Formula as ：
    -   x = 起始点坐标x原始图像宽度/1000，合法Valid range\[0, 999\]，计算完成后会进行取整操作和对齐操作。公式同样适用于纵坐标计算，X/Y坐标都是2对齐。
    -   width =区域宽度x实际图像宽度/1000，区域宽度Valid range\[1, 1000\]。计算完成后会进行取整操作和2字节对齐操作。公式同样适用于区域高度计算。
    -   计算后的宽高不能小于通道Supported的最小宽高。

-   若crop\_mode as OT\_COORD\_ABS（绝对坐标模式），X/Y坐标都是要求2对齐，宽高要求2对齐。
...
<td class="cellrowborder" valign="top" width="55.00000000000001%" headers="mcps1.1.3.1.2 "><p id="p14252mcpsimp"><a name="p14252mcpsimp"></a><a name="p14252mcpsimp"></a>当前通道是否Enabled。</p>
...
<td class="cellrowborder" valign="top" width="55.00000000000001%" headers="mcps1.1.3.1.2 "><p id="p14259mcpsimp"><a name="p14259mcpsimp"></a><a name="p14259mcpsimp"></a>VI channel实时帧率。</p>
</td>
</tr>
...
-   当PIPE处于VI离线，VPSSOffline mode，有字段都有效。
-   当PIPE处于VI离线，VPSSOnline Mode，this interfaceNot supported.
-   当PIPE处于VI在线，VPSSOffline mode，有字段都有效。
-   当PIPE处于VI在线，VPSSOnline Mode，this interfaceNot supported.
...
<td class="cellrowborder" valign="top" width="53%" headers="mcps1.1.3.1.2 "><p id="p11728mcpsimp"><a name="p11728mcpsimp"></a><a name="p11728mcpsimp"></a>拼接组内的PIPE独立使用对应ISP的Configure 。</p>
</td>
</tr>
...
<td class="cellrowborder" valign="top" width="53%" headers="mcps1.1.3.1.2 "><p id="p11733mcpsimp"><a name="p11733mcpsimp"></a><a name="p11733mcpsimp"></a>拼接组内的PIPE共同使用拼接组内第0个PIPE对应ISP的Configure 。</p>
</td>
</tr>
</tbody>
</table>
...
<td class="cellrowborder" valign="top" width="75%" headers="mcps1.1.3.1.2 "><p id="p11770mcpsimp"><a name="p11770mcpsimp"></a><a name="p11770mcpsimp"></a>是否Enabled拼接组。</p>
</td>
</tr>
...
<td class="cellrowborder" valign="top" width="75%" headers="mcps1.1.3.1.2 "><p id="p11775mcpsimp"><a name="p11775mcpsimp"></a><a name="p11775mcpsimp"></a>拼接组ISPConfigure 的同步方式。</p>
</td>
</tr>
...
-   一个PIPE最多只能属于一个拼接组。
-   SYNC模式Not supported虚拟pipe设置拼接属性。
...
-   仅SS928V100SupportedDIS的OT\_DIS\_MODE\_GYRO模式。
-   仅SS928V100SupportedDIS的OT\_DIS\_MODE\_HYBRID模式，HYBRID模式Supported的最大图像分辨率 as 4096x4096。
...
<td class="cellrowborder" valign="top" width="45%" headers="mcps1.1.3.1.2 "><p id="p12059mcpsimp"><a name="p12059mcpsimp"></a><a name="p12059mcpsimp"></a>None人机产品形态。</p>
</td>
</tr>
</tbody>
</table>
...
<td class="cellrowborder" valign="top" width="76%" headers="mcps1.1.3.1.2 "><p id="p12113mcpsimp"><a name="p12113mcpsimp"></a><a name="p12113mcpsimp"></a>使用DIS的产品形态，当前Supported<em id="i12114mcpsimp"><a name="i12114mcpsimp"></a><a name="i12114mcpsimp"></a>录像机</em>、DV和None人机。</p>
</td>
</tr>
...
<p id="p12120mcpsimp"><a name="p12120mcpsimp"></a><a name="p12120mcpsimp"></a>Valid range: [5,10]。仅对GME和HYBRID模式有效。</p>
</td>
</tr>
...
<p id="p12134mcpsimp"><a name="p12134mcpsimp"></a><a name="p12134mcpsimp"></a>请set to VI输出的实际帧率。</p>
<p id="p12135mcpsimp"><a name="p12135mcpsimp"></a><a name="p12135mcpsimp"></a> as 了获得较好的防抖效果，建议帧率25帧以.</p>
<a name="ul12136mcpsimp"></a><a name="ul12136mcpsimp"></a><ul id="ul12136mcpsimp"><li>当防抖mode isOT_DIS_MODE_4_DOF_GME/OT_DIS_MODE_6_DOF_GME时，低帧率会导致两帧间隔变大，两帧之间抖动幅度增加，同时，rolling shutter现象也更加严重，预期可能会出现防抖效果下降，特别是在抖动幅度大的场景;</li><li>当防抖mode isDIS_MODE_GYRO；除了对防抖效果的影响和GME一致外，随着帧率的降低，陀螺仪的最高采样率将下降。当最小帧率 as 20时，陀螺仪采样率不能高于2.5KHz。当最小帧率 as 15时，陀螺仪采样率不能高于1.8KHz。</li></ul>
</td>
</tr>
...
<p id="p12149mcpsimp"><a name="p12149mcpsimp"></a><a name="p12149mcpsimp"></a><span xml:lang="sv-SE" id="ph12150mcpsimp"><a name="ph12150mcpsimp"></a><a name="ph12150mcpsimp"></a>scale</span> as false的情况下输出图像宽高等于输入宽高乘以<span xml:lang="sv-SE" id="ph12151mcpsimp"><a name="ph12151mcpsimp"></a><a name="ph12151mcpsimp"></a>crop_ratio</span>，输出宽高是2对齐的。</p>
</td>
</tr>
</tbody>
</table>
...
-   camera\_steady只在_录像机_产品形态下有效，在DV和None人机产品形态下会默认set to TD\_FALSE。
-   ot\_dis\_cfg中的参数除crop\_ratio外，其他参数都不可动态修改，如果需要修改，必须先关闭DIS，再动态修改ot\_dis\_cfg中的参数。
-   对于crop\_ratio参数：
    -   scale as TRUE时，crop\_ratio可以调用接口[ss\_mpi\_vi\_set\_chn\_dis\_cfg](#ZH-CN_TOPIC_0000002408258606)动态修改。
    -   scale as FALSE时，crop\_ratio不可以动态修改，必须先关闭DIS，再动态修改crop\_ratio值。

-   对于参数未有任何修改，但调用[ss\_mpi\_vi\_set\_chn\_dis\_cfg](#ZH-CN_TOPIC_0000002408258606)重复设置时，接口返回Success.
...
<td class="cellrowborder" valign="top" width="61%" headers="mcps1.1.3.1.2 "><p id="p12221mcpsimp"><a name="p12221mcpsimp"></a><a name="p12221mcpsimp"></a>DISEnabled开关。</p>
...
<p xml:lang="sv-SE" id="p12231mcpsimp"><a name="p12231mcpsimp"></a><a name="p12231mcpsimp"></a>TD_FALSE：<span xml:lang="en-US" id="ph12232mcpsimp"><a name="ph12232mcpsimp"></a><a name="ph12232mcpsimp"></a>Not enabled</span>Bypass GDC<span xml:lang="en-US" id="ph12233mcpsimp"><a name="ph12233mcpsimp"></a><a name="ph12233mcpsimp"></a>功能</span>，<span xml:lang="en-US" id="ph12234mcpsimp"><a name="ph12234mcpsimp"></a><a name="ph12234mcpsimp"></a>, i.e., 正常调用</span>GDC<span xml:lang="en-US" id="ph12235mcpsimp"><a name="ph12235mcpsimp"></a><a name="ph12235mcpsimp"></a>做</span>PMF<span xml:lang="en-US" id="ph12236mcpsimp"><a name="ph12236mcpsimp"></a><a name="ph12236mcpsimp"></a>矫正</span>；</p>
<p xml:lang="sv-SE" id="p12237mcpsimp"><a name="p12237mcpsimp"></a><a name="p12237mcpsimp"></a>TD_TRUE：<span xml:lang="en-US" id="ph12238mcpsimp"><a name="ph12238mcpsimp"></a><a name="ph12238mcpsimp"></a>Enabled</span>Bypass GDC<span xml:lang="en-US" id="ph12239mcpsimp"><a name="ph12239mcpsimp"></a><a name="ph12239mcpsimp"></a>功能</span>，<span xml:lang="en-US" id="ph12240mcpsimp"><a name="ph12240mcpsimp"></a><a name="ph12240mcpsimp"></a>, i.e., 不调用</span>GDC<span xml:lang="en-US" id="ph12241mcpsimp"><a name="ph12241mcpsimp"></a><a name="ph12241mcpsimp"></a>做</span>PMF<span xml:lang="en-US" id="ph12242mcpsimp"><a name="ph12242mcpsimp"></a><a name="ph12242mcpsimp"></a>矫正。</span></p>
</td>
</tr>
...
<td class="cellrowborder" valign="top" width="61%" headers="mcps1.1.3.1.2 "><p id="p12247mcpsimp"><a name="p12247mcpsimp"></a><a name="p12247mcpsimp"></a>用于判断物体是否是运动的级别，Valid range: [0,7]。仅对GME和HYBRID模式有效。级别越大，防抖效果越弱。值越小，运动过程中越稳定，但更容易出现偏移情况。值较大时，运动过程中背景抖动幅度大，但能够改善图像偏移现象。</p>
...
<td class="cellrowborder" valign="top" width="61%" headers="mcps1.1.3.1.2 "><p id="p12259mcpsimp"><a name="p12259mcpsimp"></a><a name="p12259mcpsimp"></a>帧起始时间和陀螺仪数据采集时间的时间差，Valid range: [-2000000, 2000000]，单位 as 微秒。仅对GYRO和HYBRID模式有效。当值设置较大时会发生丢帧卡死。</p>
</td>
</tr>
...
<td class="cellrowborder" valign="top" width="61%" headers="mcps1.1.3.1.2 "><p id="p12264mcpsimp"><a name="p12264mcpsimp"></a><a name="p12264mcpsimp"></a>水平偏移限制，Valid range: [0,1000]。仅对GME和HYBRID模式有效。当大面积物体经过引起背景拖拽的水平偏移超过一定幅度时就不进行防抖。偏移幅度计算：2047 * <span xml:lang="sv-SE" id="ph12265mcpsimp"><a name="ph12265mcpsimp"></a><a name="ph12265mcpsimp"></a>hor_limit</span>/1000。</p>
</td>
</tr>
...
<td class="cellrowborder" valign="top" width="61%" headers="mcps1.1.3.1.2 "><p id="p197662045135510"><a name="p197662045135510"></a><a name="p197662045135510"></a>DIS防抖效果开关，defaults to FALSE，当DISEnabled as TRUE时生效。</p>
...
<td class="cellrowborder" valign="top" width="61%" headers="mcps1.1.3.1.2 "><p id="p12281mcpsimp"><a name="p12281mcpsimp"></a><a name="p12281mcpsimp"></a>陀螺仪防抖的强度控制，Valid range: [0,1024]。仅对GYRO和HYBRID模式有效，对GME模式invalid，默认强度 as 1024。</p>
</td>
</tr>
</tbody>
</table>
...
-   hor\_limit和ver\_limit只在camera\_steady as TD\_TRUE时生效，当camera\_steady as TD\_FALSE时，会默认set to 最大值。
-   在SS928V100上不建议使用gdc\_bypass参数作 as DIS功能开关，默认设置其值 as TD\_FALSE，当set to TD\_TRUE时，DIS的PMF矫正功能不生效。使用中，如果不想EnabledDIS，请设置DIS开关enable as TD\_FALSE；如果想EnabledDIS,请把DIS开关enableset to TD\_TRUE，同时把gdc\_bypassset to TD\_FALSE。
...
<td class="cellrowborder" valign="top" width="61%" headers="mcps1.1.3.1.2 "><p id="p12320mcpsimp"><a name="p12320mcpsimp"></a><a name="p12320mcpsimp"></a>大幅度运动的防抖衰减参数。Valid range[0, 100]。Default value及具体参数效果参见《DIS 调试指南》。</p>
</td>
</tr>
...
<td class="cellrowborder" valign="top" width="61%" headers="mcps1.1.3.1.2 "><p id="p12325mcpsimp"><a name="p12325mcpsimp"></a><a name="p12325mcpsimp"></a>低频运动的保留程度。Valid range[0, 100]。Default value及具体参数效果参见《DIS 调试指南》。</p>
</td>
</tr>
...
<td class="cellrowborder" valign="top" width="61%" headers="mcps1.1.3.1.2 "><p id="p12330mcpsimp"><a name="p12330mcpsimp"></a><a name="p12330mcpsimp"></a>低频运动的截止频率。Valid range[0, 100]。Default value及具体参数效果参见《DIS 调试指南》。</p>
</td>
</tr>
...
<td class="cellrowborder" valign="top" width="61%" headers="mcps1.1.3.1.2 "><p id="p159154192110"><a name="p159154192110"></a><a name="p159154192110"></a>自适应查找特征点对数阈值的开关。Default value及具体参数效果参见《DIS 调试指南》。</p>
...
-   参数只在GME和HYBRIDvalid in mode，不Configure 使用默认参数。
-   算法特征点对数的阈值默认是30对，fpd\_adaptive\_enEnabled后会自适应降低阈值，最低降低到10对，fpd\_adaptive\_en关闭、enable开关关闭或still\_crop开关打开后，特征点阈值都会恢复成默认的30。
...
<p id="p2418163145716"><a name="p2418163145716"></a><a name="p2418163145716"></a>WDRmode is2TO1时Valid range[0, 1]，0 as 匹配短帧，1 as 匹配长帧；</p>
<p id="p12491627105712"><a name="p12491627105712"></a><a name="p12491627105712"></a>WDRmode is3TO1时Valid range[0, 2]，0 as 匹配短帧，1 as 匹配中帧，2 as 匹配长帧。</p>
</td>
</tr>
</tbody>
</table>
...
参数只在GYROvalid in mode，不Configure 使用默认参数
...
<td class="cellrowborder" valign="top" width="61%" headers="mcps1.1.3.1.2 "><p id="p12366mcpsimp"><a name="p12366mcpsimp"></a><a name="p12366mcpsimp"></a>视场角是否Enabled。</p>
</td>
</tr>
...
视频输入APIError Codeas follows。

**Table 1**  视频输入APIError Code
...
<td class="cellrowborder" valign="top" width="31.683168316831683%" headers="mcps1.2.4.1.3 "><p id="p12403mcpsimp"><a name="p12403mcpsimp"></a><a name="p12403mcpsimp"></a>视频输入设备号invalid</p>
</td>
</tr>
...
<td class="cellrowborder" valign="top" width="31.683168316831683%" headers="mcps1.2.4.1.3 "><p id="p12410mcpsimp"><a name="p12410mcpsimp"></a><a name="p12410mcpsimp"></a>视频输入PIPE号invalid</p>
</td>
</tr>
...
<td class="cellrowborder" valign="top" width="31.683168316831683%" headers="mcps1.2.4.1.3 "><p id="p12417mcpsimp"><a name="p12417mcpsimp"></a><a name="p12417mcpsimp"></a>视频输入Channel numberinvalid</p>
</td>
</tr>
...
<td class="cellrowborder" valign="top" width="31.683168316831683%" headers="mcps1.2.4.1.3 "><p id="p12424mcpsimp"><a name="p12424mcpsimp"></a><a name="p12424mcpsimp"></a>视频输入组号invalid</p>
</td>
</tr>
...
<td class="cellrowborder" valign="top" width="31.683168316831683%" headers="mcps1.2.4.1.3 "><p id="p12431mcpsimp"><a name="p12431mcpsimp"></a><a name="p12431mcpsimp"></a>视频输入参数设置invalid</p>
</td>
</tr>
...
<td class="cellrowborder" valign="top" width="31.683168316831683%" headers="mcps1.2.4.1.3 "><p id="p12445mcpsimp"><a name="p12445mcpsimp"></a><a name="p12445mcpsimp"></a>视频设备或Channel attribute未Configure </p>
</td>
</tr>
...
<td class="cellrowborder" valign="top" width="31.683168316831683%" headers="mcps1.2.4.1.3 "><p id="p12452mcpsimp"><a name="p12452mcpsimp"></a><a name="p12452mcpsimp"></a>操作Not supported</p>
</td>
</tr>
...
<td class="cellrowborder" valign="top" width="31.683168316831683%" headers="mcps1.2.4.1.3 "><p id="p12487mcpsimp"><a name="p12487mcpsimp"></a><a name="p12487mcpsimp"></a>视频输入缓存 as 空</p>
</td>
</tr>
...
<td class="cellrowborder" valign="top" width="31.683168316831683%" headers="mcps1.2.4.1.3 "><p id="p12494mcpsimp"><a name="p12494mcpsimp"></a><a name="p12494mcpsimp"></a>视频输入缓存 as 满</p>
</td>
</tr>
...
<td class="cellrowborder" valign="top" width="31.683168316831683%" headers="mcps1.2.4.1.3 "><p id="p12501mcpsimp"><a name="p12501mcpsimp"></a><a name="p12501mcpsimp"></a>视频输入系统未Initialization</p>
</td>
</tr>
...
<td class="cellrowborder" valign="top" width="31.683168316831683%" headers="mcps1.2.4.1.3 "><p id="p12508mcpsimp"><a name="p12508mcpsimp"></a><a name="p12508mcpsimp"></a>视频Configure 属性超时</p>
</td>
</tr>

...
flowchart LR cap[Capture<br>checkerboard pairs] --> calib[Camera calibration] calib --> rect[Stereo rectification] rect --> tune[Disparity tuning<br>preview] tune --> lut[LUT generation] lut --> board[Load on board → DPU runtime]
``` ## Where to go next The full tool walkthrough — installation, capturing calibration
...
your application code), see the [Motion Fusion](../../motionfusion/index.md)
chapter and the
[Open Harmony / Linux peripheral interface](../../../soc-linux/peripherals/index.md).
